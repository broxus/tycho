...
impl<Fut: Future> Shared<Fut> {
    ...
    pub fn weak_future(&self) -> Option<WeakSharedFuture<Fut>> {
        self.inner.as_ref().map(|inner| WeakSharedFuture {
...
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct WeakSharedFuture<Fut: Future> {
    inner: Option<Weak<Inner<Fut>>>,
    permit_fut: Option<SyncBoxFuture<Result<OwnedSemaphorePermit, AcquireError>>>,
    permit: Option<OwnedSemaphorePermit>,
}

impl<Fut> Future for WeakSharedFuture<Fut>
where
    Fut: Future,
    Fut::Output: Clone,
{
