...
impl proto::ControlServer for ControlServer {
    ...
    async fn get_block(
        ...
        let data = blocks.load_block_data_uncompressed(&handle).await?;
    ...
    async fn get_block_proof(
        ...
        let data = Bytes::from(blocks.load_block_proof_raw(&handle).await?);
        Ok(proto::BlockResponse::Found { data })
    }
    ...
    async fn get_queue_diff(
        ...
        let data = Bytes::from(blocks.load_queue_diff_raw(&handle).await?);
        Ok(proto::BlockResponse::Found { data })
    }
