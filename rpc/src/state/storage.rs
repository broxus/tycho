...
impl RpcStorage {
    pub fn new(db: RpcDb) -> Self {
        ...
        if state.get(INSTANCE_ID).unwrap().is_none() {
            state.insert(INSTANCE_ID, rand::random::<InstanceId>()).unwrap();
        }
        ...
        this.min_tx_lt.store(min_lt.unwrap_or(u64::MAX), Ordering::Release);
    ...
    pub fn get_brief_shards_descr(
        ...
        let shard_count = u32::from_le_bytes(
            value[tables::BlocksByMcSeqno::VALUE_LEN..tables::BlocksByMcSeqno::VALUE_LEN
                    + 4]
                .try_into()
                .unwrap(),
        ) as usize;
    ...
    pub fn get_block_transactions(
        ...
        let Some(ids) = self
            .get_block_transaction_ids(block_id, reverse, cursor, snapshot)? else {
            return Ok(None);
        };
        Ok(
            Some(BlockTransactionsIterBuilder {
                ids,
    ...
    pub fn get_block_transaction_ids(
        ...
        if cursor.is_some() && let Some(key) = iter.key() && key == range_from.as_slice()
        {
            if reverse {
                iter.prev();
            } else {
    ...
    pub fn get_transaction_ext<'db>(
        ...
        Ok(
            tx
                .map(move |data| TransactionDataExt {
                    info,
    ...
    pub async fn reset_accounts(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(reset_accounts)),
            file!(),
            603u32,
        );
        let shard_state = shard_state;
        let split_depth = split_depth;
        ...
        let started_at = Instant::now();
        {
            ...
            let __result = self.remove_code_hashes(&shard_ident).await;
        ...
        let span = tracing::Span::current();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    tracing::info!(
                        split_depth, "started building new code hash indices"
                    );
                    let started_at = Instant::now();
                    ...
                    let mut code_hashes_by_address_key = [0u8; {
                        tables::CodeHashesByAddress::KEY_LEN
                    }];
                    code_hashes_by_address_key[0] = workchain as u8;
                    for (virtual_shard, accounts) in virtual_shards {
                        tracing::info!(
                            % virtual_shard, "started collecting code hashes"
                        );
                        let started_at = Instant::now();
                        for entry in accounts.iter() {
                            ...
                            let code_hash = match extract_code_hash(&account)? {
                                ...
                                ExtractedCodeHash::Skip => {
                                    anyhow::bail!("code in account state is pruned")
                                }
                            };
                            ...
                            code_hashes_by_address_key[1..33]
                                .copy_from_slice(id.as_slice());
                            write_batch
                                .put_cf(code_hashes_cf, code_hashes_key.as_slice(), []);
                            write_batch
                                .put_cf(
    ...
    pub async fn remove_old_transactions(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(remove_old_transactions)),
            file!(),
            749u32,
        );
        let mc_seqno = mc_seqno;
        let min_lt = min_lt;
        let keep_tx_per_account = keep_tx_per_account;
        ...
        impl<'a> GcState<'a> {
            ...
            fn flush(&mut self) -> Result<()> {
                self.raw.write_opt(std::mem::take(&mut self.batch), self.writeopt)?;
                Ok(())
            }
        }
        if let Some(known_min_lt) = self.db.state.get(TX_MIN_LT)? {
            let known_min_lt = u64::from_le_bytes(
                known_min_lt.as_ref().try_into().unwrap(),
            );
            let was_running = matches!(
                self.db.state.get(TX_GC_RUNNING) ?, Some(status) if ! status.is_empty()
            );
            if !was_running && min_lt <= known_min_lt {
                tracing::info!(known_min_lt, "skipping removal of old transactions");
                {
                    __guard.end_section(861u32);
                    return Ok(());
                };
            }
        }
        ...
        let span = tracing::Span::current();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    'block: {
                        ...
                        key[4] = -1i8 as u8;
                        key[5..13]
                            .copy_from_slice(&ShardIdent::PREFIX_FULL.to_be_bytes());
                        key[13..17].copy_from_slice(&mc_seqno.to_be_bytes());
                        ...
                        let value = value.as_ref();
                        debug_assert!(
                            value.len() >= tables::BlocksByMcSeqno::VALUE_LEN + 4
                        );
                        let shard_count = u32::from_le_bytes(
                            value[tables::BlocksByMcSeqno::VALUE_LEN..tables::BlocksByMcSeqno::VALUE_LEN
                                    + 4]
                        ...
                        for i in 0..shard_count {
                            let offset = tables::BlocksByMcSeqno::DESCR_OFFSET
                                + i * tables::BlocksByMcSeqno::DESCR_LEN;
                            let descr = &value[offset..offset
                                + tables::BlocksByMcSeqno::DESCR_LEN];
                            top_block_ids
                                .push(BlockIdShort {
                        ...
                        for block_id in top_block_ids {
                            ...
                            range_from[1..9]
                                .copy_from_slice(&block_id.shard.prefix().to_be_bytes());
                            range_from[9..13]
                                .copy_from_slice(&block_id.seqno.to_be_bytes());
                            range_to[0..13].copy_from_slice(&range_from[0..13]);
                            batch
                                .delete_range_cf(
                                    &block_transactions.cf(),
                                    range_from,
                                    range_to,
                                );
                            ...
                            batch
                                .delete_range_cf(&known_blocks.cf(), range_from, range_to);
                    ...
                    readopts.set_snapshot(&snapshot);
                    let mut iter = raw
                        .raw_iterator_cf_opt(&db.transactions.cf(), readopts);
                    iter.seek_to_last();
                    ...
                    tracing::info!(
                        elapsed = % humantime::format_duration(started_at.elapsed()),
                        total_invalid, total_tx = gc.total_tx, total_tx_by_hash = gc
                        .total_tx_by_hash, total_tx_by_in_msg = gc.total_tx_by_in_msg,
    ...
    pub async fn update(
        ...
        let block = block;
        let rpc_blacklist = rpc_blacklist;
        ...
        let start_lt = {
            __guard.end_section(1379u32);
            let __result = tokio::task::spawn_blocking(move || {
                    let prepare_batch_histogram = HistogramGuard::begin(
                    ...
                    let accounts = if account_blocks.is_empty() {
                        ...
                        if old_accounts.repr_hash() == new_accounts.repr_hash() {
                            ...
                            let accounts = Lazy::<ShardAccounts>::from_raw(new_accounts)?
                                .load()?;
                    ...
                    let mut buffer = Vec::with_capacity(
                        64 + BriefBlockInfo::MIN_BYTE_LEN,
                    );
                    {
                        let mut key = [0u8; tables::BlocksByMcSeqno::KEY_LEN];
                        key[0..4].copy_from_slice(&mc_seqno.to_be_bytes());
                        ...
                        buffer.clear();
                        buffer.extend_from_slice(block_id.root_hash.as_slice());
                        buffer.extend_from_slice(block_id.file_hash.as_slice());
                        buffer.extend_from_slice(&info.start_lt.to_le_bytes());
                        buffer.extend_from_slice(&info.end_lt.to_le_bytes());
                        if let Some(shard_hashes) = shard_hashes {
                            ...
                            buffer
                                .reserve(
                                    4 + tables::BlocksByMcSeqno::DESCR_LEN * shards.len(),
                                );
                            buffer
                            ...
                            for shard in shards {
                                ...
                                buffer
                                    .extend_from_slice(
                                        &shard.shard_ident.prefix().to_le_bytes(),
                                    );
                        ...
                        write_batch
                            .put_cf(&db.blocks_by_mc_seqno.cf(), key, buffer.as_slice());
                    }
                    ...
                    for item in account_blocks.iter() {
                        ...
                        for item in account_block.transactions.values() {
                            ...
                            tycho_types::boc::ser::BocHeader::<
                                ahash::RandomState,
                            >::with_root(tx_cell.inner().as_ref())
                                .encode(&mut buffer);
                            write_batch
                                .put_cf(
                                    tx_by_hash_cf,
                    ...
                    let brief_block_info = BriefBlockInfo::new(
                        block.as_ref(),
                        info,
                        extra,
                        block_tx_count,
                    ...
                    buffer.clear();
                    buffer.extend_from_slice(&tx_info[46..114]);
                    brief_block_info.write_to_bytes(&mut buffer);
                    write_batch
                        .put_cf(
                            &db.known_blocks.cf(),
                    ...
                    let _execute_batch_histogram = HistogramGuard::begin(
                        "tycho_storage_rpc_execute_batch_time",
                    );
                    db.rocksdb().write_opt(write_batch, db.transactions.write_config())?;
                    Ok::<_, anyhow::Error>(info.start_lt)
                })
                .await;
            __guard.start_section(1379u32);
            __result
        }??;
        'min_lt: {
            if start_lt < self.min_tx_lt.fetch_min(start_lt, Ordering::Release) {
                let _guard = {
                    __guard.end_section(1387u32);
                    let __result = self.min_tx_lt_guard.lock().await;
                    __guard.start_section(1387u32);
                    __result
                };
                if start_lt > self.min_tx_lt.load(Ordering::Acquire) {
                    {
                        __guard.end_section(1392u32);
                        __guard.start_section(1392u32);
                        break 'min_lt;
                    };
                }
    ...
    fn update_code_hash(
        ...
        if remove && old_code_hash.is_none()
            || matches!(
                (& old_code_hash, & new_code_hash), (Some(old), Some(new)) if old
                .as_ref() == new.as_slice()
            )
        {
            return Ok(());
    ...
    async fn remove_code_hashes(&self, shard: &ShardIdent) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(remove_code_hashes)),
            file!(),
            1485u32,
        );
        let shard = shard;
...
impl BriefBlockInfo {
    ...
    fn new(
        ...
        let prev_blocks = match info.load_prev_ref()? {
            ...
            PrevBlockRef::AfterMerge { left, right } => {
                vec![left.as_block_id(shard_ident), right.as_block_id(shard_ident),]
            }
        };
        Ok(Self {
    ...
    fn write_to_bytes(&self, target: &mut Vec<u8>) {
        let packed_flags = ((self.master_ref.is_some() as u8) << 7)
            | ((self.after_merge as u8) << 6) | ((self.before_split as u8) << 5)
            | ((self.after_split as u8) << 4) | ((self.want_split as u8) << 3)
            | ((self.want_merge as u8) << 2) | ((self.is_key_block as u8) << 1);
    ...
    for<'a> F: FnMut(&'a StdAddr, u64, &'a [u8]) -> Option<R>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            break (self
                .map)(&id.account, id.lt, TransactionData::read_transaction(&value));
        }
    }
}
#[derive(Debug, Clone)]
...
impl<'a> TransactionData<'a> {
    ...
    pub fn in_msg_hash(&self) -> Option<HashBytes> {
        ...
        assert!(! value.is_empty());
        let mask = TransactionMask::from_bits_retain(value[0]);
        mask.has_msg_hash().then(|| HashBytes::from_slice(&value[33..65]))
    }
...
fn split_shard(
    ...
    fn split_shard_impl(
        ...
        let (left_shard_ident, right_shard_ident) = 'split: {
            if depth > 0 && let Some((left, right)) = shard.split() {
        ...
        split_shard_impl(&left_shard_ident, &left_accounts, depth - 1, shards, builder)?;
        split_shard_impl(&right_shard_ident, &right_accounts, depth - 1, shards, builder)
    }
...
bitflags::bitflags! {
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)] pub struct TransactionMask : u8 {
...
mod tests {
    ...
    fn shard_prefix() {
        ...
        let tail_mask = 1u64 << (63 - prefix_len);
        let shard = ShardIdent::new(0, (account_prefix | tail_mask) & !(tail_mask - 1))
            .unwrap();
        assert_eq!(shard, unsafe { ShardIdent::new_unchecked(0, 0xabe0000000000000) });
    }
}
