...
pub(super) struct GcStateMachine {
    ...
    startup: Mutex<Option<StartupGcPass>>,
...
impl GcStateMachine {
    ...
    pub(super) fn configure_startup(
        ...
        *self.startup.lock() = Some(StartupGcPass {
            storage,
            config,
            frontier,
            pending_sealing,
    ...
    async fn run_once(&self) -> Result<TailSweepNext> {
        ...
        let startup_drive = self.drive_startup_gc_once().await?;
        if matches!(startup_drive, StartupGcDrive::Worked) {
            return Ok(TailSweepNext::Ready);
        }
        if matches!(startup_drive, StartupGcDrive::AwaitSealing) {
            return Ok(TailSweepNext::Empty);
        }
        let next = self.sweep_tail_once(published_visible_generation).await?;
        if matches!(startup_drive, StartupGcDrive::CandidateExhausted) {
            match next {
                TailSweepNext::Ready => {}
                TailSweepNext::Empty | TailSweepNext::AwaitRequestGeneration { .. } => {
                    self.complete_startup_gc_pass();
                }
                TailSweepNext::AwaitPublishedGeneration { .. } => {
                    return Err(conflicting_authoritative_error(
                        "RPC tail retirement queue is ahead of published control without a startup GC intent",
                    ));
                }
            }
        }
    ...
    async fn drive_startup_gc_once(&self) -> Result<StartupGcDrive> {
        let Some(startup) = self.startup.lock().clone() else {
            return Ok(StartupGcDrive::Inactive);
        };
        let Some(permit) = acquire_background_permit_after_transient_check(
            || startup_sealing_pending(&startup),
            self.acquire_background_permit(),
        ).await? else {
            return Ok(StartupGcDrive::AwaitSealing);
        };
        let storage = startup
            .storage
            .upgrade()
            .context("RPC transaction GC startup storage was dropped")?;
        let config = startup.config.clone();
        let frontier = startup.frontier;
        let observed_phase = self.observed_phase.clone();
        let cancelled = self.cancelled.clone();
        let step = tokio::task::spawn_blocking(move || -> Result<StartupGcStep> {
            let _permit = permit;
            let Some(intent) = storage.begin_or_resume_gc_at_frontier(
                config.as_ref(),
                &frontier,
            )? else {
                return Ok(StartupGcStep::CandidateExhausted);
            };
            observe_gc_intent_state(&observed_phase, &cancelled, intent);
            match intent.phase {
                GcIntentPhase::Evacuating => Ok(startup_step_after_evacuation(
                    intent,
                    storage.evacuate_gc_chunk(intent)?,
                )),
                GcIntentPhase::Prepared => Ok(StartupGcStep::Prepared(intent)),
                GcIntentPhase::CutoverCommitted | GcIntentPhase::Deleting => {
                    Ok(StartupGcStep::Delete(intent))
                }
            }
        })
        .await
        .context("RPC transaction GC startup step failed")??;
        match step {
            StartupGcStep::Worked => Ok(StartupGcDrive::Worked),
            StartupGcStep::CandidateExhausted => Ok(StartupGcDrive::CandidateExhausted),
            StartupGcStep::Prepared(intent) => {
                ...
                drop(storage);
                drop(permit);
                Ok(StartupGcDrive::Worked)
            }
            StartupGcStep::Delete(intent) => {
                self.observe_gc_intent(intent);
                self.delete_source(intent).await?;
                Ok(StartupGcDrive::Worked)
            }
    ...
    fn complete_startup_gc_pass(&self) {
        let Some(startup) = self.startup.lock().take() else {
            return;
        };
        record_gc_startup_pass_completed(startup.started_at.elapsed());
        #[cfg(test)]
        {
...
fn startup_sealing_pending(startup: &StartupGcPass) -> Result<bool> {
    let storage = startup
        .storage
        .upgrade()
        .context("RPC transaction GC startup storage was dropped")?;
    let pending = storage.startup_sealing_pending(startup.pending_sealing);
    drop(storage);
    Ok(pending)
}
...
mod tests {
    ...
    fn evacuation_observation_precedes_work_and_preserves_prepared_result() {
        ...
        metrics::with_local_recorder(&recorder, || {
            ...
            match startup_step_after_evacuation(
                intent,
                GcEvacuationChunkResult::Prepared(prepared),
            ) {
                StartupGcStep::Prepared(actual) => {
