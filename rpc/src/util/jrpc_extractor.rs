...
macro_rules! declare_jrpc_method {
    (
        $(#[$($meta:tt)*])* $vis:vis enum $method_enum:ident : $method_name_enum:ident {
        $($(#[$($method_meta:tt)*])* $method_name:ident ($method_params:ty)),*$(,) ? }
    ) => {
        #[allow(clippy::enum_variant_names)] #[derive(Debug, Clone, Copy, PartialEq, Eq,
        $crate::__internal::serde::Deserialize)] #[serde(rename_all = "camelCase")] $vis
        enum $method_name_enum { $($(#[$($method_meta)*])* $method_name,)* } impl
        $crate::util::jrpc_extractor::ParseParams for $method_name_enum { type Params =
        $method_enum; fn parse_params(self, params :
        &$crate::__internal::serde_json::value::RawValue) -> Result < Self::Params,
        serde_json::Error > { let params = params.get(); match self {
        $(Self::$method_name => $crate::__internal::serde_json::from_str(params)
        .map($method_enum ::$method_name),)* } } fn method_name(& self) -> &'static str {
        match self { $(Self::$method_name => stringify!($method_name),)* } } }
        #[derive(Debug)] $vis enum $method_enum { $($(#[$($method_meta)*])* $method_name
        ($method_params),)* }
    };
}
    ...
    S: Send + Sync,
{
    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let bytes = match {
            __guard.end_section(119u32);
            let __result = Bytes::from_request(req, state).await;
            __guard.start_section(119u32);
            __result
        } {
            Ok(bytes) => bytes,
            Err(e) => {
        ...
        let (id, code, message) = match serde_json::from_slice::<
            Request<'_, T, B::Id>,
        >(&bytes) {
            Ok(req) if req.jsonrpc == JSONRPC_VERSION => {
                match req.method {
                    ParsedMethod::Known(known) => {
                        ...
                        match known.parse_params(req.params) {
                            ...
                            Err(e) => {
                                (Some(req.id), INVALID_PARAMS_CODE, e.to_string().into())
                            }
                        }
                    }
            ...
            Ok(req) => {
                (Some(req.id), INVALID_REQUEST_CODE, "invalid jronrpc version".into())
            }
            Err(e) => {
                let code = match e.classify() {
                    serde_json::error::Category::Data => INVALID_REQUEST_CODE,
                    serde_json::error::Category::Syntax
                    | serde_json::error::Category::Eof => PARSE_ERROR_CODE,
