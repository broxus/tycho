...
mod cache;
declare_jrpc_method! {
    pub enum MethodParams : Method { GetCapabilities(EmptyParams),
    GetLatestKeyBlock(EmptyParams), GetBlockchainConfig(EmptyParams),
    GetStatus(EmptyParams), GetTimings(EmptyParams), SendMessage(SendMessageRequest),
    GetContractState(GetContractStateRequest), GetLibraryCell(GetLibraryCellRequest),
    GetAccountsByCodeHash(GetAccountsByCodeHashRequest),
    GetTransactionsList(GetTransactionsListRequest),
    GetTransaction(GetTransactionRequest), GetDstTransaction(GetDstTransactionRequest),
    GetTransactionBlockId(GetTransactionRequest),
    GetKeyBlockProof(GetKeyBlockProofRequest), GetBlockProof(GetBlockRequest),
    GetBlockData(GetBlockRequest), }
}
pub async fn route(
    State(state): State<RpcState>,
    req: Jrpc<RfcBehaviour, Method>,
) -> Response {
    ...
    match req.params {
        ...
        MethodParams::GetLatestKeyBlock(_) => {
            match &*state.jrpc_cache().load_latest_key_block() {
        ...
        MethodParams::GetBlockchainConfig(_) => {
            match &*state.jrpc_cache().load_blockchain_config() {
                Some(config) => ok_to_response(req.id, config.as_ref()),
                None => error_to_response(req.id, RpcStateError::NotReady),
            }
        }
        ...
        MethodParams::GetLibraryCell(p) => {
            let res = match state.jrpc_cache().get_library_cell_response(&p.hash) {
                Some(value) => value,
                None => {
                    match state.get_raw_library(&p.hash) {
                        Ok(res) => {
                            state.jrpc_cache().insert_library_cell_response(p.hash, res)
                        }
                        Err(e) => {
                            __guard.end_section(94u32);
                            return error_to_response(req.id, RpcStateError::Internal(e));
                        }
                    }
                }
            };
        ...
        MethodParams::GetContractState(p) => {
            ...
            ok_to_response(
                ...
                match item {
                    ...
                    LoadedAccountState::Found { state, timings, mc_ref_handle, .. } => {
        ...
        MethodParams::GetAccountsByCodeHash(p) => {
            if p.limit == 0 {
                {
                    ...
                    return JrpcOkResponse::<_, RfcBehaviour>::new(req.id, [(); 0])
                        .into_response();
                };
            ...
            match state
                .get_accounts_by_code_hash(&p.code_hash, p.continuation.as_ref(), None)
            {
                Ok(list) => {
                    ok_to_response(
                        req.id,
                        GetAccountsByCodeHashResponse {
        ...
        MethodParams::GetTransactionsList(p) => {
            ...
            match state
                .get_transactions(&p.account, None, p.last_transaction_lt, true, None)
            {
                Ok(list) => {
                    ok_to_response(
                        req.id,
        ...
        MethodParams::GetTransaction(p) => {
            match state.get_transaction(&p.id, None) {
        ...
        MethodParams::GetTransactionBlockId(p) => {
            match state.get_transaction_info(&p.id, None) {
                Ok(value) => {
                    ok_to_response(
                        req.id,
                        value
                            .map(|info| BlockIdResponse {
        ...
        MethodParams::GetKeyBlockProof(p) => {
            let res = match state.jrpc_cache().get_key_block_proof_response(p.seqno) {
                ...
                None => {
                    ...
                    state.jrpc_cache().insert_key_block_proof_response(p.seqno, res)
                }
            };
            ok_to_response(req.id, res.as_ref())
        }
        MethodParams::GetBlockProof(p) => {
            ...
            let proof = {
                ...
                let __result = state.get_block_proof(&p.block_id).await;
        ...
        MethodParams::GetBlockData(p) => {
            ...
            let Some(data) = ({
                __guard.end_section(219u32);
                let __result = state.get_block_data(&p.block_id).await;
                __guard.start_section(219u32);
                __result
            }) else {
                {
