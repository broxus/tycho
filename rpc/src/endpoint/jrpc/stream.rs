...
use crate::state::{AccountUpdate, RegisterError, RpcSubscriptions};
...
pub async fn stream_route<S>(
    State(subs_state): State<SubscriptionsState>,
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    let subs = subs_state.subs.clone();
    ...
    let main_stream = stream::unfold(
        StreamState::new(uuid, rx, subs, subs_state.keep_alive),
        |mut st| async move {
            tokio::select! {
                ...
                _ = st.ticker.tick() => {
                    let payload = KeepAlivePayload { dropped: st.refresh_dropped() };
    ...
    Sse::new(uuid_stream.chain(main_stream)).into_response()
}
...
mod tests {
    ...
    async fn stream_lifecycle_with_drops_and_unsubscribe() {
        ...
        let response = stream_route::<SubscriptionsState>(
            State(Arc::new(ctx)),
            Query(StreamParams { binary: false }),
        )
        .await;
