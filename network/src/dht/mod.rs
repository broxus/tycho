...
use crate::proto::dht::{
    NodeInfoResponse, NodeResponse, PeerValue, PeerValueKey, PeerValueKeyName,
    PeerValueKeyRef, PeerValueRef, Value, ValueRef, ValueResponseRaw, rpc,
};
...
impl DhtClient {
    ...
    pub async fn get_node_info(&self, peer_id: &PeerId) -> Result<PeerInfo> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_node_info)),
            file!(),
            68u32,
        );
        let peer_id = peer_id;
    ...
    pub async fn find_value(
        ...
        let mode = mode;
        {
            ...
            let __result = self.inner.find_value(&self.network, key_hash, mode).await;
...
impl<'a> DhtQueryBuilder<'a> {
        ...
        for<'tl> T: tl_proto::TlRead<'tl>,
    {
        ...
        match {
            ...
            let __result = self
                ...
                .await;
            __guard.start_section(121u32);
            __result
        } {
            Some(value) => {
                match value.as_ref() {
                    Value::Peer(value) => {
                        tl_proto::deserialize(&value.data)
                            .map_err(FindValueError::InvalidData)
                    }
                    Value::Merged(_) => {
                        Err(
                            FindValueError::InvalidData(
    ...
    pub async fn find_peer_value_raw(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(find_peer_value_raw)),
            file!(),
            138u32,
        );
        let peer_id = peer_id;
        ...
        match {
            ...
            Some(value) => {
                realloc_box_enum!(
                    value, { Value::Peer(value) => Box::new(value) => Ok(value),
                    Value::Merged(_) =>
                    Err(FindValueError::InvalidData(tl_proto::TlError::UnknownConstructor,)),
                    }
                )
            }
            None => Err(FindValueError::NotFound),
...
impl DhtQueryWithDataBuilder<'_> {
    ...
    pub async fn store(&self) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(store)),
            file!(),
            199u32,
        );
        ...
        value.signature = &signature;
        {
            ...
            let __result = dht
                .store_value(network, &ValueRef::Peer(value), self.with_peer_info)
                .await;
...
impl DhtServiceBackgroundTasks {
    pub fn spawn(self, network: &Network) {
        self.inner.start_background_tasks(Network::downgrade(network));
    }
}
...
impl DhtServiceBuilder {
    ...
    pub fn build(self) -> (DhtServiceBackgroundTasks, DhtService) {
        ...
        let (announced_peers, _) = broadcast::channel(
            config.announced_peers_channel_capacity,
        );
...
impl Service<ServiceRequest> for DhtService {
    ...
    fn on_query(&self, req: ServiceRequest) -> Self::OnQueryFuture {
        ...
        let response = crate::match_tl_request!(
            body, tag = constructor, { rpc::FindNode as ref r => { tracing::debug!(key =
            % PeerId::wrap(& r.key), k = r.k, "find_node");
            metrics::counter!(METRIC_IN_REQ_FIND_NODE_TOTAL) .increment(1); let res =
            self.0.handle_find_node(r); Some(tl_proto::serialize(res)) }, rpc::FindValue
            as ref r => { tracing::debug!(key = % PeerId::wrap(& r.key), k = r.k,
            "find_value"); metrics::counter!(METRIC_IN_REQ_FIND_VALUE_TOTAL)
            .increment(1); let res = self.0.handle_find_value(r);
            Some(tl_proto::serialize(res)) }, rpc::GetNodeInfo as _ => {
            tracing::debug!("get_node_info");
            metrics::counter!(METRIC_IN_REQ_GET_NODE_INFO_TOTAL) .increment(1); self.0
            .handle_get_node_info().map(tl_proto::serialize) }, }, e => {
            tracing::debug!("failed to deserialize query: {e}"); None }
        );
        if response.is_none() {
            metrics::counter!(METRIC_IN_REQ_FAIL_TOTAL).increment(1);
        }
    ...
    fn on_message(&self, req: ServiceRequest) -> Self::OnMessageFuture {
        ...
        crate::match_tl_request!(
            body, tag = constructor, { rpc::StoreRef <'_ > as ref r => {
            tracing::debug!("store"); metrics::counter!(METRIC_IN_REQ_STORE_TOTAL)
            .increment(1); if let Err(e) = self.0.handle_store(r) {
            tracing::debug!("failed to store value: {e}"); has_error = true; } } }, e =>
            { tracing::debug!("failed to deserialize message: {e}"); has_error = true; }
        );
        if has_error {
            metrics::counter!(METRIC_IN_REQ_FAIL_TOTAL).increment(1);
        }
...
impl DhtInner {
    ...
    async fn store_value(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(store_value)),
            file!(),
            543u32,
        );
        let network = network;
        let value = value;
        let with_peer_info = with_peer_info;
...
pub fn xor_distance(left: &PeerId, right: &PeerId) -> usize {
    for (i, (left, right)) in std::iter::zip(left.0.chunks(8), right.0.chunks(8))
        .enumerate()
    {
