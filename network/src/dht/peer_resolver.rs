...
use crate::network::{
    KnownPeerHandle, KnownPeersError, Network, PeerBannedError, WeakNetwork,
};
use crate::proto::dht;
...
impl PeerResolverInner {
    ...
    async fn run_task(
        ...
        loop {
            __guard.checkpoint(185u32);
            if let Some(t) = timings {
                let update_at = std::time::UNIX_EPOCH
                    + Duration::from_secs(t.update_at as u64);
                ...
                );
                {
                    ...
                    let __result = tokio::time::sleep(remaining).await;
            ...
            match {
                ...
                let __result = self.resolve_peer(&data, &timings).await;
                    ...
                    let peer_info = match &*handle {
                        Some(handle) => {
                            match handle.update_peer_info(&peer_info) {
                                ...
                                Err(KnownPeersError::PeerBanned(PeerBannedError)) => {
    ...
    async fn resolve_peer(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(resolve_peer)),
            file!(),
            244u32,
        );
        let data = data;
        let prev_timings = prev_timings;
        ...
        loop {
            ...
            let is_stale = attempts > self.config.fast_retry_count as usize;
            {
                ...
                {
                    tracing::trace!(
                        peer_id = % data.peer_id, attempts, is_stale, "peer info exists",
                    );
                    {
                        __guard.end_section(274u32);
                ...
                let res = {
                    let _permit = {
                        ...
                        let __result = self.semaphore.acquire().await;
                ...
                match res {
                    Ok(
                        peer_info,
                    ) if peer_info.id == data.peer_id && peer_info.verify(now) => {
                    ...
                    Ok(_) => {
                        tracing::trace!(
                            peer_id = % data.peer_id, attempts, is_stale,
...
impl PeerResolverTimings {
    fn is_new_info(this: &Option<Self>, peer_info: &PeerInfo) -> bool {
        match this {
            Some(this) => {
                peer_info.created_at > this.created_at
                    && peer_info.expires_at > this.expires_at
            }
            None => true,
        }
    }
    ...
    fn new(
        ...
        Self {
            inner: ManuallyDrop::new(
                Arc::new(PeerResolverHandleInner {
                    _task: Some(task),
                    data,
