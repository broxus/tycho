...
pub use self::config::{
    CongestionAlgorithm, ConnectionMetricsLevel, NetworkConfig, QuicConfig,
};
...
use crate::types::{
    Address, DisconnectReason, PeerEvent, PeerId, PeerInfo, Response, Service,
    ServiceExt, ServiceRequest,
};
mod config;
mod connection;
...
impl NetworkBuilder {
        ...
        S: Service<ServiceRequest, QueryResponse = Response>,
    {
        ...
        let keypair = ed25519::KeyPair::from(
            &ed25519::SecretKey::from_bytes(private_key),
        );
...
fn set_socket_buffer(
    ...
    if let Some(size) = config_size {
        if let Err(e) = set_buffer_fn(socket, size) {
            tracing::error!(
                % size, "failed to set socket {} buffer size: {e:?}", buffer_type
            );
        }
    } else if let Some(max) = max_size {
        if let Err(e) = set_buffer_fn(socket, max) {
            tracing::error!(
                % max, "failed to set socket {} buffer size to max value: {e:?}",
                buffer_type
            );
        }
        tracing::info!("set socket {} buffer size to max value: {}", buffer_type, max);
    }
}
#[derive(Clone)]
#[repr(transparent)]
...
impl Network {
        ...
        T: Into<Address>,
    {
        ...
        let peer_id = peer_id;
        {
            ...
            let __result = self.0.connect(addr.into(), peer_id).await;
    ...
    pub async fn shutdown(&self) {
        ...
        );
        {
            ...
            let __result = self.0.shutdown().await;
...
impl NetworkInner {
    ...
    async fn connect(
        ...
        let Ok(res) = ({
            __guard.end_section(309u32);
            let __result = rx.await;
            __guard.start_section(309u32);
            __result
        }) else {
            {
    ...
    fn disconnect(&self, peer_id: &PeerId) {
        self.active_peers.remove(peer_id, DisconnectReason::Requested);
    }
...
macro_rules! impl_to_socket_for_addr {
    ($($ty:ty),*$(,) ?) => {
        $(impl ToSocket for $ty { fn to_socket(self) -> Result < std::net::UdpSocket,
        BindError > { bind_socket_to_addr(self) } })*
    };
}
impl_to_socket_for_addr! {
    SocketAddr, std::net::SocketAddrV4, std::net::SocketAddrV6, (std::net::IpAddr, u16),
    (std::net::Ipv4Addr, u16), (std::net::Ipv6Addr, u16), (& str, u16), (String, u16), &
    str, String, & [SocketAddr], Address,
}
    ...
    for addr in socket_addrs {
        let socket = match Socket::new(
            ...
            Err(e) => {
                if last_bind_error.is_none() {
                    last_bind_error = Some(BindError::SocketCreation {
                        addr,
                        source: e,
                    });
                }
        ...
        match socket.bind(&socket2::SockAddr::from(addr)) {
            ...
            Err(e) => {
                ...
                last_bind_error = Some(BindError::SocketBind {
                    addr,
                    source: e,
                });
            }
...
pub enum BindError {
    ...
    #[error(
        "no suitable addresses found to bind to after attempting all resolved addresses"
    )]
    NoAddressesToBind,
    #[error("failed to create new socket for address {addr:?}: {source}")]
    SocketCreation { addr: SocketAddr, #[source] source: std::io::Error },
    #[error("failed to bind socket to address {addr}: {source}")]
    SocketBind { addr: SocketAddr, #[source] source: std::io::Error },
}
...
impl MaxBufferSize {
    ...
    pub fn read() -> Result<Option<Self>> {
        ...
        let proc_path = std::env::var("MOCK_PROC_PATH")
...
mod tests {
    ...
    use super::*;
    use crate::types::{
        BoxCloneService, PeerInfo, Request, service_message_fn, service_query_fn,
    };
    ...
    async fn connection_manager_works() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(connection_manager_works)),
            file!(),
            549u32,
        );
        ...
        let peer2 = make_network()?;
        {
            ...
            let __result = peer1.connect(peer2.local_addr(), peer2.peer_id()).await;
            __guard.start_section(557u32);
            __result
        }
            .unwrap();
        {
            __guard.end_section(561u32);
            let __result = peer2.connect(peer1.local_addr(), peer1.peer_id()).await;
            __guard.start_section(561u32);
            __result
        }
            .unwrap();
        Ok(())
    ...
    async fn invalid_peer_id_detectable() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(invalid_peer_id_detectable)),
            file!(),
            568u32,
        );
        ...
        let _handle = peer1.known_peers().insert(make_invalid_peer_info(&peer2), false)?;
        let _handle = peer2.known_peers().insert(make_peer_info(&peer1), false)?;
        let _handle = peer2.known_peers().insert(make_invalid_peer_info(&peer1), false)?;
        let req = Request {
            version: Default::default(),
            body: "hello".into(),
        };
        {
            ...
            let __result = peer1.query(peer2.peer_id(), req.clone()).await;
            __guard.start_section(598u32);
            __result
        }?;
        {
            __guard.end_section(599u32);
            let __result = peer2.query(peer1.peer_id(), req.clone()).await;
            __guard.start_section(599u32);
            __result
        }?;
        ...
        let err = {
            ...
            let __result = peer1.query(&PeerId([0; 32]), req.clone()).await;
        ...
        let err = {
            ...
            let __result = peer2.query(&PeerId([0; 32]), req.clone()).await;
        ...
        let err = {
            ...
            let __result = peer1.query(&invalid_peer_id, req.clone()).await;
    ...
    async fn simultaneous_queries() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(simultaneous_queries)),
            file!(),
            639u32,
        );
        tycho_util::test::init_logger("simultaneous_queries", "debug");
        for _ in 0..10 {
            __guard.checkpoint(642u32);
            let peer1 = make_network()?;
            let peer2 = make_network()?;
            let _peer1_peer2_handle = peer1
                .known_peers()
                .insert(make_peer_info(&peer2), false)?;
            let _peer2_peer1_handle = peer2
                .known_peers()
                .insert(make_peer_info(&peer1), false)?;
            let req = Request {
                version: Default::default(),
                body: "hello".into(),
            };
            ...
            let (res1, res2) = {
                __guard.end_section(656u32);
                let __result = futures_util::future::join(peer1_fut, peer2_fut).await;
                __guard.start_section(656u32);
                __result
            };
    ...
    async fn uni_message_handler() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(uni_message_handler)),
            file!(),
            665u32,
        );
