...
impl Endpoint {
    ...
    pub async fn wait_idle(&self, timeout: Duration) {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(wait_idle)),
            file!(),
            63u32,
        );
        let timeout = timeout;
        if {
            ...
            let __result = tokio::time::timeout(timeout, self.inner.wait_idle()).await;
...
pin_project_lite::pin_project! {
    #[must_use = "futures do nothing unless you `.await` or poll them"] pub (crate)
    struct Accept <'a > { #[pin] inner : quinn::Accept <'a >, metrics : Option <
    ConnectionMetricsLevel >, }
}
                ...
                c.and_then(|c| {
                    ...
                    match c.accept() {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                % remote_addr,
                                "failed to accept an incoming connection: {e:?}"
                            );
                            None
                        }
                    }
...
impl Connecting {
    ...
    pub fn into_0rtt(self) -> Into0RttResult {
        match self.inner.into_0rtt() {
            Ok((c, accepted)) => {
                match c.peer_identity() {
                    Some(identity) => {
                        match parse_peer_identity(identity) {
                            Some(peer_id) => {
                                Into0RttResult::Established(
                                    Connection::with_peer_id(
                                        c,
                                        self.origin,
                                        peer_id,
                                        self.metrics,
                    ...
                    None => {
                        Into0RttResult::WithoutIdentity(ConnectingFallback {
                            inner: Some(c),
                            accepted,
                            origin: self.origin,
            ...
            Err(inner) => {
                Into0RttResult::Unavailable(Self {
                    inner,
                    origin: self.origin,
                    metrics: self.metrics,
                })
