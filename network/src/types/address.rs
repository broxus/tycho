        ...
        match self {
            ...
            Self::Dns { hostname, port } => {
                let mut iter = {
                    ...
                    let __result = tokio::net::lookup_host((hostname.as_ref(), *port))
                        .await;
...
impl std::fmt::Display for Address {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Ip { ip, port } => {
                std::fmt::Display::fmt(&SocketAddr::new(*ip, *port), f)
            }
...
impl TlWrite for Address {
    ...
    fn max_size_hint(&self) -> usize {
        let len = match self {
            Self::Ip { ip: IpAddr::V4(_), .. } => 4,
            Self::Ip { ip: IpAddr::V6(_), .. } => 16,
...
impl<'a> TlRead<'a> for Address {
    ...
    fn read_from(packet: &mut &'a [u8]) -> tl_proto::TlResult<Self> {
        ...
        fn read_port(packet: &mut &[u8]) -> tl_proto::TlResult<u16> {
            u32::read_from(packet)?.try_into().map_err(|_e| TlError::InvalidData)
        }
        Ok(
...
const ADDRESS_V4_TL_ID: u32 = tl_proto::id!(
    "transport.address.ipv4", scheme = "proto.tl"
);
const ADDRESS_V6_TL_ID: u32 = tl_proto::id!(
    "transport.address.ipv6", scheme = "proto.tl"
);
const ADDRESS_DNS_TL_ID: u32 = tl_proto::id!(
    "transport.address.dns", scheme = "proto.tl"
);
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn serde() {
        for addr in [SOME_ADDR_V4, SOME_ADDR_V6, SOME_ADDR_DNS] {
            let from_json: Address = serde_json::from_str(&format!("\"{addr}\""))
                .unwrap();
    ...
    fn tl() {
        ...
        for addr in addrs {
            assert!(
                matches!(tl_proto::deserialize::< Address > (&
                tl_proto::serialize(addr)), Err(tl_proto::TlError::InvalidData))
            );
        }
    }
    #[test]
