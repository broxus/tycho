...
impl DbCleaner {
    ...
    fn least_to_keep(
        ...
        let least_to_keep = (committed - conf.consensus.reset_rounds())
            .min(consensus.max(top_known_anchor) - conf.consensus.max_total_rounds());
        let remainder = least_to_keep.0
            % NodeConfig::get().clean_db_period_rounds.get() as u32;
    ...
    pub fn new_task(
        ...
        task_ctx
            .spawn(async move {
                ...
                let mut prev_least_to_keep = Self::least_to_keep(
                    consensus,
                    committed,
                    top_known,
                    round_ctx.conf(),
                ...
                loop {
                    __guard.checkpoint(58u32);
                    {
                        __guard.end_section(59u32);
                        let __result = tokio::select! {
                            biased; new_consensus = consensus_round.next() => consensus =
                            new_consensus ?, new_committed = committed_round.next() =>
                            committed = new_committed ?, new_top_known = top_known_anchor
                            .next() => top_known = new_top_known ?,
                        };
                    ...
                    let new_least_to_keep = Self::least_to_keep(
                        consensus,
                        committed,
                        top_known,
                        round_ctx.conf(),
                        ...
                        let task = round_ctx
                            ...
                            .spawn_blocking(move || {
                                ...
                                match db.clean_points(&up_to_exclusive) {
                                    ...
                                    Ok(None) => {
                                        tracing::info!(
                                            "mempool DB is already clean before {}", new_least_to_keep.0
                                        );
                                    }
                                    Err(e) => {
                                        metrics::gauge!(DB_CLEAN_ERRORS, "kind" => "points")
                                            .increment(1);
                                        tracing::error!(
                                ...
                                match db.wait_for_compact() {
                                    Ok(()) => {}
                                    Err(e) => {
                                        metrics::gauge!(DB_CLEAN_ERRORS, "kind" => "compact")
                                            .increment(1);
                                        tracing::error!(
                                            "wait compaction of mempool DB failed: {e}"
                                        );
                                    }
                                };
                            });
