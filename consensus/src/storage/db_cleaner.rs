...
impl DbCleaner {
    ...
    fn least_to_keep(consensus: Round, top_known_anchor: Round, conf: &MempoolConfig) -> Round {
        ...
        let least_to_keep = consensus.max(top_known_anchor) - conf.consensus.max_total_rounds();
    ...
    pub fn run(mut self, round_ctx: &RoundCtx) -> Task<Never> {
        ...
        task_ctx.spawn(async move {
            ...
            let mut prev_least_to_keep =
                Self::least_to_keep(consensus, top_known, round_ctx.conf());
            loop {
                tokio::select! {
                    biased;
                ...
                let new_least_to_keep = Self::least_to_keep(consensus, top_known, round_ctx.conf());
