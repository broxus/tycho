...
pub struct MempoolDb {
    ...
    pub(super) wee: WeeDb<MempoolTables>,
...
impl MempoolDb {
    ...
    pub(super) fn clean(
        ...
        let status_cf = self.wee.tables().points_status.cf();
        let info_cf = self.wee.tables().points_info.cf();
        let points_cf = self.wee.tables().points.cf();
        let rocksdb = self.wee.rocksdb();
    ...
    pub(super) fn wait_for_compact(&self) -> anyhow::Result<()> {
        ...
        opt.set_flush(true);
        self.wee.rocksdb().wait_for_compact(&opt)?;
        Ok(())
    }
    ...
    pub(super) fn has_compatible_data(&self, data_version: &[u8]) -> anyhow::Result<bool> {
        let default_cf = self.wee.rocksdb();
