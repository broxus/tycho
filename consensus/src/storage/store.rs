...
impl MempoolStoreImpl for MempoolDb {
    fn insert_point(&self, point: &Point, status: PointStatusStoredRef<'_>) -> Result<()> {
        ...
        let db = self.wee.rocksdb();
        let points_cf = self.wee.points.cf();
        let info_cf = self.wee.points_info.cf();
        let status_cf = self.wee.points_status.cf();
    ...
    fn multi_get_info(&self, keys: &[(Round, Digest)]) -> Result<Vec<PointInfo>> {
        ...
        for (result_option_bytes, (round, digest)) in (self.wee.points_info)
    ...
    fn get_info(&self, round: Round, digest: &Digest) -> Result<Option<PointInfo>> {
        ...
        let table = &self.wee.points_info;
        table
            .get(key.as_slice())
            .context("db get")?
    ...
    fn get_status(&self, round: Round, digest: &Digest) -> Result<Option<PointStatusStored>> {
        ...
        let table = &self.wee.points_status;
        table
            .get(key.as_slice())
    ...
    fn reset_statuses(&self, range: &RangeInclusive<Round>) -> Result<()> {
        let status_t = &self.wee.tables().points_status;
        let db = self.wee.rocksdb();
    ...
    fn load_restore(&self, range: &RangeInclusive<Round>) -> Result<Vec<PointRestoreSelect>> {
        ...
        let status_iter = (self.wee.rocksdb()).iterator_cf_opt(
            &self.wee.points_status.cf(),
            opts(range),
            IteratorMode::Start,
        );
        let mut info_iter =
            (self.wee.rocksdb()).raw_iterator_cf_opt(&self.wee.points_info.cf(), opts(range));
