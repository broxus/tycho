...
impl RoundCtx {
    ...
    fn own_point(&self, own_point: Result<&Point, &ProduceError>) {
        ...
        const SKIPPED: &str = "tycho_mempool_engine_produce_skipped";
        match own_point {
            Ok(own_point) => {
                tracing::info!(
                    parent: self.span(),
                ...
                metrics::counter!(SKIPPED).increment(0);
            }
            Err(reason) => {
                match reason {
                ...
                let label = match reason {
                    ProduceError::NotAllowed | ProduceError::NotEnoughEvidence => "evidence",
                    ProduceError::NextRoundThreshold => "next round",
                    ProduceError::NotScheduled => "not in v_set",
                    ProduceError::PrevPointMismatch { .. } => "prev point",
                };
                metrics::counter!(SKIPPED, "kind" => label).increment(1);
            }
        }
    }
}
