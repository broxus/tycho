...
use crate::intercom::{
    BroadcastFilter, Broadcaster, BroadcasterSignal, Collector, CollectorSignal,
    Dispatcher, Downloader, PeerSchedule, Responder,
};
...
impl RoundTaskReady {
    ...
    fn init_prev_broadcast(&mut self, prev_last_point: Point, round_ctx: &RoundCtx) {
        ...
        let (stub_tx, collector_signal_rx) = watch::channel(CollectorSignal::Retry {
            ready: true,
        });
    ...
    async fn own_point_task(
        ...
        let store = store;
        let head = head;
        let mut collector_signal_rx = collector_signal_rx;
        ...
        let is_in_time = loop {
            {
                ...
                tokio::select!(
                    ...
                    collector_signal_rx.changed() => { if recv_status.is_err() { break
                    false; } match * collector_signal_rx.borrow_and_update() {
