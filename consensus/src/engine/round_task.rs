...
impl RoundTaskReady {
    ...
    pub fn own_point_task(
        ...
        round_ctx: &RoundCtx,
    ) -> BoxFuture<'static, Option<TaskResult<Point>>> {
    ...
    pub fn run(
        self,
        own_point_fut: BoxFuture<'static, Option<TaskResult<Point>>>,
        ...
        let broadcaster_run = round_ctx.task().spawn({
            ...
            async move {
                let maybe_own_point = own_point_fut.await;

                if let Some(own_point) = maybe_own_point {
                    let own_point = own_point?;
