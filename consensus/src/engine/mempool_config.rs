...
impl CachedConfig {
    ...
    pub fn init(config: &MempoolConfig) -> (Point, OverlayId) {
        ...
        hasher.update(&(config.genesis_info.millis as u128).to_be_bytes());
        ...
        let genesis = Point::new(
            ...
            PointData {
                ...
                time: UnixTime::from_millis(config.genesis_info.millis),
                includes: Default::default(),
                witness: Default::default(),
                anchor_trigger: Link::ToSelf,
                anchor_proof: Link::ToSelf,
                anchor_time: UnixTime::from_millis(config.genesis_info.millis),
            },
        );
