...
impl EventData {
    ...
    pub fn tag(&self) -> EventTag {
        match self {
            Self::NodeStarted(_, _) => EventTag::NodeStarted,
    ...
    pub fn severity(&self) -> Option<EventSeverity> {
        #[allow(clippy::match_same_arms, reason = "common order")]
        match self {
            EventData::NodeStarted(_, _) | EventData::Banned { .. } | EventData::Unbanned(_) => {
...
impl Display for EventData {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Banned { unban_at, .. } => {
                write!(f, "banned until {unban_at}")
            }
            Self::Unbanned(_) => {
                write!(f, "unbanned")
            }
