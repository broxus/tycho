...
#[tokio::test]
async fn point_not_corrupt_across_batches() -> Result<()> {
    let journal_point_max_bytes = JournalPoint::max_tl_bytes(&default_test_config().conf);
    ...
    main_store.insert_point(&point, &PointStatusStored::Valid(<_>::random()));
    ...
    load_same_points(&store, std::slice::from_ref(&point)).context("after first store")?;

    store.store_records(batch(&[second]), journal_point_max_bytes)?;
    load_same_points(&store, std::slice::from_ref(&point)).context("after second store")?;

    store.delete(UnixTime::from_millis(0)..UnixTime::from_millis(1_500))?;
    load_same_points(&store, std::slice::from_ref(&point)).context("after deleting first")?;
...
async fn test() -> Result<()> {
    test_logger::spans("moderator_test", "info");
    ...
    store.store_records(batch(&stored), journal_point_max_bytes)?;

    let stored = stored
        .into_iter()
        .map(|full| RecordFull {
            key: full.key,
            value: item_to_value(full.item, &points),
        })
        .collect::<Vec<_>>();
...
fn item_to_value(item: JournalItem, stored_points: &[Point]) -> RecordValue {
    let mut points = Vec::new();
    let mut keys = Vec::new();
    item.fill_points(&mut points, &mut keys);

    let stored_keys = stored_points
        .iter()
    ...
    let point_keys = points
        ...
        .unique()
        .sorted()
        .map(|key| JournalPointRef {
            key,
            is_stored: stored_keys.contains(&key),
        })
        .collect();
