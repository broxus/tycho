...
use crate::moderator::{JournalPointRef, RecordKey, RecordValue};
...
pub fn batch(full_items: &[JournalItemFull]) -> RecordBatch<'_> {
    let mut all_keys_set = FastHashMap::with_capacity(full_items.len()); // roughly
    let mut batch = RecordBatch {
        records: Vec::with_capacity(full_items.len()),
        points: FastHashMap::with_capacity(full_items.len()), // roughly
        keys: Vec::with_capacity(full_items.len()),           // roughly
        max_record_bytes: 0,
        all_record_bytes: 0,
    };
    ...
    for JournalItemFull { key, item } in full_items {
        ...
        for point in r_points.drain(..) {
            match batch.points.entry(point.info().key()) {
                hash_map::Entry::Occupied(mut exist) => exist.get_mut().0 += 1,
                hash_map::Entry::Vacant(empty) => _ = empty.insert((1, point)),
            }
            r_unique_point_keys.insert(point.info().key());
        }

        for key in r_point_keys.drain(..) {
            if r_unique_point_keys.contains(&key) {
                continue; // duplicate reference as both key and point
            }
            match all_keys_set.entry(key) {
        ...
        for key in r_unique_point_keys.drain() {
            linked_point_keys.push(JournalPointRef {
                key,
                is_stored: false,
            });
        }
    ...
    for (key, (ref_count, _)) in &mut batch.points {
        // will not read points that are in mem
        if let Some(removed) = all_keys_set.remove(key) {
            *ref_count += removed;
        }
    }
