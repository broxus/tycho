...
use crate::moderator::event_data::EventData;
...
impl Moderator {
    ...
    pub fn new(network: &Network, mempool_db: Arc<MempoolDb>, version: &'static str) -> Self {
        ...
        let (delayed_writes_tx, delayed_writes_rx) = mpsc::unbounded_channel();
        ...
        delayed_writes_tx.send(start_event).ok();

        // todo :   load events for updater, wait until finished
        //   это можно делать в асинк таске сразу после создания -
        //   нужно только читать => не нужен конфиг который только для записи

        Self(Arc::new(ModeratorInner {
            delayed_writes_tx,
            ban_core: BanCore::new(network),
...
struct ModeratorInner {
    delayed_writes_tx: mpsc::UnboundedSender<Event>,
    ban_core: BanCore,
...
impl ModeratorTrait for ModeratorInner {
    ...
    fn send_report(&self, data: EventData) {
        ...
        self.delayed_writes_tx.send(Event { key, data }).ok();
    }
...
async fn delayed_db_write(
    event_store: EventStore,
    mut delayed_writes_rx: mpsc::UnboundedReceiver<Event>,
    ...
    loop {
        tokio::select! {
            ...
            Some(event) = delayed_writes_rx.recv() => {
                event_store
