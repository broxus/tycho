...
impl Collector {
    ...
    pub async fn run(
        ...
        let ctx = ctx;
        let head = head;
        let collector_signal = collector_signal;
        ...
        loop {
            {
                ...
                let __result = tokio::select! {
                    ...
                    => { if self.is_ready() { break; } else { self.collector_signal
                    .send_replace(CollectorSignal::Retry { ready : self.is_includes_ready
                    }); } }, consensus_round = self.consensus_round.next() => { if self
                    .match_consensus(consensus_round ?).is_err() { break; } }, else =>
