        ...
        loop {
            __guard.checkpoint(130u32);
            {
                ...
                let __result = tokio::select! {
                    biased; result = self.collector_status.changed() => { if self
                    .should_finish(result) { break; } }, update = self.peer_updates
    ...
    fn should_finish(
        ...
        let is_ready = if collector_signal.is_ok() {
            if self.rejections.len() >= self.signers_count.reliable_minority() {
                ...
                if collector_status.attempt
                    >= self.ctx.conf().consensus.min_sign_attempts.get()
        ...
        tracing::debug!(
            parent : self.ctx.span(), result = is_ready, collector =
            format!("{{ {collector_signal:?}, attempt={}, ready={} }}", collector_status
            .attempt, collector_status.ready), signatures = self.signatures.len(), "2F" =
            self.signers_count.majority_of_others(), rejections = self.rejections.len(),
            "F+1" = self.signers_count.reliable_minority(), "ready?",
        );
        is_ready
    }
