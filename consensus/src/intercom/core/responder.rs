    ...
    async fn handle_query(self: Arc<Self>, req: ServiceRequest) -> Option<Response> {
        ...
        let raw_query = match QueryRequestRaw::new(req.body) {
            ...
            Err(error) => {
                ...
                (state.moderator).send_report(JournalEvent::UnknownQuery(*peer_id));
                return None;
            }
        };
        ...
        let medium_query = match raw_query.parse() {
            ...
            Err(error) => {
                ...
                let event = JournalEvent::BadRequest(*peer_id, raw_query_tag, error);
        ...
        let _permit = match rate_limit {
            ...
            Err(error) => {
                ...
                let event = JournalEvent::QueryLimitReached(*peer_id, raw_query_tag, error);
                state.moderator.send_report(event);
                return None;
            }
        };
        ...
        let query = match medium_query {
            QueryRequestMedium::Broadcast(bytes, _) => {
                match rayon_run_fifo(|| Point::parse(bytes.into())).await {
                    ...
                    Ok(Err(bad_point)) => {
                        ...
                        (state.moderator).send_report(JournalEvent::BadPoint(*peer_id, bad_point));
                        return None;
                    }
                    Err(tl_error) => {
                        tracing::error!(
                        ...
                        let event = JournalEvent::BadRequest(*peer_id, raw_query_tag, tl_error);
                        state.moderator.send_report(event);
                        return None;
                    }
                }
            ...
            QueryRequestMedium::PointById(point_id) => QueryRequest::PointById(point_id),
        };
