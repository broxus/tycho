...
impl QueryRequestRaw {
    ...
    pub fn parse(self) -> Result<QueryRequest, TlError> {
        Ok(match self.tag {
            QueryRequestTag::Broadcast => {
                Point::verify_hash_inner(&self.request_body)?;
                QueryRequest::Broadcast(Point::new_from_bytes(self.request_body.into())?)
            }
