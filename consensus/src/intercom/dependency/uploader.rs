...
static LIMIT: LazyLock<SpawnLimit> = LazyLock::new(|| SpawnLimit::new(
    NodeConfig::get().max_upload_tasks.get() as usize,
));
pub struct Uploader;
impl Uploader {
    pub async fn find(
            ...
            let task_opt = LIMIT
                .try_spawn_blocking(
                    round_ctx.task(),
                    {
                        ...
                        move || {
                            let status_opt = store
                                .get_status(point_id.round, &point_id.digest);
                            let result = match &status_opt {
                                Some(
                                    PointStatusStored::Validated(usable),
                                ...
                                Some(
                                    PointStatusStored::IllFormed(ill),
                                ) if ill.is_certified => PointByIdResponse::TryLater,
                                Some(
                                    PointStatusStored::NotFound(not_found),
                                ) if not_found.is_certified => PointByIdResponse::TryLater,
