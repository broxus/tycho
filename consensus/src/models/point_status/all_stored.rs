    ...
    fn decode_one() -> Result<()> {
        for _ in 0..10 {
            ...
            match PointStatusStored::decode(&PointStatusIllFormed::random().bytes())? {
                ...
                x => anyhow::bail!("IllFormed read as {x}"),
            }
            match PointStatusStored::decode(&PointStatusNotFound::random().bytes())? {
                PointStatusStored::NotFound(_) => {}
                x => anyhow::bail!("NotFound read as {x}"),
            }
            match PointStatusStored::decode(&PointStatusFound::random().bytes())? {
                PointStatusStored::Found(_) => {}
                x => anyhow::bail!("Found read as {x}"),
            }
            match PointStatusStored::decode(&PointStatusCommittable::random().bytes())? {
                PointStatusStored::Committable(_) => {}
                x => anyhow::bail!("NotFound read as {x}"),
            }
            match PointStatusStored::decode(&PointStatusProven::random().bytes())? {
                PointStatusStored::Proven(_) => {}
                x => anyhow::bail!("Proven read as {x}"),
            }
        }
        Ok(())
    }
