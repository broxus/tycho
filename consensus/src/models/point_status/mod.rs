...
bitflags::bitflags! {
    ...
    pub struct StatusFlags : u16 {
        ...
        const Resolved = 0b_1 << 15; // top as we should keep the validation result
        const FirstResolved = 0b_1 << 14;
        // Resolved=true + Found=false => `NotFound` status (no point stored)
        // Resolved=false + Found=true => `Found` point that must be validated
        // Resolved=false + Found=false => `Proven` status to merge `HasProof` flag
        const Found = 0b_1 << 13;
