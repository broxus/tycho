    ...
    pub fn point(key_pair: &KeyPair, payload: &[Bytes], conf: &MempoolConfig) -> Point {
        ...
        let data = PointData {
            ...
            witness: FastHashMap::from_iter(std::array::from_fn::<_, { PEERS / 3 }, _>(|_| {
                (PeerId(rand::random()), Digest::new_random())
            })),
            evidence,
