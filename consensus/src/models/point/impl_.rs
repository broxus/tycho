...
use crate::models::point::proto_utils::{
    PointBodyWrite, PointRawRead, PointRead, PointWrite,
};
...
impl Point {
    pub fn max_byte_size(consensus_config: &ConsensusConfig) -> usize {
        ...
        let max_payload_size: usize = tl_proto::bytes_max_size_hint(min_ext_msg_boc_size)
            * (1
                + (consensus_config.payload_batch_bytes.get() as usize
                    / min_ext_msg_boc_size));
    ...
    pub fn new(
        ...
        assert_eq!(
            author, PeerId::from(local_keypair.public_key),
        ...
        serialized[4 + Digest::MAX_TL_BYTES..body_offset]
            .copy_from_slice(signature.inner());
        let payload_len = u32::try_from(payload.len()).unwrap_or(u32::MAX);
        let payload_bytes = u32::try_from(payload.iter().fold(0, |acc, b| acc + b.len()))
            .unwrap_or(u32::MAX);
    ...
    pub fn from_bytes(serialized: Vec<u8>) -> Result<Self, TlError> {
        ...
        let payload_bytes = u32::try_from(
                read.body.payload.iter().fold(0, |acc, b| acc + b.len()),
            )
            .unwrap_or(u32::MAX);
        let info = PointInfo::new(
            read.digest,
    ...
    pub fn parse(
        serialized: Vec<u8>,
    ) -> Result<Result<Self, PointIntegrityError>, TlError> {
        fn is_evidence_ok(info: &PointInfo) -> bool {
            info.prev_digest()
                .is_none_or(|prev_proof| {
                    (info.evidence().iter())
                        .all(|(peer, sig)| sig.verifies(peer, prev_proof))
                })
        }
    ...
    pub fn read_payload_from_tl_bytes<T>(
        data: T,
        bump: &Bump,
    ) -> Result<Vec<&[u8]>, TlError>
    where
        T: AsRef<[u8]>,
    {
...
mod tests {
    ...
    pub fn check_sig() {
        ...
        println!("check {PEERS} sigs took {}", humantime::format_duration(elapsed));
        let timer = Instant::now();
        assert!(
            data.par_iter().all(| (peer_id, sig) | sig.verifies(peer_id, & digest)),
            "invalid signature"
        );
        let elapsed = timer.elapsed();
        println!(
            "check {PEERS} sigs in par iter took {}", humantime::format_duration(elapsed)
        );
    }
    #[tokio::test]
    pub async fn check_sig_on_rayon() {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(check_sig_on_rayon)),
            file!(),
            330u32,
        );
        let _conf = default_test_config().conf;
        ...
        let timer = Instant::now();
        {
            ...
            let __result = rayon_run(move || {
                    assert!(
                        data.iter().all(| (peer_id, sig) | sig.verifies(peer_id, &
    ...
    pub fn check_new_point() {
        ...
        println!(
            "tl write {} bytes of point with {} bytes payload took {}", point
            .serialized().len(), info.payload_bytes(),
    ...
    pub fn massive_point_deserialization() {
        ...
        println!(
            ...
            point.serialized().len(), info.payload_bytes(),
            humantime::format_duration(elapsed)
        );
    }
    #[test]
    pub fn read_payload_from_tl() {
        ...
        assert_eq!(
            payload.iter().map(| m | m.len()).sum::< usize > (), info.payload_bytes() as
            usize
        );
        println!(
            "read {} bytes of payload took {}", info.payload_bytes(),
            humantime::format_duration(elapsed)
        );
    }
    #[test]
