...
use crate::models::{
    AnchorStageRole, ChainedAnchorProof, Digest, IndirectLink, Link, PointData, PointId, PointKey,
    Round, Signature, StructureIssue, UnixTime,
};
...
impl PointInfo {
    ...
    pub fn chained_proof_to_through(&self) -> Option<(&PointId, PointId)> {
        self.chained_anchor_proof().map(|link| {
            let through = self.0.data.indirect_link_through(link, self.round());
            (&link.to, through)
        })
    }
