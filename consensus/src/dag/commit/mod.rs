...
impl Committer {
    pub fn init(&mut self, bottom_round: &DagRound, conf: &MempoolConfig) -> Round {
        assert_eq!(self.full_history_bottom, Round::BOTTOM, "already initialized");
        self.dag.init(bottom_round);
        self.full_history_bottom = bottom_round.round()
            + conf.consensus.commit_history_rounds.get();
    ...
    pub fn drop_upto(
        ...
        self.full_history_bottom = actual_bottom
            + conf.consensus.commit_history_rounds.get();
    ...
    fn commit_up_to(
        ...
        while let Some(next) = self.anchor_chain.next() {
            ...
            let _span = tracing::error_span!(
                "anchor", author = display(next.anchor.author().alt()), round = next
                .anchor.round().0, digest = display(next.anchor.digest().alt()), proof =
                display(next.proof.digest().alt()), trigger = (next.direct_trigger
                .as_ref()).map(| p | display(p.digest().alt())),
            )
                .entered();
            match self.dequeue_anchor(next, conf) {
                Ok(data) => committed.push(data),
                Err(SyncError::TryLater) => break,
                Err(SyncError::HistoryConflict(round)) => {
    ...
    fn enqueue_new_anchors(&mut self, current_round: Round) -> CommitterResult<()> {
        ...
        for trigger in triggers {
            let last_proof_round = match self.anchor_chain.top_proof_round() {
                ...
                None => {
                    match self.dag.last_unusable_proof_round(&trigger) {
                        Ok(last_unusable_proof_round) => last_unusable_proof_round,
                        Err(SyncError::TryLater) => return Ok(()),
                        Err(SyncError::HistoryConflict(round)) => {
                            return Err(HistoryConflict(round));
                        }
                    }
            ...
            for next in chain_part {
                if let Some(back) = self.anchor_chain.top() {
                    assert_eq!(
                        next.anchor.anchor_round(AnchorStageRole::Proof), back.proof
                        .round(), "chain part is not contiguous by rounds"
                    );
                }
    ...
    fn dequeue_anchor(
        ...
        (self.dag)
            .drop_upto(next.anchor.round() - conf.consensus.commit_history_rounds.get());
        let uncommitted = match (self.dag)
            .gather_uncommitted(self.full_history_bottom, &next.anchor, conf)
        {
...
impl std::fmt::Debug for AltFmt<'_, Committer> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f, "{:?} chain {:?} full history bottom {}", inner.dag.alt(), inner
            .anchor_chain.alt(), inner.full_history_bottom.0,
        )
    }
}
impl std::fmt::Display for AltFmt<'_, Committer> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let inner = AltFormat::unpack(self);
        write!(
            f, "{} chain {} full history bottom {}", inner.dag.alt(), inner.anchor_chain
            .alt(), inner.full_history_bottom.0,
        )
    }
}
#[cfg(test)]
mod test {
    ...
    async fn test_commit_with_gap() {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(test_commit_with_gap)),
            file!(),
            293u32,
        );
        ...
        let (peer_schedule, stub_downloader, genesis, engine_ctx) = test_utils::make_engine_parts(
            &peers,
            local_keys.clone(),
        );
        let conf = engine_ctx.conf();
        let mut round_ctx = RoundCtx::new(&engine_ctx, conf.genesis_round);
        let genesis_round = DagRound::new_bottom(
            conf.genesis_round,
            &peer_schedule,
            conf,
        );
        ...
        for round in (0..100).map(Round) {
            ...
            if let Some(skip_to) = dag
                .fill_to_top(round, Some(&mut committer), &peer_schedule, &round_ctx)
            {
                println!(
                    "gap: next anchor with full history not earlier than {skip_to:?}"
                );
            }
            {
                __guard.end_section(345u32);
                let __result = test_utils::populate_points(
                        dag.top(),
                        &peers,
                        local_keys,
                        &peer_schedule,
        ...
        println!(
            "{} with {PEER_COUNT} peers populated and validated", committer.dag.alt()
        );
        assert_eq!(
            committer.dag.bottom_round(), Round(25),
            "test config changed? should update test then"
        );
        let mut r_points = vec![];
        for i in 50..55 {
            __guard.checkpoint(368u32);
    ...
    fn remove_round(
        dag: &mut DagBack,
        round: Round,
    ) -> (Round, FastDashMap<PeerId, DagLocation>) {
    ...
    fn restore_round(
        dag: &mut DagBack,
        pack: (Round, FastDashMap<PeerId, DagLocation>),
    ) {
    ...
    fn remove_point(
        ...
        match in_question.anchor_stage() {
            Some(stage) if stage.leader == peer_id => {
                match stage.role {
                    AnchorStageRole::Trigger => println!("removed trigger @ {round:?}"),
                    AnchorStageRole::Proof => println!("removed proof @ {round:?}"),
                }
    ...
    fn commit(
        ...
        let committed = if let Some(up_to) = up_to {
            ...
            committer.commit(conf).expect("no certified NotFound or Invalid points")
        };
