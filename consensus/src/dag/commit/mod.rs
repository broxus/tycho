...
impl Committer {
    ...
    pub fn remove_committed(&mut self, last_anchor: Round, conf: &MempoolConfig) -> TaskResult<()> {
        // in case previous anchor was triggered directly - rounds are already dropped
        for r_0 in (self.dag).drain_upto(last_anchor - conf.consensus.commit_history_rounds) {
