...
use crate::models::{DagPoint, Digest, PerPeerStats, Round};
...
pub struct RoundInspector {
    data: FastHashMap<PeerId, PerPeer>,
    last_round: Round,
...
impl RoundInspector {
    pub fn inspect(&mut self, r_0: &DagRound) -> TaskResult<()> {
        let leader_used = r_0
            .anchor_stage()
            .map(|a| (a.leader, a.is_used.load(atomic::Ordering::Acquire)));

        // map has full peer set for this round, but maybe with empty value
        let p_0 = Self::collect_points(r_0)?;

        for (peer, versions) in &p_0 {
            let per_peer = self.data.entry(*peer).or_insert_with(|| PerPeer {
                first_round: r_0.round(),
                stats: PerPeerStats::default(),
            });
            ...
            for version in versions {
                match version {
                    ...
                    DagPoint::Invalid(_) => {
                        per_peer.stats.invalid_points += 1;
                        signed_versions += 1;
                    }
                    DagPoint::IllFormed(_) => {
                        per_peer.stats.ill_formed_points += 1;
                        signed_versions += 1;
                    }
                    DagPoint::NotFound(_) => {}
                }
            }
            per_peer.stats.valid_points += has_valid as u32;
            if signed_versions == 0 {
                per_peer.stats.skipped_rounds += 1;
            } else {
                per_peer.stats.equivocated += signed_versions - 1;
            }
            ...
            {
                if used {
                    per_peer.stats.was_leader += 1;
                } else {
                    per_peer.stats.was_not_leader += 1;
                }
            }
        }
        ...
        if self.last_round == r_0.round().prev() {
            ...
            for (signer, ref_data) in prev_peer_ref_data {
                ...
                let per_peer = self.data.get_mut(&signer).expect("must be in set");
                per_peer.stats.references_skipped += ref_data.signed.len() as u32;
            }
        }
