...
impl RoundInspector {
    ...
    pub fn inspect(&mut self, r_0: &DagRound) -> TaskResult<()> {
        ...
        for (author, authored) in &p_0 {
            ...
            author_stats.last_round = r_0.round().0;
            ...
            for version in authored {
                match version {
                    ...
                    DagPoint::Invalid(invalid) => {
                        authored_versions += 1;
                        author_stats.invalid_points += 1;
                        self.invalid.push(invalid.clone());
                    }
                    DagPoint::IllFormed(ill) => {
                        authored_versions += 1;
                        author_stats.ill_formed_points += 1;
                        self.ill_formed.push(ill.clone());
                    }
                    DagPoint::NotFound(_) => {}
                }
            }
            author_stats.valid_points += has_valid as u32;
            if authored_versions == 0 {
                author_stats.skipped_rounds += 1;
            } else {
                author_stats.equivocated += authored_versions - 1;
                if author_stats.equivocated > 0 {
            ...
            {
                if used {
                    author_stats.was_leader += 1;
                } else {
                    author_stats.was_not_leader += 1;
                }
            }
        }
        ...
        if prev_last_round == self.last_round.prev() {
            ...
            for (signer, signer_prev_round) in prev_last_round_data_map {
                ...
                signer_stats.references_skipped += signer_prev_round.signed_proofs.len() as u32;
