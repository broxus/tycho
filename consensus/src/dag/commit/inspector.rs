...
use crate::models::{DagPoint, Digest, Round};
...
pub struct RoundInspector {
    accum: FastHashMap<PeerId, MempoolPeerStats>,
    last_round: Round,
...
impl RoundInspector {
    pub fn get_stats(&mut self) -> FastHashMap<PeerId, MempoolPeerStats> {
        let capacity = if self.accum.capacity() >= self.accum.len() * 4 {
            self.accum.len() * 2
        } else {
            self.accum.capacity()
        };
        mem::replace(&mut self.accum, FastHashMap::with_capacity(capacity))
    }
    ...
    pub fn inspect(&mut self, r_0: &DagRound) -> TaskResult<()> {
        ...
        if self.last_round == r_0.round().prev() {
            ...
            for (signer, ref_data) in prev_peer_ref_data {
                ...
                let peer_stats = self.accum.get_mut(&signer).expect("must be in set");
