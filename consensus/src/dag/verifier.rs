...
use crate::models::{
    ...
    PointMap, Round, StructureIssue, Through, UnixTime,
};
...
pub enum InvalidReason {
    #[error("invalid after load from DB{}", no_dag_round.then_some(" (no dag round)").unwrap_or("")
    )]
...
impl std::fmt::Display for InvalidDependency {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "dependency invalid {}: ", self.through.alt())?;
        write!(f, "{:?} {}", self.point_id.alt(), self.reason)
    }
}
...
impl Verifier {
    ...
    async fn check_indirect_links(
        ...
        let anchor_trigger_id = match info.anchor_link(AnchorStageRole::Trigger) {
            Link::Indirect(link) => Some(link),
            Link::ToSelf | Link::Direct(_) => None,
        };
        let anchor_proof_id = match info.anchor_link(AnchorStageRole::Proof) {
            Link::Indirect(link) => Some(link),
            Link::ToSelf | Link::Direct(_) => None,
        };
        let chained_proof_to_through = info.chained_anchor_proof();

        let mut rev_sorted = [anchor_trigger_id, anchor_proof_id, chained_proof_to_through];
        ...
        while let Some(task_result) = linked_deps.next().await {
            ...
            if let Some(anchor_proof_id) = anchor_proof_id
                && dep_id == anchor_proof_id.to
    ...
    fn dependency<'a>(
        ...
        let (inv_info, root_cause_id, reason) = match dag_point {
            ...
            DagPoint::TransInvalid(invalid) => {
                ...
                let last_invalid = invalid.root_cause().point_id.round
                    + conf.consensus.commit_history_rounds.get();
                if prev_round >= last_invalid {
                    return Ok(invalid.info());
                }
                let root_cause_id = &invalid.root_cause().point_id;
                let reason = &invalid.root_cause().reason;
                (invalid.info(), root_cause_id, reason)
            }
    ...
    fn is_well_formed(info: &PointInfo, conf: &MempoolConfig) -> Option<IllFormedReason> {
        if info.round() == conf.genesis_round {
            ...
            let must_use_chained_proof = info.anchor_proof() == &Link::ToSelf;
