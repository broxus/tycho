...
impl InclusionState {
    ...
    pub fn resolve(&self, dag_point: &DagPoint) {
        ...
        if dag_point.is_first_resolved() {
            ValidateCtx::resolved(dag_point, true);
            ...
            match resolved {
                FirstResolved::Valid(digest, _) | FirstResolved::NotValid(digest)
                ...
                FirstResolved::Closed => {} // uncounted
                _ => ValidateCtx::resolved(dag_point, false),
            }
        } else {
            ValidateCtx::resolved(dag_point, false);
        }
