...
def build_target(
    ...
    t = target(expr, legend_format=legend_format)

    actual_scale = 1 if scale in (None, 0) else scale
    if actual_scale != 1:
        t.expr = f"({t.expr}) / {actual_scale}"
...
def create_gauge_panel(
    ...
    if isinstance(expr, str):
        exprs = [Expr(metric=expr, label_selectors=labels)]
    ...
    targets = [build_target(e, legend_format=legend_format, scale=scale) for e in exprs]
...
def collator_wu_params() -> RowPanel:
    metrics = [
        create_gauge_panel(
            "tycho_wu_tuner_actual_wu_price",
            "actual wu price",
        ),
        create_gauge_panel(
            "tycho_wu_tuner_last_adjustment_wu_price",
            "last adjustment wu price",
        ),
        create_gauge_panel(
            "tycho_wu_tuner_adaptive_wu_price",
            "adaptive wu price",
        ),
        create_gauge_panel(
            "tycho_wu_tuner_target_wu_price",
            "target wu price",
        ),
        create_gauge_panel(
...
def mempool_stats() -> RowPanel:
    ...
    metrics = [
        ...
        counter_with_defaults("points_proved", "Proved points"),
...
def mempool_misc() -> RowPanel:
    metrics = [
        ...
        create_counter_panel(
            ...
            "Moderator event",
            labels_selectors=['peer_id=~"$peer_id"'],
...
def mempool_storage() -> RowPanel:
    metrics = [
        ...
        create_heatmap_panel(
            ...
            "Wait for compact in clean tasks",
        ),
        create_gauge_panel(
