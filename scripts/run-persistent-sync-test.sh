...
#######################################
# Build & stage repo copy
#######################################
log "🔨 Pre‑compiling Rust binaries…"
cargo build
...
jq '.starter.custom_boot_offset="0s" | .starter.each_key_block_is_persistent=true' \
...
while :; do
  ...
  if [[ -n "$DIFF_TAIL" && ${DIFF_TAIL%.*} -gt $DIFF_TAIL_THRESHOLD && $NODE4_STARTED -eq 0 ]]; then
    ...
    for id in "${ACTIVE_NODES[@]}"; do
      ...
      ./target/debug/tycho tool bc set-param \
    ...
    while true; do
      ...
      for id in 1 2 3 4; do
        ...
        if [[ -z $DIFF || ${DIFF%.*} -gt 1 ]]; then all_ok=false; log "⏳ node $id diff‑tail=${DIFF:-<none>}"; fi
      done
