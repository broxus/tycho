...
impl Validator for ValidatorStdImpl {
    fn add_session(&self, info: AddSession<'_>) -> Result<()> {
        ...
        match shard_sessions.entry(info.session_id) {
            indexmap::map::Entry::Vacant(entry) => {
                tracing::debug!(
                    target : tracing_targets::VALIDATOR, session = ?
                    DebugLogValidatorSesssion(& session), "new validator session added",
                );
            ...
            indexmap::map::Entry::Occupied(_) => {
                anyhow::bail!(
                    "validator session already exists: ({}, {:?})", info.shard_ident,
                    info.session_id
                )
            }
        }
    ...
    async fn validate(
        ...
        let session = 'session: {
            ...
            anyhow::bail!(
                "validator session not found: ({}, {:?})", block_id.shard, session_id,
            );
        };
        {
            __guard.end_section(143u32);
            let __result = session.validate_block(block_id).await;
    ...
    fn cancel_validation(
        ...
        let session = {
            ...
            let session = session_id
                .and_then(|id| {
                    shard_sessions
                        ...
                        .or_else(|| {
                            tracing::warn!(
                                target : tracing_targets::VALIDATOR, session_id = ? id,
                                shard = % until.shard,
            ...
            let session = match session {
                ...
                None => {
                    if let Some(s) = shard_sessions
                        .iter()
                        .rev()
                        .find_map(|(_, session)| {
                            (session.start_block_seqno() <= until.seqno)
                                .then(|| session.clone())
                        })
                    {
                        s
