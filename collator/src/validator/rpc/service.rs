        ...
        BoxFutureOrNoop::future(
            async move {
                ...
                match {
                    ...
                    Err(e) => {
                        tracing::debug!(
                            target : tracing_targets::VALIDATOR, % peer_id, block_seqno,
    ...
    E: ExchangeSignatures,
{
    ...
    #[tracing::instrument(
        ...
        skip_all,
        fields(shard_ident = %self.shard_ident, session_id = ?self.session_id)
    )]
    fn on_query(&self, req: ServiceRequest) -> Self::OnQueryFuture {
        ...
        tycho_network::match_tl_request!(
            body, tag = constructor, { rpc::ExchangeSignaturesOwned as r => {
            tracing::trace!(target : tracing_targets::VALIDATOR, block_seqno = r
            .block_seqno, "exchangeSignatures",); self.handle_exchange_signatures(& req
            .metadata.peer_id, r.block_seqno, r.signature) } }, e => {
            tracing::debug!(target : tracing_targets::VALIDATOR,
            "failed to deserializer query: {e}",); BoxFutureOrNoop::Noop }
        )
    }
    #[inline]
