...
fn gc_task<V: InternalMessageValue>(
    ...
    for (partition, delete_until) in &delete_until {
        for (shard, current_last_key) in delete_until.iter() {
            ...
            if can_delete {
                let range = vec![
                    QueueShardRange { shard_ident : * shard, from : QueueKey::default(),
                    to : current_last_key.end_key, }
                ];
                tracing::info!(
                    target : tracing_targets::MQ, % partition, % shard, last_queue_key =
                    % current_last_key.end_key, on_top_block_id = % current_last_key
                    .on_top_block_id.as_short_id(), "executing messages queue GC"
                );
                if let Err(e) = queue_state.delete(*partition, range.as_slice()) {
                    tracing::error!(
                        target : tracing_targets::MQ, "failed to delete messages: {e:?}"
                    );
                }
