...
use tycho_consensus::prelude::{
    AnchorStageRole, ConsensusConfigExt, MempoolAdapterStore, MempoolConfigBuilder,
    MempoolDb, MempoolNodeConfig, RoundWatch,
};
    ...
    pub fn load(
        ...
        for (anchor_round, (anchor, history)) in anchors {
            let payloads = (self.store)
                .expand_anchor_history(&anchor, &history, &bump, false);
            let ParserOutput { unique_messages, .. } = parser
                .parse_unique(anchor_round, payloads);
            let prev_linked_anchor = anchor
...
mod test {
    ...
    async fn dump_mempool_anchors() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(dump_mempool_anchors)),
            file!(),
            115u32,
        );
        init_logger("test_dump_mempool_anchors", "debug");
        let storage_conf = StorageConfig {
            root_dir: "../.temp/db1".into(),
            ..Default::default()
        };
        let ctx = {
            __guard.end_section(123u32);
            let __result = StorageContext::new(storage_conf).await;
