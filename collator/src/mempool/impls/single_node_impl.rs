...
impl MempoolAdapterSingleNodeImpl {
    fn process_start(&self, config_guard: &mut SingleNodeConfigAdapter) {
        ...
        let mut anchor_handler =
            AnchorSingleNodeHandler::new(self.cache.clone(), self.local_peer_id, consensus_config);
...
impl MempoolAdapter for MempoolAdapterSingleNodeImpl {
    async fn handle_mc_state_update(&self, new_cx: StateUpdateContext) -> Result<()> {
        ...
        if config_guard.inner_process.is_none() {
            self.process_start(&mut config_guard);
        }
