...
use crate::mempool::{
    DebugStateUpdateContext, ExternalMessage, GetAnchorResult, MempoolAdapter,
...
impl MempoolAdapterStubImpl {
    ...
    {
        tracing::info!(
            target : tracing_targets::MEMPOOL_ADAPTER, "creating mempool adapter"
        );
        let adapter = Self {
            listener,
    ...
    async fn stub_externals_generator(self: Arc<Self>, now: Option<u64>) {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(stub_externals_generator)),
            file!(),
            77u32,
        );
        let now = now;
        ...
        for anchor_id in 1.. {
            ...
            if self.sleep_between_anchors.load(Ordering::Acquire) {
                {
                    ...
                    let __result = tokio::time::sleep(make_round_interval() * 4).await;
            ...
            } else {
                {
                    ...
                    let __result = tokio::time::sleep(Duration::from_millis(10)).await;
                    __guard.start_section(88u32);
                    __result
                };
            }
            ...
            tracing::debug!(
                target : tracing_targets::MEMPOOL_ADAPTER, anchor_id = anchor.id,
                chain_time = anchor.chain_time, externals = anchor.externals.len(),
                "anchor added to cache",
            );
            {
                __guard.end_section(110u32);
                let __result = self.listener.on_new_anchor(anchor).await;
    ...
    async fn file_externals_generator(self: Arc<Self>, paths: Vec<PathBuf>) {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(file_externals_generator)),
            file!(),
            115u32,
        );
        let paths = paths;
                    ...
                    match make_anchor_from_file(anchor_id, prev_anchor_id, &path) {
                        ...
                        Err(e) => {
                            tracing::error!(
                                ...
                                prev_anchor_id, path = % path.display(),
...
impl MempoolAdapter for MempoolAdapterStubImpl {
    async fn handle_mc_state_update(&self, cx: StateUpdateContext) -> Result<()> {
        ...
        tracing::info!(
            ...
            "STUB: Processing state update from mc block {}: {:?}", cx.mc_block_id
            .as_short_id(), DebugStateUpdateContext(& cx),
        );
        Ok(())
    }
    async fn handle_signed_mc_block(&self, _mc_block_seqno: BlockSeqno) -> Result<()> {
        ...
        let _mc_block_seqno = _mc_block_seqno;
        Ok(())
    }
        ...
        loop {
            ...
            let Some(anchor) = self.anchors_cache.read().get(&anchor_id).cloned() else {
                ...
                last_attempt_at = Some(Instant::now());
                {
                    ...
                    let __result = tokio::time::sleep(
                            tokio::time::Duration::from_millis(1320),
                        )
                        .await;
        ...
        loop {
            ...
            match last_attempt_at {
                Some(last) => {
                    tracing::debug!(
                        target : tracing_targets::MEMPOOL_ADAPTER, prev_anchor_id,
                        anchor_id = anchor.id, elapsed = % format_duration(last
                        .elapsed()), "STUB: Returned the next anchor from mempool",
                    );
                }
                None => {
                    tracing::debug!(
    ...
    async fn update_delayed_config(
        ...
        let _consensus_config = _consensus_config;
        let _genesis_info = _genesis_info;
...
pub(crate) fn make_anchor_from_file(
    ...
    tracing::debug!(
        target : tracing_targets::MEMPOOL_ADAPTER, file_name, "read external from file"
    );
...
mod tests {
    ...
    impl MempoolEventListener for MempoolEventStubListener {
        async fn on_new_anchor(&self, anchor: Arc<MempoolAnchor>) -> Result<()> {
            ...
            tracing::trace!(
                ...
                anchor.id, anchor.chain_time, anchor.externals.len(),
            );
            Ok(())
        }
    }
    ...
    async fn test_stub_anchors_generator() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(test_stub_anchors_generator)),
            file!(),
            492u32,
        );
        ...
        let result = {
            ...
            let __result = adapter.get_anchor_by_id(3).await;
            __guard.start_section(499u32);
            __result
        }?;
        assert!(result.anchor().is_some());
        assert_eq!(result.anchor().unwrap().id, 3);
        let result = {
            __guard.end_section(504u32);
            let __result = adapter.get_next_anchor(3).await;
            __guard.start_section(504u32);
            __result
        }?;
        assert!(result.anchor().is_some());
        ...
        let result = {
            ...
            let __result = adapter.get_next_anchor(5).await;
        ...
        let result = {
            ...
            let __result = adapter.get_anchor_by_id(4).await;
        ...
        let result = {
            ...
            let __result = adapter.get_anchor_by_id(6).await;
