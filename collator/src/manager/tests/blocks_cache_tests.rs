...
fn test_apllied_range_update() {
    ...
    cache.update_last_collated_block_id(&block_id);
    println!();
    println!("update_last_collated_block_id(3405)");
    println!(
        "last collated mc seqno: {:?}, applied_mc_queue_range: {:?}",
        cache.last_collated_mc_block_id.map(|id| id.as_short_id()),
        cache.applied_mc_queue_range
    );

    assert_eq!(cache.last_collated_mc_block_id, Some(block_id));
