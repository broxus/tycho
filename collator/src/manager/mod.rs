    ...
    V: Validator,
{
    ...
    async fn handle_collation_skipped(
        ...
        if updated_collator_state == Some(CollatorState::Cancelled) {
            ...
            {
                __guard.end_section(695u32);
                return Ok(());
            };
        }
        {
            __guard.end_section(710u32);
            let __result = self
    ...
    pub async fn handle_collated_block_candidate(
        ...
        {
            __guard.end_section(817u32);
            let __result = self.ready_to_sync.notified().await;
            __guard.start_section(817u32);
            __result
        };
        scopeguard::defer!(self.ready_to_sync.notify_one());
        ...
        let should_sync_to_last_applied_mc_block = 'check_should_sync: {
            if let Some((_, applied_range_end)) = store_res.applied_mc_queue_range {
                ...
                if matches!(
                    ...
                    {
                        __guard.end_section(975u32);
                        __guard.start_section(975u32);
    ...
    async fn process_handle_block_from_bc_queue(
        ...
        loop {
            __guard.checkpoint(1193u32);
            let mut batch = Vec::with_capacity(BATCH_SIZE);
            {
                __guard.end_section(1195u32);
    ...
    async fn handle_block_from_bc(
        ...
        let Some(store_res) = {
            ...
            let __result = self
                ...
                .await;
            __guard.start_section(1278u32);
            __result
        }? else {
            {
                __guard.end_section(1280u32);
                return Ok(());
            };
        };
        if store_res.block_mismatch {
        ...
        let should_sync_to_last_applied_mc_block = 'check_should_sync: {
            if !is_last_mc_block_in_batch && !is_key_block {
                {
                    __guard.end_section(1352u32);
                    __guard.start_section(1352u32);
                    break 'check_should_sync false;
                };
            }
            if let Some((_, applied_range_end)) = store_res.applied_mc_queue_range {
                ...
                if matches!(
                    ...
                    {
                        __guard.end_section(1368u32);
                        __guard.start_section(1368u32);
                        break 'check_should_sync false;
                    };
                }
            }
        ...
        if should_sync_to_last_applied_mc_block {
            {
                ...
                let __result = self
                    ...
                    .await;
                __guard.start_section(1479u32);
                __result
            }?;
        } else {
            if store_res.received_and_collated {
                {
                    __guard.end_section(1490u32);
                    let __result = self.commit_valid_master_block(&block_id).await;
                    __guard.start_section(1490u32);
                    __result
                }?;
            }
        }
    ...
    async fn sync_to_applied_mc_block(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(sync_to_applied_mc_block)),
            file!(),
            1536u32,
        );
        let applied_range = applied_range;
        ...
        let all_shards_processed_to_by_partitions = {
            __guard.end_section(1606u32);
            let __result = Self::get_all_shards_processed_to_by_partitions_for_mc_block(
                    &last_applied_mc_block_key,
                    &self.blocks_cache,
                    self.state_node_adapter.clone(),
                )
                .await;
            __guard.start_section(1606u32);
            __result
        }?;
            ...
            {
                {
                    ...
                    let __result = Self::get_top_blocks_seqno(
        ...
        let queue_restore_res = {
            __guard.end_section(1671u32);
            let __result = Self::restore_queue(
                    &self.blocks_cache,
                    self.state_node_adapter.clone(),
        ...
        {
            __guard.end_section(1705u32);
            let __result = self.notify_mc_state_update_to_mempool(mc_data.clone()).await;
            __guard.start_section(1705u32);
            __result
        }?;
        ...
        {
            __guard.end_section(1716u32);
            let __result = self
                .process_mc_state_update(mc_data.clone(), process_state_update_mode)
                .await;
            __guard.start_section(1716u32);
            __result
        }?;
        {
            __guard.end_section(1723u32);
            let __result = self
                .notify_top_processed_to_anchor_to_mempool(
    ...
    async fn restore_queue(
        ...
        for (shard_id, min_processed_to) in &min_processed_to_by_shards {
            ...
            while let Some(prev_block_id) = prev_block_ids.pop_front() {
                __guard.checkpoint(1850u32);
                if prev_block_id.seqno == 0 {
                    {
                        __guard.end_section(1852u32);
                        __guard.start_section(1852u32);
                        continue;
                    };
                }
                    ...
                    && prev_block_id.seqno <= *border
                {
                    ...
                    {
                        __guard.end_section(1864u32);
                        __guard.start_section(1864u32);
                        continue;
                    };
                }
                if let Some(init_mc_block_id) = init_mc_block_id {
                    ...
                    if prev_block_id.is_masterchain() {
                        ...
                        if !init_mc_block_reached {
                            let prev_ref_by_mc_seqno = {
                                __guard.end_section(1890u32);
                                let __result = state_node_adapter
                                    .get_ref_by_mc_seqno(&prev_block_id)
                                    .await;
                                __guard.start_section(1890u32);
                                __result
                            }?
                                .unwrap();
                    ...
                    if skip_diff {
                        ...
                        {
                            __guard.end_section(1911u32);
                            __guard.start_section(1911u32);
                            continue;
                        };
                    }
                }
                let Some(queue_diff_stuff) = {
                    ...
                    for shard in before_tail_block_ids.keys() {
                        __guard.checkpoint(1924u32);
                        let labels = [("workchain", shard.workchain().to_string())];
                        metrics::gauge!("tycho_collator_sync_is_running", & labels)
                            .set(0);
                    }
                    {
                        __guard.end_section(1929u32);
                        return Ok(res);
                    };
                };
                ...
                if diff_required {
                    ...
                    let block_stuff = {
                        __guard.end_section(1946u32);
                        let __result = state_node_adapter
                            .load_block(&prev_block_id)
                            .await;
                        __guard.start_section(1946u32);
                        __result
                    }?
                        .unwrap();
            ...
            if subgraph.master_block.block_id.seqno != 0 {
                    ...
                    .chain(subgraph.shard_blocks.iter())
                {
                        ...
                        && block_entry.block_id.seqno <= *border
                    {
                        ...
                        {
                            __guard.end_section(2031u32);
                            __guard.start_section(2031u32);
                            continue;
                        };
                    }
    ...
    async fn process_mc_state_update(
        ...
        if let ProcessMcStateUpdateMode::StartCollation { reset_collators } = mode {
            ...
            {
                {
                    __guard.end_section(2148u32);
                    let __result = self
                        .refresh_collation_sessions(mc_data, reset_collators)
                        .await;
                    __guard.start_section(2148u32);
                    __result
                }?;
            } else {
                tracing::warn!(
    ...
    async fn notify_to_mempool_and_process_delayed_mc_state_update(
        ...
        if let Some(mc_data) = delayed_mc_data {
            {
                __guard.end_section(2185u32);
                let __result = self
                    .notify_mc_state_update_to_mempool(mc_data.clone())
                    .await;
                __guard.start_section(2185u32);
                __result
            }?;
            {
                __guard.end_section(2192u32);
                let __result = self
                    .process_mc_state_update(
                        mc_data.clone(),
                        ProcessMcStateUpdateMode::StartCollation {
    ...
    async fn enqueue_try_collate(&self, shard_id: &ShardIdent) -> Result<()> {
        ...
        let Some(collator) = self
            ...
            {
                __guard.end_section(3185u32);
                return Ok(());
            };
        };
        {
            __guard.end_section(3188u32);
            let __result = collator.enqueue_try_collate().await;
            __guard.start_section(3188u32);
            __result
        }?;
        tracing::info!(
            target : tracing_targets::COLLATION_MANAGER,
    ...
    pub async fn handle_validated_master_block(
        ...
        if !updated {
            {
                __guard.end_section(3222u32);
                return Ok(());
            };
        }
        {
            __guard.end_section(3225u32);
            let __result = self.ready_to_sync.notified().await;
            __guard.start_section(3225u32);
            __result
        };
        {
            __guard.end_section(3228u32);
            let __result = self.commit_valid_master_block(&block_id).await;
            __guard.start_section(3228u32);
            __result
        }?;
    ...
    async fn get_top_blocks_seqno(
        ...
        match top_shard_blocks {
            None => {
                let state = match {
                    __guard.end_section(3396u32);
                    let __result = state_node_adapter
                        .load_state(mc_block_id.seqno, mc_block_id)
                        .await;
                    __guard.start_section(3396u32);
                    __result
                } {
                    Err(err) => {
