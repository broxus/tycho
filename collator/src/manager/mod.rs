    ...
    V: Validator,
{
        ...
        let state_cloned = state.clone();
        {
            ...
            let __result = self
                .spawn_task(move |worker| {
                    Box::pin(
                        async move { worker.cancel_validation_sessions_until_block(state_cloned) },
                    )
                })
                .await;
            __guard.start_section(line!());
    ...
    async fn on_block_accepted_external(&self, state: &ShardStateStuff) -> Result<()> {
        ...
        };
        {
            __guard.end_section(line!());
            let __result = self
                .enqueue_task(method_to_async_closure!(enqueue_handle_block_from_bc, ctx))
                .await;
            __guard.start_section(line!());
            __result
        }?;
    ...
    V: Validator,
{
    ...
    pub async fn process_new_anchor_from_mempool(&self, _anchor: Arc<MempoolAnchor>) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(process_new_anchor_from_mempool)
            ),
            file!(),
            line!(),
        );
        ...
        I: Iterator<Item = ShardDescriptionShort>,
    {
        ...
        );
        {
            __guard.end_section(line!());
            let __result = self
                .notify_top_processed_to_anchor_to_mempool(mc_block_seqno, top_processed_to_anchor)
                .await;
            __guard.start_section(line!());
            __result
        }?;
    ...
    pub async fn handle_collated_block_candidate(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(handle_collated_block_candidate)
            ),
            file!(),
            line!(),
        );
            ...
            if store_res.received_and_collated {
                ...
                tokio::spawn(async move {
                    ...
                    let validation_result = {
                        __guard.end_section(line!());
                        let __result = validator.validate(validation_session_id, &block_id).await;
                        __guard.start_section(line!());
                        __result
                    };
    ...
    pub async fn enqueue_handle_block_from_bc(&self, ctx: HandledBlockFromBcCtx) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(enqueue_handle_block_from_bc)
            ),
            file!(),
            line!(),
        );
    ...
    async fn process_handle_block_from_bc_queue(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(process_handle_block_from_bc_queue)
            ),
            file!(),
            line!(),
        );
    ...
    async fn sync_to_applied_mc_block_if_exist(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(sync_to_applied_mc_block_if_exist)
            ),
            file!(),
            line!(),
        );
            ...
            let processed_to = match processed_to_opt {
                ...
                None => {
                    if top_block_id.seqno == 0 {
                        ...
                        let state = {
                            __guard.end_section(line!());
                            let __result = state_node_adapter.load_state(&top_block_id).await;
                            __guard.start_section(line!());
                            __result
                        }?;
                        let processed_upto = state.state().processed_upto.load()?;
        ...
        for (shard_id, min_processed_to) in &min_processed_to_by_shards {
            ...
            while let Some(prev_block_id) = prev_block_ids.pop_front() {
                    ...
                    if prev_block_id.is_masterchain() {
                        ...
                        if !init_mc_block_reached {
                            let prev_ref_by_mc_seqno = {
                                __guard.end_section(line!());
                                let __result =
                                    state_node_adapter.get_ref_by_mc_seqno(&prev_block_id).await;
                                __guard.start_section(line!());
                                __result
                            }?
                            .unwrap();
                ...
                if diff_required {
                    ...
                    let block_stuff = {
                        __guard.end_section(line!());
                        let __result = state_node_adapter.load_block(&prev_block_id).await;
                        __guard.start_section(line!());
                        __result
                    }?
                    .unwrap();
    ...
    async fn notify_mc_state_update_to_mempool(&self, mc_data: Arc<McData>) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(notify_mc_state_update_to_mempool)
            ),
            file!(),
            line!(),
        );
    ...
    fn detect_next_collation_step(
        ...
        let should_collate_by_current_shard = if hard_forced_for_all {
            ...
            tracing::info!(
                target : tracing_targets::COLLATION_MANAGER,
                "Master block collation forced in current shard {}", shard_id,
            );
            true
        } else {
            ...
            if mc_block_interval_exceeded {
                tracing::info!(
                    target : tracing_targets::COLLATION_MANAGER,
                    mc_block_min_interval_ms, chain_time_elapsed_ms, % shard_id,
        ...
        tracing::info!(
            target : tracing_targets::COLLATION_MANAGER, hard_forced_for_all,
            forced_in_current_shard, mc_block_min_interval_ms, next_mc_block_chain_time,
    ...
    pub async fn handle_validated_master_block(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(handle_validated_master_block)
            ),
            file!(),
            line!(),
        );
    ...
    async fn get_top_blocks_seqno(
        ...
        match top_shard_blocks {
            None => {
                let state = match {
                    __guard.end_section(line!());
                    let __result = state_node_adapter.load_state(mc_block_id).await;
                    __guard.start_section(line!());
                    __result
                } {
