...
#[async_trait]
impl<CF, V> ValidatorEventListener for AsyncDispatcher<CollationManager<CF, V>>
where
    CF: CollatorFactory,
    V: Validator,
{
    ...
    V: Validator,
{
    ...
    pub async fn process_collated_block_candidate(
        ...
        if block_already_stored_by_blockchain {
            if candidate_id.is_masterchain() {
                self.process_valid_master_block(&candidate_id).await?;
            } else {
                self.process_valid_shard_block(&candidate_id).await?;
            }
        } else {
