...
type ShardBlocksMap = FastDashMap<ShardIdent, BTreeMap<u32, ShardBlockData>>;
            ...
            .load_state_ext(ref_by_mc_seqno, block_id, hint, |block_id| {
                self.shard_blocks
                    ...
                    .and_then(|shard_blocks| {
                        let block = shard_blocks.get(&block_id.seqno)?;
    ...
    fn fill_shard_blocks_cache(&self, ref_by_mc_seqno: u32, block: BlockStuff) -> Result<()> {
        ...
        if !block_id.is_masterchain() {
            ...
            tracing::debug!(
                ...
                %prev_id,
                ref_by_mc_seqno,
                "fill_shard_blocks_cache: inserted shard block update"
            );
    ...
    async fn handle_state(&self, mc_block_id: &BlockId, state: &ShardStateStuff) -> Result<()> {
        ...
        if shard.is_masterchain() {
            ...
            for entry in state.shards()?.latest_blocks() {
                ...
                // Keep it and everything after to preserve the merkle update chain.
                let retain_from = sb
                    .range(..top_block_id.seqno)
                    .find(|(seqno, data)| {
                        let block_id = BlockId {
                            shard: top_block_id.shard,
                            seqno: **seqno,
                    ...
                    .map_or(top_block_id.seqno, |(seqno, _)| *seqno);
            ...
            metrics::gauge!("tycho_shard_blocks_count_in_collation_manager_cache")
                .set(count as f64);
        }
