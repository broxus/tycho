...
impl StateNodeAdapter for StateNodeAdapterStdImpl {
    ...
    async fn load_state_root(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(load_state_root)),
            file!(),
            line!(),
        );
        let ref_by_mc_seqno = ref_by_mc_seqno;
        ...
        let _histogram = HistogramGuard::begin(
            "tycho_collator_state_load_state_root_time",
        );
        let hash = self.storage.shard_state_storage().load_state_root(block_id)?;
        let cell = self.storage.shard_state_storage().load_cell(&hash, ref_by_mc_seqno)?;
        Ok(cell)
    }
    async fn store_state_root(
        &self,
