    ...
    async fn handle_state(&self, state: &ShardStateStuff) -> Result<()> {
        ...
        for (shard, seqno) in &to_split {
            if let Some(mut shard_blocks) = self.blocks.get_mut(shard) {
                ...
                to_drop.push(std::mem::replace(&mut *shard_blocks, new_shard_blocks));
            }
        }
        tracing::warn!(
            "aaaaaaaaaaaaaaaaaa secs={}, to_split={}",
        ...
        let start = std::time::Instant::now();
        drop(to_drop);
        tracing::warn!("bbbbbbbbbbbbbbbbb secs={}", start.elapsed().as_secs_f64());
