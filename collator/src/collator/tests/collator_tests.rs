...
async fn test_import_init_anchors() {
    ...
    tracing::debug!(
        ...
        anchors_count_above_last_imported_in_current_shard, anchors_info.as_slice(),
    );
    ...
    let (last_imported_id, last_imported_ct) = anchors_cache
    ...
    assert!(
        matches!(anchor_info, InitAnchorSource::FromCache(anchor) if anchor.id == 12)
    );
    let anchors_info = filter_imported(anchors_info);
    tracing::debug!(
        "imported anchors on init (count_above_last = {}): {:?}",
...
fn test_get_anchors_processing_info() {
    ...
    let mut mc_data = McData {
        ...
        shards: vec![
            (ShardIdent::new_full(0), ShardDescriptionShort { seqno : 0,
            ext_processed_to_anchor_id : 0, top_sc_block_updated : false, end_lt : 0,
            root_hash : Default::default(), file_hash : Default::default(), })
        ],
        global_id: 0,
    ...
    assert_eq!(
        anchors_proc_info.processed_to_anchor_id, prev_processed_upto_externals
        .processed_to.0,
    );
    assert_eq!(
        anchors_proc_info.processed_to_msgs_offset, prev_processed_upto_externals
        .processed_to.1,
    );
    assert_eq!(anchors_proc_info.last_imported_chain_time, prev_gen_chain_time,);
    ...
    assert_eq!(
        anchors_proc_info.processed_to_msgs_offset, min_externals_processed_to.1,
    );
    assert_eq!(anchors_proc_info.last_imported_chain_time, mc_data.gen_chain_time,);
    assert_eq!(anchors_proc_info.last_imported_in_block_id, mc_data.block_id,);
}
