...
impl<MQ, QI, MP, ST> CollatorProcessorStdImpl<MQ, QI, MP, ST> {
    pub(super) async fn finalize_block(
    ...
    ) -> Result<(BlockCandidate, Arc<ShardStateStuff>)> {
        ...
        #[cfg(feature = "venom")]
        {
            //TODO: should support venom?
            new_state.gen_utime_ms = info.gen_utime_ms;
        }
        new_state.gen_lt = new_block_info.end_lt;
        new_state.min_ref_mc_seqno = new_block_info.min_ref_mc_seqno;
        new_state.set_out_msg_queue_info(&out_msg_queue_info)?;
        new_state.externals_processed_upto = collation_data.externals_processed_upto.clone();

        //TODO: should set when split/merge logic implemented
        new_state.before_split = new_block_info.before_split;
        new_state.overload_history = 0;
        new_state.underload_history = 0;

        new_state.total_balance = value_flow.to_next_block.clone();
        ...
        new_state.custom = mc_state_extra.as_ref().map(Lazy::new).transpose()?;
        #[cfg(feature = "venom")]
        {
            //TODO: should support venom?
            new_state.shard_block_refs = None;
        }
        ...
        let new_state_stuff = Arc::new(new_state_stuff);
    ...
    fn create_mc_state_extra(
        ...
        let (config, is_key_block) = if let Some(new_config) = new_config_opt {
            ...
            let is_key_block =
                new_config.important_config_parameters_changed(prev_config, false)?;
