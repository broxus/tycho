...
impl Collator for AsyncQueuedDispatcher<CollatorStdImpl> {
    async fn enqueue_stop(&self) -> Result<()> {
        ...
        let cancel_token = self.cancel_token().clone();
        {
            __guard.end_section(line!());
            let __result = self
                .enqueue_task(method_to_queued_async_closure!(stop_collator, cancel_token))
                .await;
            __guard.start_section(line!());
            __result
        }
...
impl CollatorStdImpl {
    ...
    async fn check_and_import_init_anchors(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(check_and_import_init_anchors)
            ),
            file!(),
            line!(),
        );
    ...
    async fn resume_collation(
        ...
        if self.shard_id.is_masterchain() {
            {
                __guard.end_section(line!());
                let __result = self.try_collate_next_master_block_impl(working_state).await;
                __guard.start_section(line!());
                __result
            }
        } else {
            {
                __guard.end_section(line!());
                let __result = self.try_collate_next_shard_block_impl(working_state).await;
                __guard.start_section(line!());
                __result
            }
        }
    ...
    async fn init_working_state(
        ...
        let (prev_states, prev_queue_diff_hashes) = {
            __guard.end_section(line!());
            let __result = Self::load_states_and_diffs(state_node_adapter, prev_blocks_ids).await;
            __guard.start_section(line!());
            __result
        }?;
                ...
                GetNewShardStateStuff::ReloadFromStorage(store_new_state_task) => {
                    ...
                    let load_task = JoinTask::new({
                        async move {
                            ...
                            {
                                __guard.end_section(line!());
                                let __result = state_node_adapter.load_state(&block_id).await;
                                __guard.start_section(line!());
                                __result
                            }
                        }
    ...
    async fn load_states_and_diffs(
        ...
        let (prev_states, prev_hash) = {
            __guard.end_section(line!());
            let __result = futures_util::future::join(load_state_fut, prev_hashes_fut).await;
            __guard.start_section(line!());
            __result
        };
    ...
    async fn load_prev_states(
        ...
        for prev_block_id in prev_blocks_ids {
            let state = {
                __guard.end_section(line!());
                let __result = state_node_adapter.load_state(prev_block_id).await;
                __guard.start_section(line!());
                __result
            }?;
            tracing::debug!(
        ...
        let mut next_anchor = match last_anchor {
            ...
            None => {
                let GetAnchorResult::Exist(anchor) = {
                    __guard.end_section(line!());
                    let __result = mpool_adapter.get_anchor_by_id(processed_to_anchor_id).await;
                    __guard.start_section(line!());
                    __result
                }?
                else {
    ...
    fn check_has_unprocessed_messages(&mut self, working_state: &mut WorkingState) -> Result<bool> {
        ...
        let FinalizedMessagesReader {
            mut reader_state, ..
        } = messages_reader.finalize(0, &Default::default())?;
    ...
    async fn wait_state_and_try_collate_wrapper(&mut self) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(wait_state_and_try_collate_wrapper)
            ),
            file!(),
            line!(),
        );
    ...
    async fn wait_state_and_do_collate_wrapper(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(wait_state_and_do_collate_wrapper)
            ),
            file!(),
            line!(),
        );
    ...
    async fn try_collate_next_master_block_impl(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(try_collate_next_master_block_impl)
            ),
            file!(),
            line!(),
        );
    ...
    async fn try_collate_next_shard_block_impl(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(try_collate_next_shard_block_impl)
            ),
            file!(),
            line!(),
        );
        ...
        match try_collate_check {
            ...
            | TryCollateCheck::ForceEmptyShardBlock => {
                ...
                drop(histogram);
                {
                    __guard.end_section(line!());
                    let __result = self
                        .do_collate(working_state, None, force_next_mc_block)
                        .await;
                    __guard.start_section(line!());
                    __result
                }?;
