...
use super::types::{
    AnchorInfo, AnchorsCache, BlockCollationData, BlockCollationDataBuilder,
    BlockSerializerCache, CollationResult, ExecuteResult, FinalResult,
    FinalizeBlockResult, FinalizeCollationResult, FinalizeMessagesReaderResult, PrevData,
    WorkingState,
};
...
use crate::internal_queue::types::{
    Bound, DiffZone, EnqueuedMessage, QueueShardBoundedRange,
};
...
impl CollatorStdImpl {
    ...
    pub(super) async fn do_collate(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(do_collate)),
            file!(),
            84u32,
        );
        ...
        let force_next_mc_block = force_next_mc_block;
        let labels: [(&str, String); 1] = [
            ("workchain", self.shard_id.workchain().to_string()),
        ];
        let total_collation_histogram = HistogramGuard::begin_with_labels(
        ...
        tracing::info!(
            ...
            mc_data.block_id.as_short_id(), DisplayBlockIdsIntoIter(prev_shard_data
            .blocks_ids()), top_shard_blocks_info.as_ref().map(| v |
            DisplayBlockIdsIter(v.iter().map(| i | & i.block_id))),
        );
        let Some(AnchorInfo { ct: last_imported_chain_time, author, .. }) = self
            .anchors_cache
            .remove_last_imported_above(next_chain_time) else {
            bail!(
            ...
            && mc_data.block_id.seqno > 0
        {
            if next_block_id_short.is_masterchain() {
            ...
            } else {
                {
                    ...
                    let __result = self
                        .compute_part_stat_ranges(&mc_data, &next_block_id_short)
                        .await;
        ...
        let collation_data = self
            .create_collation_data(
                next_block_id_short,
                next_chain_time,
                created_by,
                &mc_data,
        ...
        let mut do_collate_res = None;
        {
            ...
            let __result = tokio::select! {
                res = do_collate_fut => { do_collate_res = Some(res); }, _ = async move {
                collation_cancelled. await; tracing::info!(target :
                tracing_targets::COLLATOR,
                "collation was cancelled by manager on do_collate",);
                collation_is_cancelled.cancel(); std::future::pending::< () > (). await;
                } => {}
            };
        ...
        let last_read_to_anchor_chain_time = reader_state
            .externals
        ...
        let FinalizeCollationResult { handle_block_candidate_elapsed } = {
        ...
        if let Some(wu_tuner_event_sender) = &self.wu_tuner_event_sender {
            if let Err(err) = {
                ...
                let __result = wu_tuner_event_sender
                    .send(WuEvent {
        ...
        let collation_mngmnt_overhead = elapsed_from_prev_block
        ...
        let diff_time = now_millis() as i64
            - last_read_to_anchor_chain_time.unwrap_or(next_chain_time) as i64;
    ...
    fn run(
        ...
        let collation_is_cancelled = state.collation_is_cancelled.clone();
        let histogram_prepare = HistogramGuard::begin_with_labels(
        ...
        let prepare_phase = Phase::<
            PrepareState,
        >::new(mq_adapter.clone(), reader_state, anchors_cache, state);
        ...
        let (
            ...
            .finalize_messages_reader(messages_reader, mq_adapter.clone())?;
        let all_shards_processed_to = build_all_shards_processed_to_by_partitions(
            ...
            mc_data.processed_upto.get_internals_processed_to_by_partitions(),
        ...
        if collation_is_cancelled.check() {
            return Err(CollatorError::Cancelled(CollationCancelReason::ExternalCancel));
        }
        let diff_tail_len = mq_adapter
            .get_diffs_tail_len(&shard_id, &min_processed_to.unwrap_or_default()) + 1;
        let span = tracing::Span::current();
        let (finalize_phase_result, update_queue_task_result) = rayon::join(
            || {
                let _span = span.enter();
                finalize_phase
                    .finalize_block(FinalizeBlockContext {
                        collation_session,
                        wu_used_from_last_anchor,
                        usage_tree,
                        queue_diff,
        ...
        finalized.finalize_wu.append_elapsed_timings(&finalized.finalize_metrics);
        Self::log_finalize_timings(&finalized.finalize_metrics);
        assert_eq!(
            finalized.collation_data.int_enqueue_count, finalized.collation_data
            .inserted_new_msgs_count - finalized.collation_data.execute_count_new_int
        );
    ...
    fn calc_start_lt(
        ...
        tracing::debug!(
            target : tracing_targets::COLLATOR, "start_lt set to {}", start_lt
        );
        Ok(start_lt)
    }
    fn init_value_flow(
        ...
        if block_id.is_masterchain() {
            ...
            value_flow.recovered = Self::compute_recovered_amount(
                config,
                created,
                &shard.fees,
                total_validator_fees,
            )?;
            value_flow.minted = Self::compute_minted_amount(
                block_id.seqno,
                config,
                old_global_balance,
            )?;
        ...
        value_flow.fees_collected.try_add_assign(&value_flow.created)?;
    ...
    fn compute_recovered_amount(
        ...
        match config.get_fee_collector_address() {
            Ok(_addr) => {
                ...
                let threshold = apply_price_factor(
                    RECOVERY_BASE_THRESHOLD,
                    gas_price_factor,
                );
    ...
    fn import_new_shard_top_blocks_for_masterchain(
        ...
        for (shard_id, top_block_descr) in top_shard_blocks_info_map {
            ...
            let mut new_shard_descr = Box::new(
                ShardDescription::from_block_info(
                    block_id,
                    &block_info,
                    processed_to_anchor_id,
                    &value_flow,
            ...
            if new_shard_descr.gen_utime > collation_data_builder.gen_utime {
                bail!(
                    ...
                    block_id.as_short_id(), new_shard_descr.gen_utime,
            ...
            let top_sc_block_updated = match collation_data_builder
                .shards_mut()?
                .entry(shard_id)
            {
                ...
                hash_map::Entry::Occupied(mut entry) => {
                    ...
                    let top_sc_block_updated = prev_shard_descr.seqno
                        != new_shard_descr.seqno;
                    new_shard_descr.top_sc_block_updated = top_sc_block_updated;
                    entry.insert(new_shard_descr);
                    top_sc_block_updated
                }
        ...
        collation_data_builder.mc_shards_processed_to_by_partitions = shards_processed_to_by_partitions;
        Ok(())
    }
    fn update_stats(&mut self, collation_data: &BlockCollationData) {
        ...
        self.stats.total_execute_msgs_time_mc
            += collation_data.total_execute_msgs_time_mc;
        self.stats.avg_exec_msgs_per_1000_ms = (self.stats.total_execute_count_all
            as u128 * 1_000 * 1_000)
    ...
    fn create_collation_data(
        ...
        let rand_seed = HashBytes(
            tl_proto::hash(RandSeed {
        ...
        tracing::trace!(
            target : tracing_targets::COLLATOR, "rand_seed from chain time: {}",
            rand_seed
        );
        let is_masterchain = self.shard_id.is_masterchain();
        if !is_masterchain {
            self.shard_blocks_count_from_last_anchor = self
                .shard_blocks_count_from_last_anchor
                .saturating_add(1);
        }
        let block_limits = mc_data.config.get_block_limits(is_masterchain)?;
        tracing::debug!(
            target : tracing_targets::COLLATOR, "Block limits: {:?}", block_limits
        );
        ...
        if is_masterchain {
            let shards = prev_shard_data
                .observable_states()[0]
                .shards()?
                .iter()
                .map(|entry| {
                    entry.map(|(shard_ident, descr)| (shard_ident, Box::new(descr)))
                })
            ...
            collation_data_builder.mc_shards_processed_to_by_partitions = mc_data
                .shards_processed_to_by_partitions
                .clone();
        }
        ...
        let mut collation_data = Box::new(
            collation_data_builder.build(start_lt, block_limits),
        );
        let prev_total_balance = &prev_shard_data
            .observable_accounts()
            .root_extra()
            .balance;
    ...
    async fn finalize_collation(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(finalize_collation)),
            file!(),
            931u32,
        );
        let ctx = ctx;
        ...
        let store_new_state_task = JoinTask::new({
            ...
            async move {
                ...
                {
                    __guard.end_section(959u32);
                    let __result = adapter
                        .store_state_root(&block_id, meta, new_state_root, hint)
                        .await;
                    __guard.start_section(959u32);
                    __result
                }
            }
        });
        let handle_block_candidate_elapsed;
        {
            ...
            let new_queue_diff_hash = *finalized
                .block_candidate
                .queue_diff_aug
                .diff_hash();
            ...
            let force_next_mc_block = if !self.shard_id.is_masterchain()
                && !has_unprocessed_messages
            {
            ...
            };
            {
                ...
                let __result = self
                    ...
                    .on_block_candidate(BlockCollationResult {
                        ...
                        collation_config: collation_config.clone(),
                        force_next_mc_block,
                        has_processed_externals: finalized
                            .collation_data
                            .execute_count_ext > 0,
                    })
                    .await;
                __guard.start_section(996u32);
                __result
            }?;
    ...
    fn report_collation_metrics(&self, collation_data: &BlockCollationData) {
        ...
        let BlockParamLimits { soft_limit, hard_limit, .. } = lt_delta;
        metrics::gauge!("tycho_do_collate_total_items_current", & labels)
            .set(collation_data.block_limit.total_items as f64);
        metrics::gauge!("tycho_do_collate_total_items_soft_limit", & labels)
            .set(*soft_limit as f64);
        metrics::gauge!("tycho_do_collate_total_items_hard_limit", & labels)
            .set(*hard_limit as f64);
        let bytes = &collation_data.block_limit.block_limits.bytes;
        let BlockParamLimits { soft_limit, hard_limit, .. } = bytes;
    ...
    fn log_block_and_stats(
        ...
        tracing::info!(
            ...
            diffs_tail_len: {}",
            finalize_result.block_candidate.block.id(), collation_data.start_lt,
            collation_data.next_lt, collation_data.execute_count_all, collation_data
            .execute_count_ext, collation_data.ext_msgs_error_count, collation_data
            .execute_count_int, collation_data.execute_count_new_int, collation_data
            .int_enqueue_count, collation_data.int_dequeue_count, collation_data
            .new_msgs_created_count, collation_data.inserted_new_msgs_count,
            queue_diff_messages_count, collation_data.in_msgs.len(), collation_data
            .out_msgs.len(), execute_result.msgs_reader_metrics.read_ext_msgs_count,
            execute_result.msgs_reader_metrics.read_existing_msgs_count, execute_result
            .msgs_reader_metrics.read_new_msgs_count, final_result
            .has_unprocessed_messages, collation_data.total_execute_msgs_time_mc,
            collation_data.diff_tail_len,
        );
        tracing::debug!(
            target : tracing_targets::COLLATOR, prepare = % format_duration(final_result
            .prepare_elapsed), execute_tick = % format_duration(execute_result
            .execute_metrics.execute_tick_elapsed), execute_special = %
            format_duration(execute_result.execute_metrics.execute_special_elapsed),
            execute_incoming_msgs = % format_duration(execute_result.execute_metrics
            .execute_incoming_msgs_elapsed), execute_tock = %
            format_duration(execute_result.execute_metrics.execute_tock_elapsed),
            fill_msgs_total = % format_duration(execute_result.prepare_msg_groups_wu
            .total_elapsed), init_iterator = % format_duration(execute_result
            .msgs_reader_metrics.init_iterator_timer.total_elapsed), read_ext = %
            format_duration(execute_result.msgs_reader_metrics.read_ext_messages_timer
            .total_elapsed), read_existing = % format_duration(execute_result
            .msgs_reader_metrics.read_existing_messages_timer.total_elapsed), read_new =
            % format_duration(execute_result.msgs_reader_metrics.read_new_messages_timer
            .total_elapsed), add_to_groups = % format_duration(execute_result
            .msgs_reader_metrics.add_to_message_groups_timer.total_elapsed),
            exec_msgs_total = % format_duration(execute_result.execute_wu
            .execute_groups_vm_only_elapsed), process_txs_total = %
            format_duration(execute_result.execute_wu.process_txs_elapsed),
            create_queue_diff = % format_duration(finalize_result.finalize_metrics
            .create_queue_diff_elapsed), apply_queue_diff = %
            format_duration(finalize_result.finalize_metrics.apply_queue_diff_elapsed),
            finalize_block = % format_duration(finalize_result.finalize_metrics
            .finalize_block_elapsed), finalize_total = % format_duration(finalize_result
            .finalize_metrics.total_timer.total_elapsed), "collation timings"
        );
    }
    fn log_finalize_timings(finalize_metrics: &FinalizeMetrics) {
        tracing::debug!(
            target : tracing_targets::COLLATOR, total_elapsed = %
            format_duration(finalize_metrics.total_timer.total_elapsed),
            create_queue_diff = % format_duration(finalize_metrics
            .create_queue_diff_elapsed), apply_queue_diff = %
            format_duration(finalize_metrics.apply_queue_diff_elapsed), finalize_block =
            % format_duration(finalize_metrics.finalize_block_elapsed),
            parallel_build_accounts_and_msgs = % format_duration(finalize_metrics
            .build_accounts_and_messages_in_parallel_elased), only_build_accounts = %
            format_duration(finalize_metrics.build_accounts_elapsed),
            incl_update_shard_accounts = % format_duration(finalize_metrics
            .update_shard_accounts_elapsed), incl_build_accounts_blocks = %
            format_duration(finalize_metrics.build_accounts_blocks_elapsed),
            only_build_in_msgs = % format_duration(finalize_metrics
            .build_in_msgs_elapsed), only_build_out_msgs = %
            format_duration(finalize_metrics.build_out_msgs_elapsed),
            build_mc_state_extra = % format_duration(finalize_metrics
            .build_mc_state_extra_elapsed), build_state_update = %
            format_duration(finalize_metrics.build_state_update_elapsed), build_block = %
            format_duration(finalize_metrics.build_block_elapsed),
    ...
    fn log_final_timings(
        ...
        tracing::debug!(
            target : tracing_targets::COLLATOR, block_time_diff = block_time_diff,
            from_prev_block = % format_duration(elapsed_from_prev_block), overhead = %
            format_duration(collation_mngmnt_overhead), total = %
            format_duration(collation_total_elapsed), handle_block_candidate = %
            format_duration(handle_block_candidate_elapsed), "total collation timings"
        );
    }
    ...
    async fn compute_part_stat_ranges(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(compute_part_stat_ranges)),
            file!(),
            1205u32,
        );
        let mc_data = mc_data;
        let block_id_short = block_id_short;
        let Some(prev_mc_data) = &mc_data.prev_mc_data else {
            {
                __guard.end_section(1207u32);
                return Ok(None);
            };
        };
        let prev_shards: FastHashMap<_, _> = prev_mc_data
            .shards
            .iter()
            .cloned()
            .collect();
        ...
        {
            __guard.end_section(1222u32);
            let __result = self.build_ranges(&mc_data.block_id, shard_pairs).await;
            __guard.start_section(1222u32);
            __result
        }
    }
    ...
    async fn mc_compute_part_stat_ranges(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(mc_compute_part_stat_ranges)),
            file!(),
            1232u32,
        );
        let mc_data = mc_data;
        ...
        {
            __guard.end_section(1246u32);
            let __result = self.build_ranges(&mc_data.block_id, shard_pairs).await;
            __guard.start_section(1246u32);
            __result
        }
    }
    async fn build_ranges(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(build_ranges)),
            file!(),
            1253u32,
        );
        let master_block_id = master_block_id;
        let shard_pairs = shard_pairs;
        let Some(master_max_msg) = {
            __guard.end_section(1256u32);
            let __result = self.get_diff_max_message(master_block_id).await;
            __guard.start_section(1256u32);
            __result
        }
            .context("loading diff for mc block")? else {
            {
                __guard.end_section(1259u32);
                return Ok(None);
            };
        };
        let mut ranges = vec![
            QueueShardBoundedRange { shard_ident : master_block_id.shard, from :
            Bound::Included(master_max_msg), to : Bound::Included(master_max_msg), }
        ];
        for (shard, prev_id, curr_id) in shard_pairs {
            __guard.checkpoint(1269u32);
            if prev_id.seqno == 0 {
                {
                    __guard.end_section(1271u32);
                    return Ok(None);
                };
            }
            let (first_msg, last_msg) = {
                __guard.end_section(1274u32);
                let __result = tokio::try_join!(
                    self.get_diff_max_message(& prev_id), self.get_diff_max_message(&
                    curr_id)
                );
                __guard.start_section(1274u32);
                __result
            }
    ...
    async fn get_diff_max_message(
        &self,
        block_id: &BlockId,
    ) -> Result<Option<QueueKey>> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_diff_max_message)),
            file!(),
            1294u32,
        );
            ...
            .get_diff_info(&block_id.shard, block_id.seqno, DiffZone::Both)?
        {
            ...
            let __result = self.state_node_adapter.load_diff(block_id).await;
            ...
            tracing::warn!(
                target : tracing_targets::COLLATOR,
                "diff not found by block: {block_id:?}"
            );
            Ok(None)
        }
    }
