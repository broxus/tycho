...
impl CollatorStdImpl {
    ...
    #[tracing::instrument(
        ...
        fields(
            ...
            ct = self.anchors_cache.last_imported_anchor(

            ).map(|a|a.ct).unwrap_or_default(),
        )
    )]
    pub(super) async fn do_collate(
        ...
        let is_first_block_after_prev_master =
            is_first_block_after_prev_master(prev_shard_data.blocks_ids()[0], &mc_data.shards);
    ...
    async fn mc_compute_part_stat_ranges(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(mc_compute_part_stat_ranges)
            ),
            file!(),
            line!(),
        );
