...
impl CollatorStdImpl {
    ...
    async fn mc_compute_part_stat_ranges(
        ...
        {
            __guard.end_section(1241u32);
            let __result = self.build_ranges(&mc_data.block_id, shard_pairs).await;
            __guard.start_section(1241u32);
            __result
        }
    }
    async fn build_ranges(
        ...
        for (shard, prev_id, curr_id) in shard_pairs {
            __guard.checkpoint(1264u32);
            if prev_id.seqno == 0 {
                {
                    __guard.end_section(1266u32);
                    return Ok(None);
                };
            }
            let (first_msg, last_msg) = {
                __guard.end_section(1269u32);
                let __result = tokio::try_join!(
                    self.get_diff_max_message(& prev_id), self.get_diff_max_message(&
                    curr_id)
                );
                __guard.start_section(1269u32);
                __result
            }
