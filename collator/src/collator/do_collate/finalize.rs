    ...
    fn create_mc_state_extra(
        ...
        if is_key_block {
            ...
            let next_session_start_round = if consensus_info != prev_state_extra.consensus_info {
                ...
                let mempool_last_round_to_create = prev_processed_to_anchor
                    + prev_consensus_config.max_consensus_lag_rounds as u32;
                let cur_session_last_round =
                    if consensus_info.vset_switch_round > consensus_info.genesis_info.start_round {
                        ...
                        let full_history_round = consensus_info.vset_switch_round.min(
                            // ensure prev session has enough length too;
                            // MIN with current includes the case when prev was short after Genesis
                            consensus_info.prev_vset_switch_round
                                + prev_consensus_config.max_total_rounds(),
                        ) + prev_consensus_config.max_total_rounds();
                ...
                || collation_config.shuffle_mc_validators != prev_shuffle_mc_validators
            {
                ...
                // if PPtA (TKA) is before current vset_switch - cannot apply outdated current
                // schedule, otherwise resulting prev_* session will be too short: then just
                // overwrite outdated current (skip it) preserving prev_* attributes as they were
                if consensus_info.vset_switch_round <= prev_processed_to_anchor {
