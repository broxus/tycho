...
impl Phase<FinalizeState> {
    ...
    fn create_mc_state_extra(
        ...
        if let Some(mp_cfg_override) = &collation_data.mempool_config_override {
            if mp_cfg_override.start_round >= consensus_info.genesis_info.start_round
                && mp_cfg_override.genesis_time_millis > consensus_info.genesis_info.millis
            {
                consensus_info.genesis_info.start_round = mp_cfg_override.start_round;
                consensus_info.genesis_info.millis = mp_cfg_override.genesis_time_millis;
