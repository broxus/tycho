...
impl InternalQueueTransaction {
    ...
    pub fn set_last_committed_mc_block_id(&mut self, mc_block_id: &BlockId) -> Result<()> {
        ...
        // // get the old pointer if it exists
        // let old_last_mc_block_id = self
        //     .db
        //     .rocksdb()
        //     .get_cf(&cf, INT_QUEUE_LAST_COMMITTED_MC_BLOCK_ID_KEY)?
        //     .map(|bytes| BlockId::from_slice(&bytes));

        // // do not update if old pointer if above a new one
        // if let Some(old) = old_last_mc_block_id
        //     && mc_block_id.seqno < old.seqno
        // {
        //     return Ok(());
        // }
