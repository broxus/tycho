    ...
    pub async fn import_from_file(
        ...
        let span = tracing::Span::current();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                ...
                let mut reader =
                    QueueStateReader::begin_from_mapped(mapped.as_slice(), &top_update)?;
                ...
                while let Some(mut part) = reader.read_next_queue_diff()? {
                    ...
                    while let Some(cell) = part.read_next_message()? {
                        ...
                        let partition =
                            get_partition(&queue_diff.router_partitions_dst, &dest_addr)
                                .or_else(|| {
                                    get_partition(&queue_diff.router_partitions_src, &src_addr)
                                })
                                .unwrap_or_default();
