...
use tycho_types::models::{
    BlockId, IntAddr, Message, MsgInfo, OutMsgQueueUpdates, ShardIdent,
};
    ...
    pub async fn import_from_file(
        ...
        let span = tracing::Span::current();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    let commit_pointers_cf = this
                        .db
                        .internal_message_commit_pointer
                        .cf();
                    let mut first_diff_read = false;
                    let mut batch = weedb::rocksdb::WriteBatch::default();
                    let mut buffer = Vec::new();
                    let mut statistics: FastHashMap<
                        QueuePartitionIdx,
                        FastHashMap<RouterAddr, u64>,
                    > = FastHashMap::default();
                    while let Some(mut part) = reader.read_next_queue_diff()? {
                        ...
                        while let Some(cell) = part.read_next_message()? {
                            ...
                            let dest_shard = ShardIdent::new_full(
                                dest_addr.workchain as i32,
                            );
                            ...
                            let partition = get_partition(
                                    &queue_diff.router_partitions_dst,
                                    &dest_addr,
                                )
                                .or_else(|| get_partition(
                                    &queue_diff.router_partitions_src,
                                    &src_addr,
                                ))
                                .unwrap_or_default();
                            ...
                            BocHeader::<ahash::RandomState>::with_root(cell.as_ref())
                                .encode(&mut buffer);
                            batch.put_cf(&messages_cf, key.to_vec(), &buffer);
                            let partition_stats = statistics
                                .entry(partition)
                                .or_default();
                        ...
                        let diff_info = DiffInfo {
                            ...
                            router_partitions_src: queue_diff
                                .router_partitions_src
                                .clone(),
                            router_partitions_dst: queue_diff
