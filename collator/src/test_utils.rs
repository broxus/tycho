...
pub async fn prepare_test_storage() -> anyhow::Result<(CoreStorage, tempfile::TempDir)> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(prepare_test_storage)),
        file!(),
        35u32,
    );
    let (ctx, tmp_dir) = {
    ...
    storage.node_state().store_last_mc_block_id(&master_block_id);
    for entry in master_zerostate.load_custom()?.unwrap().shards.iter() {
        __guard.checkpoint(102u32);
...
pub async fn create_test_queue_adapter<V: InternalMessageValue>() -> Result<
    (Arc<dyn MessageQueueAdapter<V>>, tempfile::TempDir),
> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(create_test_queue_adapter)),
        file!(),
        157u32,
    );
    let (ctx, tmp_dir) = {
