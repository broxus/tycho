...
async fn test_collation_process_on_stubs() {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_collation_process_on_stubs)),
        file!(),
        61u32,
    );
    ...
    {
        __guard.end_section(144u32);
        let __result = tokio::select! {
            _ = strider_handle => { println!(); println!("block_strider finished"); }, _
            = tokio::signal::ctrl_c() => { println!();
            println!("Ctrl-C received, shutting down the test"); }, _ =
            tokio::time::sleep(tokio::time::Duration::from_secs(20)) => { println!();
            println!("Test timeout elapsed"); }
        };
        __guard.start_section(144u32);
        __result
    }
}
