...
use futures_util::future::{self, BoxFuture, Future};
use tokio::sync::mpsc;
...
impl Validator for ValidatorStub {
    ...
    async fn validate(
        ...
        if block_id.seqno >= self.block_seqno {
...
async fn test_collation_process_on_dump() {
    ...
    let needed_block_seqno = 3; // TODO: Fill with correct block seqno
    ...
    let validator = ValidatorStub {
        engine_stop_tx,
        block_seqno: needed_block_seqno,
    };
    ...
    let block_receiver = BlockReceiver { rx: engine_stop_rx };
    ...
    tokio::select! {
        ...
        _ = block_receiver => {
            tracing::info!("Stopped, found block seqno: {}", needed_block_seqno);
        }
    }
}
...
impl Future for BlockReceiver {
    type Output = ();

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
        let BlockReceiver { rx } = &mut *self;
