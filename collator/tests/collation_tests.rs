...
struct TestCollatorStateSubscriber {
...
impl StateSubscriber for TestCollatorStateSubscriber {
...
async fn start_collation(
    ...
    tracing::info!("Block id to handle: {}", block_id_to_handle);

    let collator = TestCollatorStateSubscriber {
    ...
    Ok((block_strider, manager, engine_stop_rx))
}
...
async fn test_collation_process_on_dump() {
    ...
    let (block_strider, manager, mut engine_stop_rx) =
        start_collation(&ctx, dumped_anchors, block_id_to_handle)
            .await
            .unwrap();
...
async fn test_one_shard_block_per_master_block() {
    ...
    let expected_master_block_2 = BlockIdShort {
        shard: ShardIdent::MASTERCHAIN,
        seqno: ctx.mc_block_id.seqno + 2,
    };

    let expected_sequence = vec![
        expected_shard_block_1,
        expected_master_block_1,
        expected_master_block_2,
    ];

    let (block_strider, manager, mut engine_stop_rx) =
        start_collation(&ctx, dumped_anchors, expected_master_block_2)
            .await
            .unwrap();

    scopeguard::defer! {
        // time to finish CAS, Queues tasks
        std::thread::sleep(tokio::time::Duration::from_secs(2));
        tracing::trace!("Shutting down the test");
    }
