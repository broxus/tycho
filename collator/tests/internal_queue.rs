...
use tycho_block_util::queue::{
    QueueDiff, QueueDiffStuff, QueueKey, QueuePartitionIdx, RouterAddr,
};
...
use tycho_collator::internal_queue::state::storage::{
    QueueStateImplFactory, QueueStateStdImpl,
};
...
use tycho_types::models::{
    AccountStatus, BlockId, BlockIdShort, ComputePhase, ComputePhaseSkipReason,
    CurrencyCollection, HashUpdate, IntAddr, IntMsgInfo, IntermediateAddr, MsgEnvelope,
    MsgInfo, OrdinaryTxInfo, OutMsg, OutMsgDescr, OutMsgNew, OutMsgQueueUpdates,
    OwnedMessage, ShardIdent, SkippedComputePhase, StdAddr, Transaction, TxInfo,
};
...
fn create_stored_object(
    key: u64,
    dest_addr: RouterAddr,
) -> anyhow::Result<Arc<StoredObject>> {
...
async fn test_queue() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_queue)),
        file!(),
        235u32,
    );
    let (ctx, _tmp_dir) = {
    ...
    let dest_1_low_priority = RouterAddr::from(
        StdAddr::new(-1, HashBytes::from([1; 32])),
    );
    let dest_2_low_priority = RouterAddr::from(
        StdAddr::new(-1, HashBytes::from([2; 32])),
    );
    for i in 1..=10000 {
        __guard.checkpoint(277u32);
        let stored_object = create_stored_object(i, dest_1_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    for i in 10001..=15000 {
        __guard.checkpoint(284u32);
        let stored_object = create_stored_object(i, dest_2_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    let dest_3_normal_priority = RouterAddr::from(
        StdAddr::new(0, HashBytes::from([3; 32])),
    );
    for i in 15001..=16000 {
        __guard.checkpoint(294u32);
        let stored_object = create_stored_object(i, dest_3_normal_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
    }
    assert_eq!(
        partition_router.get_partition(Some(& Default::default()), & dest_1_low_priority
        .to_int_addr()), QueuePartitionIdx(1)
    );
    assert_eq!(
        partition_router.get_partition(Some(& Default::default()), & dest_2_low_priority
        .to_int_addr()), QueuePartitionIdx(1)
    );
    assert_eq!(
        partition_router.get_partition(Some(& Default::default()), &
        dest_3_normal_priority.to_int_addr()), QueuePartitionIdx::default()
    );
    ...
    let diff_statistics = DiffStatistics::from_diff(
        &diff_with_messages,
        block1.shard,
        diff_with_messages.min_message().cloned().unwrap_or_default(),
        diff_with_messages.max_message().cloned().unwrap_or_default(),
    );
    ...
    diff_statistics
        ...
        .for_each(|(_partition, statistics)| {
            ...
            let addr1_count = statistics
                .get(&dest_1_low_priority.to_int_addr())
                .unwrap();
            assert_eq!(* addr1_count, 10000);
            let addr2_count = statistics
                .get(&dest_2_low_priority.to_int_addr())
                .unwrap();
    ...
    for i in 20001..=30000 {
        __guard.checkpoint(400u32);
        let stored_object = create_stored_object(i, dest_1_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    for i in 30001..=35000 {
        __guard.checkpoint(407u32);
        let stored_object = create_stored_object(i, dest_2_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    ...
    for i in 35001..=36000 {
        __guard.checkpoint(417u32);
        let stored_object = create_stored_object(i, dest_3_normal_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
    }
    assert_eq!(
        partition_router.get_partition(Some(& IntAddr::default()), & dest_1_low_priority
        .to_int_addr()), QueuePartitionIdx(1)
    );
    assert_eq!(
        partition_router.get_partition(Some(& IntAddr::default()), & dest_2_low_priority
        .to_int_addr()), QueuePartitionIdx(1)
    );
    assert_eq!(
        partition_router.get_partition(Some(& IntAddr::default()), &
        dest_3_normal_priority.to_int_addr()), QueuePartitionIdx::default()
    );
    let diff_with_messages = QueueDiffWithMessages {
        messages: diff.messages,
    ...
    let mc_diff_statistics = DiffStatistics::from_diff(
        ...
        mc_block.shard,
        mc_diff_with_messages.min_message().cloned().unwrap_or_default(),
        mc_diff_with_messages.max_message().cloned().unwrap_or_default(),
    );
    queue
        .apply_diff(
            mc_diff_with_messages,
    ...
    let iterators = queue
        .iterator(QueuePartitionIdx(1), &ranges, ShardIdent::MASTERCHAIN)?;
    let mut iterator_manager = StatesIteratorsManager::new(iterators);
    let mut read_count = 0;
    while iterator_manager.next()?.is_some() {
        __guard.checkpoint(593u32);
        read_count += 1;
    }
    assert_eq!(read_count, 5000);
    let iterators = queue
        .iterator(QueuePartitionIdx::default(), &ranges, ShardIdent::new_full(0))?;
    let mut iterator_manager = StatesIteratorsManager::new(iterators);
    let mut read_count = 0;
    while iterator_manager.next()?.is_some() {
        __guard.checkpoint(605u32);
        read_count += 1;
    }
    ...
    let mc2_diff_statistics = DiffStatistics::from_diff(
        ...
        mc_block2.shard,
        mc2_diff_with_messages.min_message().cloned().unwrap_or_default(),
        mc2_diff_with_messages.max_message().cloned().unwrap_or_default(),
    );
    queue
        .apply_diff(
            mc2_diff_with_messages,
...
async fn test_iteration_from_two_shards() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_iteration_from_two_shards)),
        file!(),
        700u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(701u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(701u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
    ...
    let dest_2_low_normal_priority = RouterAddr::from(
        StdAddr::new(-1, HashBytes::from([2; 32])),
    );
    for i in 1..=10000 {
        __guard.checkpoint(725u32);
        let stored_object = create_stored_object(i, dest_1_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    for i in 20001..=30000 {
        __guard.checkpoint(732u32);
        let stored_object = create_stored_object(i, dest_1_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    for i in 100000..=150000 {
        __guard.checkpoint(739u32);
    ...
    for i in 10001..=20000 {
        __guard.checkpoint(786u32);
        let stored_object = create_stored_object(i, dest_1_low_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
        partition_router.insert_dst(&stored_object.dest, QueuePartitionIdx(1))?;
    }
    for i in 200000..=250000 {
        __guard.checkpoint(793u32);
        let stored_object = create_stored_object(i, dest_2_low_normal_priority)?;
        diff.messages.insert(stored_object.key(), stored_object.clone());
    }
    let diff_with_messages = QueueDiffWithMessages {
    ...
    for partition in &[1, 0] {
        ...
        for range in &r {
            __guard.checkpoint(892u32);
            queue
                .load_diff_statistics(
                    QueuePartitionIdx(*partition),
                    range,
                    &mut statistics,
                )?;
        }
...
async fn test_queue_clear() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_queue_clear)),
        file!(),
        924u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(925u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(925u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
    ...
    let stored_objects = vec![
        create_stored_object(1, RouterAddr { workchain : 1, account : HashBytes::from([1;
        32]), }) ?
    ];
    for stored_object in &stored_objects {
        __guard.checkpoint(946u32);
        diff.messages.insert(stored_object.key(), stored_object.clone());
    }
    let diff_with_messages = QueueDiffWithMessages {
        messages: diff.messages,
...
fn test_router() -> anyhow::Result<()> {
    ...
    let res = partition_router
        .get_partition(Some(&IntAddr::default()), &dest_1.to_int_addr());
    assert_eq!(res, 1);
    let res = partition_router
        .get_partition(Some(&src_1.to_int_addr()), &IntAddr::default());
    assert_eq!(res, 2);
    let res = partition_router
        .get_partition(Some(&src_1.to_int_addr()), &dest_1.to_int_addr());
    assert_eq!(res, 1);
    let res = partition_router
        .get_partition(Some(&IntAddr::default()), &IntAddr::default());
    assert_eq!(res, 0);
    Ok(())
}
#[test]
fn test_queue_diff_with_messages_from_queue_diff_stuff() -> anyhow::Result<()> {
    ...
    let diff_with_messages = QueueDiffWithMessages::from_queue_diff(
        &queue_diff_stuff,
        &out_msg,
    )?;
    assert_eq!(diff_with_messages.processed_to, diff.processed_to,);
    assert_eq!(
        diff_with_messages.partition_router, PartitionRouter::with_partitions(& diff
        .router_partitions_src, & diff.router_partitions_dst),
    );
    assert_eq!(
        diff_with_messages.messages.into_keys().map(| key | key.hash).collect::< Vec < _
...
async fn test_queue_tail_and_diff_info() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_queue_tail_and_diff_info)),
        file!(),
        1229u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(1230u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(1230u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
    ...
    for stored_object in &stored_objects[..2] {
        __guard.checkpoint(1276u32);
        diff_mc1.messages.insert(stored_object.key(), stored_object.clone());
    }
    for stored_object in &stored_objects[2..] {
        __guard.checkpoint(1282u32);
        diff_mc2.messages.insert(stored_object.key(), stored_object.clone());
    }
    ...
    let diff_info_mc1 = queue
        .get_diff_info(&ShardIdent::MASTERCHAIN, block_mc1.seqno, DiffZone::Uncommitted)?
        .unwrap();
    assert_eq!(diff_info_mc1.max_message, QueueKey::min_for_lt(2));
    let diff_info_mc2 = queue
        .get_diff_info(&ShardIdent::MASTERCHAIN, block_mc2.seqno, DiffZone::Uncommitted)?
        .unwrap();
    assert_eq!(diff_info_mc2.max_message, QueueKey::min_for_lt(4));
    let res = queue
        .get_diff_info(&ShardIdent::MASTERCHAIN, block_mc1.seqno, DiffZone::Committed)?;
    assert!(res.is_none());
    let res = queue
        .get_diff_info(&ShardIdent::MASTERCHAIN, block_mc1.seqno, DiffZone::Both)?;
    assert!(res.is_some());
    ...
    let diff_info_mc2 = queue
        ...
        .unwrap();
    assert_eq!(diff_info_mc2.max_message, QueueKey::min_for_lt(4));
    let diff_len_mc = queue
        .get_diffs_tail_len(&ShardIdent::MASTERCHAIN, &end_key_mc1.next_value());
...
async fn test_version() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_version)),
        file!(),
        1441u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(1442u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(1442u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
...
async fn test_commit_wrong_sequence() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_commit_wrong_sequence)),
        file!(),
        1547u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(1548u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(1548u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
    ...
    for block in blocks {
        __guard.checkpoint(1593u32);
        let mut diff = QueueDiffWithMessages::new();
        let mut partition_router = PartitionRouter::default();
        for i in (block.seqno - 1) * 100 + 1..=100 * block.seqno {
            __guard.checkpoint(1598u32);
    ...
    let stat = statistics.get(&low_priority.to_int_addr()).cloned().unwrap_or_default();
    assert_eq!(stat, 300);
    let iterators = queue
        .iterator(QueuePartitionIdx(1), &ranges, ShardIdent::MASTERCHAIN)?;
    let mut iterator_manager = StatesIteratorsManager::new(iterators);
...
async fn prepare_data_from_prepared_persistent_state(
    ...
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(
            module_path!(), "::", stringify!(prepare_data_from_prepared_persistent_state)
        ),
        file!(),
        1788u32,
    );
    let file_path = file_path;
    let block_id_str = block_id_str;
    let diff_hash_str = diff_hash_str;
    ...
    {
        __guard.end_section(1803u32);
        let __result = storage.import_from_file(&top_update, file, block_id).await;
        __guard.start_section(1803u32);
        __result
    }?;
...
async fn test_import_persistent_state() -> anyhow::Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(test_import_persistent_state)),
        file!(),
        1809u32,
    );
    let (ctx, _tmp_dir) = {
        __guard.end_section(1811u32);
        let __result = StorageContext::new_temp().await;
        __guard.start_section(1811u32);
        __result
    }?;
    let queue_factory = QueueFactoryStdImpl {
        state: QueueStateImplFactory::new(ctx)?,
    ...
    let (shard_block_id, shard_top_update) = {
        __guard.end_section(1834u32);
        let __result = prepare_data_from_prepared_persistent_state(
...
fn check_imported_queue(
    ...
    let separated_stats = snapshot
        .collect_separated_stats_in_range_for_partitions(
            &block_id.shard,
            &FastHashSet::from_iter([0, 1].map(QueuePartitionIdx)),
            &QueueKey::MIN,
            &QueueKey::MAX,
    ...
    let iter_range = vec![
        QueueShardRange { shard_ident : block_id.shard, from : QueueKey::MIN
        .next_value(), to : QueueKey::MAX.next_value(), }
    ];
    let mut read_messages = 0;
    let mut iterator = queue
        .iterator(QueuePartitionIdx(0), &iter_range, ShardIdent::MASTERCHAIN)?;
    while iterator.next()?.is_some() {
        read_messages += 1;
    }
    let mut iterator = queue
        .iterator(QueuePartitionIdx(0), &iter_range, ShardIdent::BASECHAIN)?;
    while iterator.next()?.is_some() {
        read_messages += 1;
    }
