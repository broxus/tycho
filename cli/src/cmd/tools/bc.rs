...
use tycho_types::models::{
    Account, AccountState, BlockchainConfigParams, ConfigParam11, ExtInMsgInfo,
    GlobalCapability, MsgInfo, OwnedMessage, StdAddr, ValidatorSet,
};
    ...
    pub fn run(self) -> Result<()> {
        bc_rt(move || async move {
            ...
            match self.cmd {
                ...
                SubCmd::SetMasterKey(cmd) => {
                    __guard.end_section(42u32);
                    let __result = cmd.run().await;
                    __guard.start_section(42u32);
                    __result
                }
                SubCmd::SetElectorCode(cmd) => {
                    __guard.end_section(43u32);
                    let __result = cmd.run().await;
                    __guard.start_section(43u32);
                    __result
                }
                SubCmd::SetConfigCode(cmd) => {
                    __guard.end_section(44u32);
                    let __result = cmd.run().await;
                    __guard.start_section(44u32);
                    __result
                }
                SubCmd::ListProposals(cmd) => {
                    __guard.end_section(45u32);
                    let __result = cmd.run().await;
                    __guard.start_section(45u32);
                    __result
                }
                SubCmd::GenProposal(cmd) => {
                    __guard.end_section(46u32);
                    let __result = cmd.run().await;
                    __guard.start_section(46u32);
                    __result
                }
                SubCmd::GenProposalVote(cmd) => {
                    __guard.end_section(47u32);
                    let __result = cmd.run().await;
                    __guard.start_section(47u32);
                    __result
                }
            }
        })
...
impl GetParamCmd {
    async fn run(self) -> Result<()> {
        ...
        let output = serde_json::json!(
            { "global_id" : res.global_id, "seqno" : res.seqno, "param" : item, }
        );
        print_json(output)
    }
}
...
impl SetParamCmd {
    async fn run(self) -> Result<()> {
        ...
        let value = if self.raw_value {
            ...
            let value = serde_json::from_str::<serde_json::Value>(&self.value)
        ...
        };
        {
            ...
            let __result = send_config_action(
                    &client,
                    Action::SubmitParam {
                        index,
                        value,
...
impl SetMasterKeyCmd {
    async fn run(self) -> Result<()> {
        ...
        {
            __guard.end_section(170u32);
            let __result = send_config_action(
                    &client,
                    Action::UpdateMasterKey { pubkey },
                    &self.sign,
                )
                .await;
            __guard.start_section(170u32);
            __result
        }
    }
}
...
impl SetElectorCode {
    async fn run(self) -> Result<()> {
        ...
        let client = JrpcClient::new(self.rpc)?;
        let code = std::fs::read(&self.code_path)
            .context("failed to read elector code file")?;
...
impl SetConfigCode {
    async fn run(self) -> Result<()> {
        ...
        let client = JrpcClient::new(self.rpc)?;
        let code = std::fs::read(&self.code_path)
            .context("failed to read config code file")?;
...
impl CmdListProposals {
    async fn run(self) -> Result<()> {
        ...
        let client = JrpcClient::new(self.rpc)?;
        let config_account = {
            __guard.end_section(269u32);
            let __result = get_config_account(&client).await;
            __guard.start_section(269u32);
            __result
        }?;
        let blockchain_config = ConfigContract(&config_account)
            .make_blockchain_config()?;
        let proposals = ConfigContract(&config_account).get_proposals()?;
        let mut validator_sets = FastHashMap::default();
        for idx in 32..38 {
            __guard.checkpoint(274u32);
            if let Some(vset) = blockchain_config.as_dict().get(idx)?
                && let hash_map::Entry::Vacant(entry) = validator_sets
                    .entry(*vset.repr_hash())
            {
        ...
        let proposals = proposals
            ...
            .map(|(hash, proposal)| {
                let proposal = if self.raw {
                    ...
                    serde_json::to_value(
                        ParsedConfigProposal::from_raw(proposal, validator_set),
                    )
                }?;
...
impl ParsedConfigProposal {
    fn from_raw(proposal: ConfigProposal, validator_set: Option<&ValidatorSet>) -> Self {
        let action = ParsedConfigProposalValue::from_raw(
            proposal.param_idx,
        ...
        let until_expire_at = Duration::from_secs(
            proposal.expire_at.saturating_sub(now) as _,
        );
        Self {
...
fn param_value_to_json(idx: u32, value: &Cell) -> Result<serde_json::Value> {
    ...
    params.get(idx.to_string()).cloned().context("param not found")
}
#[derive(Debug, Clone, Serialize)]
#[serde(tag = "type")]
enum ParsedConfigProposalValue {
    RemoveParam { param: i32 },
    SetParam {
        param: i32,
        #[serde(with = "Boc")]
        value: Cell,
    ...
    Invalid { param: i32, #[serde(with = "Boc")] value: Option<Cell> },
}
    ...
    fn from_raw(idx: i32, value: Option<Cell>) -> Self {
        match idx {
            ConfigContract::PARAM_IDX_OWN_PUBKEY => {
                ...
                {
                    ParsedConfigProposalValue::SetPubkey {
                        pubkey,
                    }
                } else {
                    ParsedConfigProposalValue::Invalid {
                        param: idx,
                        value,
                    }
                }
            ...
            ConfigContract::PARAM_IDX_CONFIG_CODE => {
                if let Some(value) = &value && let Ok(mut cs) = value.as_slice()
            ...
            _ => {
                match value {
                    None => {
                        ParsedConfigProposalValue::RemoveParam {
                            param: idx,
                        }
                    }
                    Some(param_value) => {
                        let mut parsed = param_value_to_json(idx as u32, &param_value)
                            .ok();
...
impl CmdGenProposal {
    async fn run(self) -> Result<()> {
        ...
        fn build_update_code_value(
            ...
            let code = Boc::decode(code)
                .with_context(|| format!("invalid {name} code"))?;
        ...
        let (param, value, args) = match self.action {
            ...
            CmdGenProposalAction::SetElectorCode { code_path, upgrade_args, args } => {
                let value = build_update_code_value("elector", code_path, upgrade_args)?;
                (ConfigContract::PARAM_IDX_ELECTOR_CODE, Some(value), args)
            }
            CmdGenProposalAction::SetConfigCode { code_path, upgrade_args, args } => {
        ...
        let proposal_config = {
            let Some(config) = blockchain_config.get::<ConfigParam11>()? else {
                anyhow::bail!(
                    "blockchain config does not contain voting setup config (param 11)"
                );
            };
            if is_critical {
            ...
            match u32::try_from(ttl.as_secs()) {
                Ok(ttl) if ttl < proposal_config.min_store_sec => {
                    ...
                    anyhow::bail!(
                        ...
                        humantime::format_duration(min), proposal_config.min_store_sec
                    )
                }
                ...
                _ => {
                    ...
                    anyhow::bail!(
                        ...
                        humantime::format_duration(max), proposal_config.max_store_sec
                    )
                }
            }
        } else {
        ...
        let raw_price = ConfigContract::compute_proposal_price(
            &proposal_config,
            ttl,
            value.as_deref(),
        )?;
        let raw_content = ConfigContract::create_proposal_raw_content(
            param,
            value,
        ...
        anyhow::ensure!(
            args.allow_duplicate || ConfigContract(& config_account).get_proposal(&
            proposal_hash) ? .is_none(), "proposal with the same content already exists"
        );
        let query_id = args.query_id.unwrap_or_else(tycho_util::time::now_millis);
        let payload = ConfigContract::create_proposal_payload(
            query_id,
            ttl,
            raw_content,
            is_critical,
        ...
        print_json(
            serde_json::json!(
                { "hash" : proposal_hash, "query_id" : query_id.to_string(),
                "is_critical" : is_critical, "payload" : Boc::encode_base64(payload),
...
impl CmdGenProposalVote {
    async fn run(self) -> Result<()> {
        ...
        let client = JrpcClient::new(self.rpc)?;
        let LatestBlockchainConfig { global_id, config: blockchain_config, .. } = {
            __guard.end_section(739u32);
            let __result = client.get_config().await;
            __guard.start_section(739u32);
            __result
        }?;
        ...
        let (config_account, _) = {
            __guard.end_section(749u32);
            let __result = client
                .get_account(&StdAddr::new(-1, blockchain_config.address))
                .await;
            __guard.start_section(749u32);
            __result
        }
            .context("failed to get config account state")?
            .expect_existing()?;
        let Some(proposal) = ConfigContract(&config_account).get_proposal(&self.hash)?
        else {
            anyhow::bail!("no active proposal found with hash {}", self.hash);
        };
        let validator_idx = proposal
            .resolve_validator_idx(&blockchain_config.params, &keypair.public_key)?;
        ...
        print_json(
            serde_json::json!(
                { "query_id" : query_id.to_string(), "validator_idx" : validator_idx,
...
async fn send_config_action(
    client: &JrpcClient,
    action: Action,
    sign: &KeyArgs,
) -> Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(send_config_action)),
        file!(),
        811u32,
    );
    let client = client;
    ...
    let keypair = sign.get_keypair()?;
    {
        ...
        let __result = send_config_action_ext(client, action, &keypair, sign.ttl).await;
...
pub(crate) async fn send_config_action_ext(
    ...
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(send_config_action_ext)),
        file!(),
        821u32,
    );
    let client = client;
    ...
    let seqno = {
        __guard.end_section(831u32);
        let __result = prepare_action(client, &config_addr, &keypair.public_key).await;
        __guard.start_section(831u32);
        __result
    }?;
    ...
    {
        __guard.end_section(836u32);
        let __result = client.send_message(message_cell.as_ref()).await;
        __guard.start_section(836u32);
        __result
    }?;
    let output = serde_json::json!(
        { "expire_at" : expire_at, "message_hash" : message_cell.repr_hash(), "message" :
        Boc::encode_base64(message_cell.as_ref()), }
    );
    print_json(output)
}
async fn prepare_action(
    ...
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(prepare_action)),
        file!(),
        850u32,
    );
    let client = client;
    let config_addr = config_addr;
    let expected_pubkey = expected_pubkey;
    let state_init = match {
        __guard.end_section(851u32);
        let __result = client.get_account(config_addr).await;
        __guard.start_section(851u32);
        __result
    }? {
    ...
    data.load_raw(&mut buffer, 256).context("Failed to get public key")?;
    let public = ed25519::PublicKey::from_bytes(buffer)
        .context("Invalid config public key")?;
    anyhow::ensure!(& public == expected_pubkey, "Config has a different public key");
    Ok(seqno)
}
fn create_message(
    seqno: u32,
    ...
    builder.store_u32(action)?;
    builder.store_u32(seqno)?;
    builder.store_u32(expire_at)?;
    builder.store_builder(&data)?;
...
async fn get_config_account(client: &JrpcClient) -> Result<Box<Account>> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(get_config_account)),
        file!(),
        918u32,
    );
    let client = client;
    let config = {
        __guard.end_section(919u32);
        let __result = client.get_config().await;
        __guard.start_section(919u32);
        __result
    }?
        .config;
    let (account, _) = {
        __guard.end_section(923u32);
        let __result = client.get_account(&StdAddr::new(-1, config.address)).await;
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async move {
            ...
            let run_fut = JoinTask::new(async move {
                ...
                {
                    __guard.end_section(990u32);
                    let __result = f().await;
                    __guard.start_section(990u32);
                    __result
                }
            });
            let stop_fut = signal::any_signal(signal::TERMINATION_SIGNALS);
            {
                ...
                let __result = tokio::select! {
                    res = run_fut => res, signal = stop_fut => match signal { Ok(signal)
