...
use tycho_consensus::prelude::{
    EngineBinding, EngineHandle, EngineNetworkArgs, InputBuffer, MempoolAdapterStore,
...
impl Mempool {
    ...
    pub async fn boot(&self) -> Result<(EngineHandle, AnchorConsumer)> {
        ...
        let handle = EngineHandle::new(bind, &self.net_args, &self.merged_conf);

        handle.set_start_peers(&self.bootstrap_peers);

        tracing::info!("mempool engine initialized");

        Ok((handle, anchor_consumer))
    }
}
