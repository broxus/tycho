...
use tycho_consensus::prelude::{
    Engine, EngineCreated, InputBuffer, MempoolAdapterStore, MempoolConfigBuilder,
    MempoolMergedConfig, RoundWatch, TopKnownAnchor,
};
...
impl CmdRun {
    ...
    async fn run_impl(self, node_config: NodeConfig) -> Result<()> {
        ...
        loop {
            ...
            let (engine, mut result_rx) = engine.run();
            ...
            tokio::select! {
                result = &mut result_rx => bail!("engine exited as {result:?}"),
...
struct Mempool {
    ...
    storage: Storage,
    merged_config: MempoolMergedConfig,
}
...
impl Mempool {
    async fn new(cmd: CmdRun, node_config: NodeConfig) -> Result<Mempool> {
        ...
        Ok(Mempool {
            keypair,
            payload_step: cmd.payload_step,
            steps_until_full: cmd.steps_until_full,
            ...
            storage,
            merged_config: config_builder.build()?,
        })
    }
