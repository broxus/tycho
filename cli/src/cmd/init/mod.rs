...
pub struct Cmd {
    ...
    #[clap(
        ...
        long,
        value_parser,
        default_value_os = default_binary_path().as_os_str()
    )]
    binary: PathBuf,
...
impl Cmd {
    ...
    async fn run_impl(self, args: BaseArgs) -> Result<()> {
        ...
        let (node_keys_updated, node_keys_public) = prepare_node_keys(&node_keys_path)
            .context("failed to prepare node keys")?;
        ...
        let global_config = 'config: {
            ...
            if let Ok(meta) = Path::new(&path).metadata() && meta.is_file() {
                {
            ...
            }
            {
                ...
                let __result = {
                    ...
                    let __result = reqwest::get(&path).await;
                    __guard.start_section(153u32);
                    __result
                }
                    .with_context(|| {
                        format!("failed to fetch a global config. Path: {path}")
                    })?
                    .bytes()
                    .await;
        ...
        serde_json::from_slice::<GlobalConfig>(&global_config)
            .context("invalid global config")?;
        ...
        print_json(
            serde_json::json!(
                { "node_keys" : { "public" : node_keys_public, "path" : node_keys_path,
                "updated" : node_keys_updated, }, "elections" : elections_config,
                "node_config" : { "path" : config_path, "updated" : config_updated, },
                "global_config" : { "path" : global_config_path, "updated" : true, },
...
impl CmdInitConfig {
    fn run(self, args: BaseArgs) -> Result<()> {
        ...
        print_json(
            serde_json::json!(
                { "node_config" : { "path" : config_path, "updated" : true, }, }
            ),
        )
    }
}
...
impl CmdInitValidator {
    fn run(self, args: BaseArgs) -> Result<()> {
        ...
        print_json(
            serde_json::json!(
                { "elections_config" : ElectionsConfigInfo { wallet : simple
                .wallet_address, public : HashBytes(public.to_bytes()), stake : self
                .stake, path, updated, }, }
            ),
        )
    }
...
impl CmdInitSystemd {
    fn run(self, args: BaseArgs) -> Result<()> {
        let service_info = prepare_systemd_services(&args.home, &self.binary)
            .context("failed to prepare systemd services")?;
        print_json(serde_json::json!({ "systemd" : service_info, }))
    }
}
...
fn prepare_systemd_services(
    ...
    if !node_service_path.exists() {
        let node_service = format!(
            node_service!(), work_dir = home_dir.display(), tycho_bin = binary.display(),
        );
    ...
    if !elect_service_path.exists() {
        let elect_service = format!(
            elect_service!(), work_dir = home_dir.display(), tycho_bin = binary
            .display(),
        );
        std::fs::write(&elect_service_path, elect_service)
            .with_context(|| {
                format!("failed to save {}", elect_service_path.display())
            })?;
    }
    service_info
        .insert(
...
fn prepare_elections_config<P: AsRef<Path>>(
    ...
    let mut simple = if path.as_ref().exists() {
        match tycho_util::serde_helpers::load_json_from_file::<
            ElectionsConfig,
            _,
        >(&path)? {
        ...
        updated = true;
        SimpleElectionsConfig::from_key(
            &rand::random::<ed25519::SecretKey>(),
            Some(stake),
            None,
        )
    };
