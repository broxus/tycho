...
impl JrpcClient {
    ...
    pub async fn get_account(&self, address: &StdAddr) -> Result<AccountStateResponse> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_account)),
            file!(),
            47u32,
        );
        let address = address;
    ...
    pub async fn get_config(&self) -> Result<LatestBlockchainConfig> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_config)),
            file!(),
            61u32,
        );
        {
    ...
    {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(post)),
            file!(),
            74u32,
        );
        let data = data;
        ...
        let res = {
            ...
            let __result = response.text().await;
...
impl AccountStateResponse {
    pub fn expect_existing(self) -> Result<(Box<Account>, GenTimings)> {
        match self {
            ...
            Self::Exists { account, timings, .. } => Ok((account, timings)),
    ...
    D: serde::Deserializer<'de>,
{
    ...
    Boc::deserialize(deserializer)
        .and_then(|cell| read_account(cell).map_err(Error::custom))
}
    ...
    T: Deserialize<'de>,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(
            match de.deserialize_map(ResponseVisitor(PhantomData))? {
