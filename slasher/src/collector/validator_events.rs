...
impl ValidatorEventsCollector {
    ...
    pub fn set_default_batch_size(&self, batch_size: NonZeroU32) {
        ...
        for mut session in self.sessions.iter_mut() {
            // TODO: Split or grow the previous batches to not discard events.
    ...
    pub fn init_session(
        ...
        // Reset the live batches if their size has changed.
        // TODO: Split or grow the previous batches to not discard events.
        if session.batch_size != batch_size {
            session.batch_size = batch_size;
            let next_expected_seqno = session.next_expected_seqno;
...
impl ValidatorEventsListener for ValidatorEventsCollector {
    ...
    fn on_anchor_import(
        ...
        session.push_anchor_stats(block_id.seqno, &anchor_stats);
    }
...
impl SessionState {
    fn push_anchor_stats(&mut self, seqno: u32, anchor_stats: &AnchorStats) -> bool {
        if self.current_batch.contains_seqno(seqno) {
            (self.current_batch).push_anchor_stats(anchor_stats)
        } else if self.next_batch.contains_seqno(seqno) {
            (self.next_batch).push_anchor_stats(anchor_stats)
        } else {
            tracing::warn!(
                seqno,
...
mod tests {
    ...
    fn next_window_anchor_import_survives_rotation() {
        ...
        assert_eq!(batches[0].round_range, Some(10..=10));
        assert_eq!(points(&batches[0]), vec![(0, 1), (1, 0), (2, 1)]);
    }
    ...
    fn final_flush_commits_current_and_next_anchor_batches() {
        ...
        assert_eq!(batches[1].round_range, Some(11..=11));
        assert_eq!(points(&batches[1]), vec![(0, 0), (1, 1), (2, 0)]);
    }
    ...
    #[test]
    fn batch_size_update_resets_lookahead_batch() {
        let (collector, mut rx, session_id) = make_collector();
        ...
        let new_batch_size = BATCH_SIZE + 1;
        collector.set_default_batch_size(NonZeroU32::new(new_batch_size).unwrap());

        collector.on_anchor_import(
            session_id,
            &master_short(FIRST_SEQNO + new_batch_size),
            stats(11..=11, &[0, 1, 0]),
        );
        ...
        assert_eq!(batches.len(), 1);
        assert_eq!(batches[0].start_seqno, FIRST_SEQNO + new_batch_size);
        assert_eq!(batches[0].committed_blocks.len(), new_batch_size as usize);
        assert_eq!(batches[0].round_range, Some(11..=11));
        assert_eq!(points(&batches[0]), vec![(0, 0), (1, 1), (2, 0)]);
    }
    ...
    fn stats(round_range: RangeInclusive<u32>, points: &[u16]) -> AnchorStats {
        let filled_rounds = *round_range.end() - *round_range.start() + 1;
        assert!(
            (points.iter()).all(|points_proven| u32::from(*points_proven) <= filled_rounds),
            "test stats must keep points_proven <= filled_rounds"
        );
