...
impl ValidatorEventsCollector {
    pub fn subscribe_to_latest_block_ids(&self) -> watch::Receiver<u32> {
...
impl ValidatorEventsListener for ValidatorEventsCollector {
    ...
    fn on_validation_skipped(&self, sid: ValidationSessionId, block_id: &BlockId) {
        ...
        session.pending_blocks.remove(block_id);
        drop(session);

        self.latest_complete_block.send_if_modified(|prev| {
            let changed = *prev < block_id.seqno;
            if changed {
                *prev = block_id.seqno;
            }
            changed
