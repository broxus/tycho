...
use tycho_network::{
    ...
    PeerId, PeerResolver, PublicOverlay, Router,
};
pub struct NodeBase {
    pub network: Network,
...
pub async fn discover<N: TestNode>(nodes: &[N]) -> anyhow::Result<()> {
    ...
    loop {
        ...
        for (i, left) in nodes.iter().enumerate() {
            __guard.checkpoint(168u32);
            for (j, right) in nodes.iter().enumerate() {
                __guard.checkpoint(169u32);
                if i == j {
                    {
