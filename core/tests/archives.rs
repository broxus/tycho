...
async fn archives() -> Result<()> {
    ...
    {
        __guard.end_section(line!());
        let __result = test_orphaned_metadata_cleanup(storage, all_blocks, config, zerostate).await;
        __guard.start_section(line!());
        __result
    }?;
    Ok(())
...
async fn test_pagination(storage: CoreStorage) -> Result<Vec<BlockId>> {
    ...
    loop {
        ...
        for block in &blocks {
            ...
            let block_data = {
                __guard.end_section(line!());
                let __result = storage.block_storage().load_block_data(&block_handle).await;
                __guard.start_section(line!());
                __result
            }?;
...
async fn test_orphaned_metadata_cleanup(
    ...
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(
            module_path!(),
            "::",
            stringify!(test_orphaned_metadata_cleanup)
        ),
        file!(),
        line!(),
    );
