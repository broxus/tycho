...
impl BlockHandle {
    ...
    pub fn is_persistent(&self) -> bool {
        self.inner.meta.flags().contains(BlockFlags::IS_PERSISTENT)
            || self.inner.id.seqno == 0
    }
    ...
    pub fn has_queue_diff(&self) -> bool {
        const MASK: u32 = BlockFlags::HAS_QUEUE_DIFF.bits()
            | BlockFlags::IS_REMOVED.bits();
        let flags = self.inner.meta.flags();
        flags.bits() & MASK == BlockFlags::HAS_QUEUE_DIFF.bits()
    }
    pub fn has_all_block_parts(&self) -> bool {
        const MASK: u32 = BlockFlags::HAS_ALL_BLOCK_PARTS.bits()
            | BlockFlags::IS_REMOVED.bits();
        let flags = self.inner.meta.flags();
    ...
    pub fn has_persistent_shard_state(&self) -> bool {
        self.inner.meta.flags().contains(BlockFlags::HAS_PERSISTENT_SHARD_STATE)
    }
    pub fn has_persistent_queue_state(&self) -> bool {
        self.inner.meta.flags().contains(BlockFlags::HAS_PERSISTENT_QUEUE_STATE)
    }
...
impl Drop for Inner {
    fn drop(&mut self) {
        self.cache.remove_if(&self.id, |_, weak| weak.strong_count() == 0);
    }
}
...
impl BlockDataLock {
    ...
    pub async fn read(&self) -> BlockDataGuard<'_> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(read)),
            file!(),
            192u32,
        );
        BlockDataGuard(
    ...
    pub async fn write(&self) -> BlockDataGuard<'_> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(write)),
            file!(),
            199u32,
        );
        BlockDataGuard(
            {
                ...
                let __result = self.semaphore.acquire_many(MAX_READS).await;
