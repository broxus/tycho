...
impl BlockHandleStorage {
    ...
    pub fn set_has_persistent_shard_state(&self, handle: &BlockHandle) -> bool {
        let updated = handle.meta().add_flags(BlockFlags::HAS_PERSISTENT_SHARD_STATE);
        if updated {
            self.store_handle(handle, false);
        }
        updated
    ...
    pub fn set_has_persistent_queue_state(&self, handle: &BlockHandle) -> bool {
        let updated = handle.meta().add_flags(BlockFlags::HAS_PERSISTENT_QUEUE_STATE);
        if updated {
            self.store_handle(handle, false);
        }
        updated
    ...
    pub fn store_handle(&self, handle: &BlockHandle, is_new: bool) {
        ...
        if is_new || is_key_block {
            ...
            self.db.rocksdb().write_opt(batch, self.db.block_handles.write_config())
        } else {
            self.db
                .rocksdb()
                .merge_cf_opt(
    ...
    pub fn gc_handles_cache(
        ...
        self.cache
            .retain(|block_id, value| {
                ...
                if block_id.seqno == 0
                    || is_masterchain
                        && (block_id.seqno >= mc_seqno || value.is_key_block())
                    || !is_masterchain
                        && shard_heights
                            .contains_shard_seqno(&block_id.shard, block_id.seqno)
                {
                    true
                } else {
                    total_removed += 1;
...
mod tests {
    ...
    async fn merge_operator_works() -> anyhow::Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(merge_operator_works)),
            file!(),
            379u32,
        );
        let (ctx, _tmp_dir) = {
        ...
        };
        {
            ...
            assert_eq!(
                arc_swap::RefCnt::as_ptr(& handle), arc_swap::RefCnt::as_ptr(& handle2),
            );
        }
