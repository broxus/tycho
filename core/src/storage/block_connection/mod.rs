...
impl BlockConnectionStorage {
    ...
    pub async fn wait_for_next1(&self, prev_block_id: &BlockId) -> BlockId {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(wait_for_next1)),
            file!(),
            26u32,
        );
    ...
    pub fn store_connection(
        ...
        let guard = if is_next1 { Some(self.store_next1.read()) } else { None };
        let store = {
        ...
        if is_next1 {
            self.next1_subscriptions.notify(handle.id(), connected_block_id);
        }
        drop(guard);
        if !store {
            return;
...
fn make_key(
    block_id: &BlockId,
    ty: BlockConnection,
) -> [u8; tables::BlockConnections::KEY_LEN] {
