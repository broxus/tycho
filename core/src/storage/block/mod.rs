...
mod tests {
    ...
    async fn parallel_store_data() -> Result<()> {
        ...
        let storage = {
            __guard.end_section(576u32);
            let __result = CoreStorage::open(ctx, CoreStorageConfig::new_potato()).await;
            __guard.start_section(576u32);
            __result
        }?;
        let shard = ShardIdent::MASTERCHAIN;
        for seqno in 0..1000 {
            __guard.checkpoint(579u32);
            ...
            let store_proof_and_queue = || {
                ...
                JoinTask::new(async move {
                    ...
                    if rand::random::<bool>() {
                        {
                            __guard.end_section(621u32);
                            let __result = tokio::task::yield_now().await;
                            __guard.start_section(621u32);
                            __result
                        };
                    }
                    let res = {
                ...
                for ty in ENTRY_TYPES {
                    __guard.checkpoint(710u32);
                    {
                        ...
                        let __result = blocks
                            ...
                            .await;
                        __guard.start_section(714u32);
                        __result
                    }?;
                }
                for direction in CONNECTION_TYPES {
                    __guard.checkpoint(716u32);
