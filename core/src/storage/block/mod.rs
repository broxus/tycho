...
use tycho_util::sync::{CancellationFlag, fifo_run};
...
impl BlockStorage {
    ...
    pub async fn load_block_data(&self, handle: &BlockHandle) -> Result<BlockStuff> {
        ...
        if data.len() < BIG_DATA_THRESHOLD {
            ...
            fifo_run(move || BlockStuff::deserialize(handle.id(), data.as_ref())).await
        }
    }
    ...
    pub async fn load_queue_diff(&self, handle: &BlockHandle) -> Result<QueueDiffStuff> {
        ...
        if data.len() < BIG_DATA_THRESHOLD {
            ...
            let handle = handle.clone();
            fifo_run(move || QueueDiffStuff::deserialize(handle.id(), data.as_ref())).await
        }
    }
