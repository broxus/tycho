...
impl BlockStorage {
    ...
    pub async fn load_block_data_uncompressed(&self, handle: &BlockHandle) -> Result<Bytes> {
        if !handle.has_data() {
            return Err(BlockStorageError::BlockDataNotFound.into());
        }

        self.blob_storage
            .get_block_data(handle, &PackageEntryKey::block(handle.id()))
            .await
    }
    ...
    pub async fn get_archive_decompressed(&self, id: u32) -> Result<Option<Vec<u8>>> {
        self.blob_storage.get_archive_decompressed(id).await
    }
