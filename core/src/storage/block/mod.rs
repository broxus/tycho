    ...
    pub async fn store_block_data(
        ...
        if !handle.has_data() {
            ...
            if !handle.has_data() {
                {
                    __guard.end_section(line!());
                    let __result = self.blob_storage.add_data(&archive_id, data, &lock).await;
                    __guard.start_section(line!());
                    __result
                }?;
    ...
    pub async fn load_block_data_decompressed(&self, handle: &BlockHandle) -> Result<Bytes> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(load_block_data_decompressed)
            ),
            file!(),
            line!(),
        );
    ...
    pub async fn get_archive_compressed_full(&self, id: u32) -> Result<Option<Bytes>> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(get_archive_compressed_full)
            ),
            file!(),
            line!(),
        );
            ...
            let (data_res, proof_and_queue_res) = {
                ...
                let __result = async move {
                    ...
                    let proof_and_queue_fut = pin!(async {
                        tokio::select! { left = store_proof_and_queue() => left,
                        right = store_proof_and_queue() => right, }
                    });
                    let (data, other) =
                        futures_util::future::join(data_fut, proof_and_queue_fut).await;
