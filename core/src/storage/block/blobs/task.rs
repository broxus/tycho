    ...
    async fn cannot_commit_empty_archive() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(cannot_commit_empty_archive)
            ),
            file!(),
            line!(),
        );
        ...
        {
            __guard.end_section(line!());
            let __result = CommitArchiveTask::new(db, handles, archive_id, blocks, archives)
                .finish()
                .await;
            __guard.start_section(line!());
            __result
    ...
    async fn test_commit_task_missing_archive() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(test_commit_task_missing_archive)
            ),
            file!(),
            line!(),
        );
