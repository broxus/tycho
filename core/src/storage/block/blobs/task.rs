...
impl CommitArchiveTask {
        ...
        let handle = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                let histogram = HistogramGuard::begin(
                    "tycho_storage_commit_archive_time",
                );
                ...
                let mut decompressed_buffer = vec![0; 1024 * 1024 * 100];
                    ...
                    for raw_block_id in raw_block_ids.chunks_exact(BlockId::SIZE_HINT) {
                        ...
                        if ty == ArchiveEntryType::Block {
                            let handle = block_handle_storage
                                ...
                                .ok_or_else(|| {
                                    BlockStorageError::BlockHandleNotFound(
                            ...
                            anyhow::ensure!(
                                ...
                                flags.contains(BlockFlags::HAS_DATA), flags
                                .contains(BlockFlags::HAS_PROOF), flags
                                .contains(BlockFlags::HAS_QUEUE_DIFF)
                            );
                        }
                        let key = PackageEntryKey::from((block_id, ty));
                        let Some(data) = blocks.get(&key).unwrap() else {
                            anyhow::bail!(
                                BlockStorageError::BlockDataNotFound(block_id.as_short_id(),
                                ty,)
                            );
                        };
                ...
                tracing::info!(
                    archive_id, blocks = unique_ids.len(), elapsed = %
                    humantime::format_duration(histogram.finish()), "Archive committed"
                );
                Ok(())
            }
        });
    ...
    pub(super) async fn finish(&mut self) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(finish)),
            file!(),
            171u32,
        );
        if let Some(handle) = &mut self.handle {
            ...
            if let Err(e) = &result {
                tracing::error!(
                    archive_id = self.archive_id, "failed to commit archive: {e:?}"
                );
            }
            {
...
mod tests {
    ...
    async fn test_commit_task_block_completeness_validation() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(), "::",
                stringify!(test_commit_task_block_completeness_validation)
            ),
            file!(),
            218u32,
        );
        let (db, handles, blocks, archives, _temp_dir) = {
            __guard.end_section(219u32);
            let __result = create_test_storage_components().await;
        ...
        handles
            .store_handle(
                &create_handle_with_flags(
                    block_id,
                    BlockFlags::HAS_DATA | BlockFlags::HAS_QUEUE_DIFF,
                    &handles,
                ),
                true,
            );
        db.archive_block_ids.insert(archive_id.to_be_bytes(), block_id.to_vec())?;
        {
            __guard.end_section(239u32);
    ...
    async fn cannot_commit_empty_archive() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(cannot_commit_empty_archive)),
            file!(),
            251u32,
        );
        let (db, handles, blocks, archives, _temp_dir) = {
        ...
        let archive_id = 3u32;
        db.archive_block_ids.insert(archive_id.to_be_bytes(), vec![])?;
        {
            __guard.end_section(259u32);
            let __result = CommitArchiveTask::new(
                    db,
                    handles,
                    archive_id,
                    blocks,
    ...
    async fn test_commit_task_missing_archive() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(test_commit_task_missing_archive)),
            file!(),
            266u32,
        );
        let (db, handles, blocks, archives, _temp_dir) = {
        ...
        let err = {
            ...
            let __result = CommitArchiveTask::new(db, handles, 4, blocks, archives)
                .finish()
                .await;
            ...
            .unwrap_err();
        assert!(
            err.downcast_ref::< BlockStorageError > ().is_some_and(| e | matches!(e,
            BlockStorageError::ArchiveNotFound(_)))
        );
        Ok(())
    }
}
