...
impl BlobStorage {
    ...
    pub async fn move_into_archive(
        ...
        if let Some(to_commit) = archive_id.to_commit {
            let mut prev_archive_commit = {
                __guard.end_section(567u32);
                let __result = self.prev_archive_commit.lock().await;
                __guard.start_section(567u32);
                __result
            };
            if let Some(task) = &mut *prev_archive_commit {
                {
                    __guard.end_section(572u32);
                    let __result = task.finish().await;
                    __guard.start_section(572u32);
                    __result
                }?;
    ...
    pub async fn wait_for_archive_commit(&self) -> Result<()> {
        ...
        let mut prev_archive_commit = {
            __guard.end_section(586u32);
            let __result = self.prev_archive_commit.lock().await;
            __guard.start_section(586u32);
            __result
        };
        if let Some(task) = &mut *prev_archive_commit {
            {
    ...
    pub async fn get_block_data_decompressed(
        ...
        let id = id;
        let _lock = {
            __guard.end_section(755u32);
            let __result = lock_block_handle(handle, id.ty).await;
            __guard.start_section(755u32);
            __result
        };
        let id = *id;
        let blocks = self.blocks.clone();
        {
            __guard.end_section(769u32);
...
mod test {
    ...
    > {
        ...
        let (db, handles, temp_dir) = {
            __guard.end_section(953u32);
            let __result = create_temp_db_and_handles().await;
            __guard.start_section(953u32);
            __result
        }?;
        let blocks = Cas::<
            PackageEntryKey,
    ...
    async fn test_archive_override_mechanism() -> Result<()> {
        ...
        let (storage, _temp_dir) = {
            __guard.end_section(1010u32);
            let __result = create_test_storage().await;
            __guard.start_section(1010u32);
            __result
        }?;
        let check = |
            id: u32,
    ...
    async fn out_of_order() -> Result<()> {
        ...
        let (storage, _temp_dir) = {
            __guard.end_section(1032u32);
            let __result = create_test_storage().await;
            __guard.start_section(1032u32);
            __result
        }?;
        ...
        {
            __guard.end_section(1080u32);
            let __result = save_block(1, &storage, &mut ids, BlockDestination::Archive1)
                .await;
            __guard.start_section(1080u32);
            __result
        }?;
        for i in (2..10).rev() {
            __guard.checkpoint(1083u32);
            {
                __guard.end_section(1084u32);
                let __result = save_block(
                    ...
                    .await;
                __guard.start_section(1084u32);
                __result
            }?;
        }
        {
            __guard.end_section(1087u32);
            let __result = save_block(
                    11,
                    &storage,
                    &mut ids,
                ...
                .await;
            __guard.start_section(1087u32);
            __result
        }?;
        {
            __guard.end_section(1091u32);
            let __result = save_block(12, &storage, &mut ids, BlockDestination::Archive2)
                .await;
            __guard.start_section(1091u32);
            __result
        }?;
        {
        ...
        let archive = {
            __guard.end_section(1100u32);
            let __result = storage.get_archive_full(1).await;
            __guard.start_section(1100u32);
            __result
        }?
