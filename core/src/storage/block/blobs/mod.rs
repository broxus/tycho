...
impl BlobStorage {
    ...
    /// Adds a block to an archive and triggers commit of previous archive if needed
    ///
    /// This is the main entry point for archive creation. The process:
    ///
    /// ## Step 1: Archive ID Preparation
    /// ```
    /// archive_id = prepare_archive_id(mc_seqno, is_key_block)
    /// ```
    /// Determines which archive to use based on:
    /// - Current archive capacity (`ARCHIVE_PACKAGE_SIZE` = 100 blocks)
    /// - Key block boundaries (force new archive)
    ///
    /// ## Step 2: Block ID Accumulation
    /// ```
    /// archive_block_ids[archive_id] += block_id // via RocksDB merge operators
    /// ```
    /// This operation:
    /// - Marks the archive as "building" (presence in `ArchiveIds`)
    /// - Accumulates block IDs for later archive construction
    /// - Uses `RocksDB` merge for efficient append operations
    ///
    /// ## Step 3: Previous Archive Commit
    /// ```
    /// IF archive_id.to_commit.is_some():
    ///     spawn_commit_archive(to_commit)  // Async commit
    /// ```
    /// When starting a new archive, commits the previous one by:
    /// - Waiting for any existing commit to complete
    /// - Spawning async commit task for the previous archive
    /// - Notifying subscribers when commit completes
    ///
    /// ## Atomicity Guarantees
    ///
    /// - Block ID is committed to `RocksDB` before archive commit starts
    /// - Archive commit is atomic (all-or-nothing via Cassadilia)
    /// - Failed commits leave block IDs in `RocksDB` for recovery
    /// - Sequential archive commits prevent race conditions
    ///
    /// ## Error Recovery
    ///
    /// - If this function fails, block ID remains in `archive_block_ids`
    /// - Recovery process will detect and resume/cleanup incomplete archives
    /// - No data loss possible due to `RocksDB` durability guarantees
    ...
    /// Get the complete archive (decompressed).
    pub async fn get_archive_decompressed(&self, id: u32) -> Result<Option<Vec<u8>>> {
        if let Some(compressed_data) = self.get_archive(id).await? {
            let mut decompressed = Vec::new();
            tycho_util::compression::zstd_decompress(&compressed_data, &mut decompressed)?;
