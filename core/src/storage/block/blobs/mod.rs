                ...
                while let Some((key, value)) = iter.item() {
                    ...
                    match event {
                        ...
                        ARCHIVE_EVENT_COMMITTED => {
                            ...
                            anyhow::ensure!(
                                archives_to_commit.is_empty(),
                                "skipped archive commit"
                            );
                            anyhow::ensure!(
                        ...
                        _ => {
                            tracing::warn!(archive_id, event, "skipping unknown archive event")
                        }
                    }
                    iter.next();
                }
    ...
    pub async fn get_block_data_decompressed(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(get_block_data_decompressed)
            ),
            file!(),
            line!(),
        );
        ...
        let blocks = self.blocks.clone();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || match blocks.get(&id)? {
                ...
                None => Err(BlockStorageError::PackageEntryNotFound(
...
mod test {
    ...
    pub async fn create_test_storage_components() -> Result<(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(create_test_storage_components)
            ),
            file!(),
            line!(),
        );
    ...
    async fn test_archive_override_mechanism() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(),
                "::",
                stringify!(test_archive_override_mechanism)
            ),
            file!(),
            line!(),
        );
    ...
    async fn out_of_order() -> Result<()> {
        ...
        {
            __guard.end_section(line!());
            let __result = save_block(1, &storage, &mut ids, BlockDestination::Archive1).await;
            __guard.start_section(line!());
            __result
        }?;
        for i in (2..10).rev() {
            {
                __guard.end_section(line!());
                let __result = save_block(i, &storage, &mut ids, BlockDestination::Archive1).await;
                __guard.start_section(line!());
                __result
            }?;
        }
        {
            __guard.end_section(line!());
            let __result = save_block(11, &storage, &mut ids, BlockDestination::TriggerSplit).await;
            __guard.start_section(line!());
            __result
        }?;
        {
            __guard.end_section(line!());
            let __result = save_block(12, &storage, &mut ids, BlockDestination::Archive2).await;
            __guard.start_section(line!());
            __result
        }?;
        {
