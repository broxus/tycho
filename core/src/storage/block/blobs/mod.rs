...
use crate::storage::{
    BlockFlags, BlockHandle, BlockHandleStorage, BlockMeta, CoreDb, tables,
};
const ARCHIVE_PACKAGE_SIZE: u32 = 100;
pub(super) const DEFAULT_CHUNK_SIZE: u64 = 1024 * 1024;
...
impl BlobStorage {
    pub async fn new(
        ...
        let blobdb_path = blobdb_path;
        let pre_create_cas_dirs = pre_create_cas_dirs;
        ...
        let config = cassadilia::Config {
            ...
            fail_on_integrity_errors: false,
        };
        ...
        let storage = {
            ...
            let __result = storage.sync_state_after_init().await;
    ...
    async fn sync_state_after_init(mut self) -> Result<Self> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(sync_state_after_init)),
            file!(),
            137u32,
        );
        ...
        let (this, to_commit) = {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    while let Some((key, value)) = iter.item() {
                        ...
                        match event {
                            ...
                            ARCHIVE_EVENT_COMMITTED => {
                                ...
                                anyhow::ensure!(
                                    last == Some(archive_id), "invalid archive COMMITTED entry"
                                );
                                anyhow::ensure!(
                                    archives_to_commit.is_empty(), "skipped archive commit"
                                );
                                anyhow::ensure!(
                            ...
                            _ => {
                                tracing::warn!(
                                    archive_id, event, "skipping unknown archive event"
                                )
                            }
                        }
                        iter.next();
                    ...
                    let archive_max_id = archive_ids.last().copied();
                    {
                        ...
                        anyhow::ensure!(
                            ids.items.is_empty(), "invalid initial blob storage state"
                        );
                        ids.items.extend(archive_ids);
                        ids.override_next_id = override_next_id;
                    }
                    tracing::info!(
                        archive_count, archive_min_id, archive_max_id, ?
                        override_next_id, ? archives_to_commit, elapsed = %
                    ...
                    while let Some((key, _)) = iter.item() {
                        ...
                        let calculated = BlockMeta::deserialize(
                            &mut meta_bytes.as_ref(),
                        );
                    ...
                    drop(blocks_index);
                    tracing::info!(
                        package_entries_count, orphaned_flags_count,
                        restored_flags_count, elapsed = %
                        humantime::format_duration(started_at.elapsed()),
    ...
    pub async fn list_blocks(
        ...
        let continuation_key = continuation
            .map(|block_id| {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    for (key, _) in iter {
                        ...
                        if let Some(compressed_data) = blocks.get(key)? {
                            ...
                            tycho_util::compression::ZstdDecompress::begin(
                                    &compressed_data,
                                )?
    ...
    pub async fn find_mc_block_data(&self, mc_seqno: u32) -> Result<Option<Block>> {
        ...
        let blocks = self.blocks.clone();
        {
            __guard.end_section(463u32);
            let __result = tokio::task::spawn_blocking(move || {
                    let index = blocks
                        .read_index_state()
                        .range(lower_key..upper_key)
                        .filter_map(|(key, _)| {
                            (key.ty == ArchiveEntryType::Block).then_some(*key)
                        })
                        .collect::<Vec<_>>();
                    for key in index {
                        if key.ty != ArchiveEntryType::Block {
    ...
    pub async fn move_into_archive(
        ...
        tracing::debug!(
            "archive_id for {} is {:?}", handle.id().as_short_id(), & archive_id
        );
        ...
        if let Some(to_commit) = archive_id.to_commit {
            let mut prev_archive_commit = {
                __guard.end_section(563u32);
                let __result = self.prev_archive_commit.lock().await;
                __guard.start_section(563u32);
                __result
            };
    ...
    pub async fn wait_for_archive_commit(&self) -> Result<()> {
        ...
        let mut prev_archive_commit = {
            __guard.end_section(582u32);
            let __result = self.prev_archive_commit.lock().await;
            __guard.start_section(582u32);
            __result
        };
        if let Some(task) = &mut *prev_archive_commit {
            {
                __guard.end_section(584u32);
                let __result = task.finish().await;
                __guard.start_section(584u32);
                __result
            }?;
    ...
    pub async fn get_archive_full(&self, id: u32) -> Result<Option<Bytes>> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_archive_full)),
            file!(),
            624u32,
        );
        let id = id;
    ...
    pub async fn get_archive_chunk(&self, id: u32, offset: u64) -> Result<Bytes> {
        ...
        let offset = offset;
        anyhow::ensure!(
            offset.is_multiple_of(DEFAULT_CHUNK_SIZE), BlockStorageError::InvalidOffset {
            offset, chunk_size : DEFAULT_CHUNK_SIZE, }
        );
        let archives = self.archives.clone();
        {
    ...
    pub async fn remove_outdated_archives(&self, mut until_id: u32) -> Result<()> {
        ...
        let (len, first, last) = {
                ...
                .cloned()
            {
                ...
                None => {
                    ...
                    {
                        __guard.end_section(669u32);
                        return Ok(());
                    };
                }
            };
            let removed_ids = std::mem::replace(&mut archive_ids.items, retained_ids);
            let (Some(&first), Some(&last)) = (removed_ids.first(), removed_ids.last())
            else {
        ...
        let db = self.db.clone();
        {
            ...
            let __result = tokio::task::spawn_blocking(move || {
                    ...
                    tracing::info!(
                        archive_count = len, first, last, "finished archives GC"
                    );
                    Ok(())
                })
                .await;
    ...
    pub(crate) async fn add_data(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(add_data)),
            file!(),
            727u32,
        );
        let id = id;
    ...
    pub async fn get_block_data_decompressed(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_block_data_decompressed)),
            file!(),
            748u32,
        );
        let handle = handle;
        ...
        let _lock = {
            ...
            let __result = lock_block_handle(handle, id.ty).await;
        ...
        let blocks = self.blocks.clone();
        {
            __guard.end_section(765u32);
            let __result = tokio::task::spawn_blocking(move || match blocks.get(&id)? {
                    Some(compressed_data) => {
                        let mut output = Vec::new();
                        ZstdDecompress::begin(&compressed_data)?
                            .decompress(&mut output)?;
                        Ok(Bytes::from(output))
                    }
                    None => {
                        Err(
                            BlockStorageError::PackageEntryNotFound(
    ...
    pub async fn get_block_data_range(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(get_block_data_range)),
            file!(),
            773u32,
        );
        let handle = handle;
    ...
    fn prepare_archive_id(
        &self,
        mc_seqno: u32,
        force_split_archive: bool,
    ) -> PreparedArchiveId {
        let mut archive_ids = self.archive_ids.write();
...
async fn lock_block_handle(
    handle: &BlockHandle,
    ty: ArchiveEntryType,
) -> BlockDataGuard<'_> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(lock_block_handle)),
        file!(),
        861u32,
    );
    let handle = handle;
    let ty = ty;
    {
        __guard.end_section(868u32);
        let __result = match ty {
...
mod test {
    ...
    async fn create_temp_db_and_handles() -> Result<
        (CoreDb, Arc<BlockHandleStorage>, TempDir),
    > {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(create_temp_db_and_handles)),
            file!(),
            923u32,
        );
    ...
    pub async fn create_test_storage() -> Result<(BlobStorage, TempDir)> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(create_test_storage)),
            file!(),
            932u32,
        );
        let (db, handles, temp_dir) = {
            __guard.end_section(933u32);
            let __result = create_temp_db_and_handles().await;
            __guard.start_section(933u32);
            __result
        }?;
        let storage = {
            __guard.end_section(934u32);
            let __result = BlobStorage::new(db, handles, temp_dir.path(), false).await;
            __guard.start_section(934u32);
            __result
        }?;
        Ok((storage, temp_dir))
    }
    pub async fn create_test_storage_components() -> Result<
        (CoreDb, Arc<BlockHandleStorage>, Cas<PackageEntryKey>, Cas<u32>, TempDir),
    > {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(create_test_storage_components)),
            file!(),
            944u32,
        );
        let (db, handles, temp_dir) = {
            __guard.end_section(945u32);
            let __result = create_temp_db_and_handles().await;
            __guard.start_section(945u32);
            __result
        }?;
        let blocks = Cas::<
            PackageEntryKey,
        >::open(temp_dir.path().join("packages"), cassadilia::Config::default())?;
        let archives = Cas::<
            u32,
        >::open(temp_dir.path().join("archives"), cassadilia::Config::default())?;
    ...
    async fn store_all_block_parts(
        storage: &BlobStorage,
        block_id: &BlockId,
    ) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(store_all_block_parts)),
            file!(),
            983u32,
        );
        let storage = storage;
        ...
        for (entry_type, data) in [
            ...
            __guard.checkpoint(984u32);
            {
                __guard.end_section(994u32);
                let __result = store_block_data(
                        &storage.blocks,
                        PackageEntryKey::from((*block_id, entry_type)),
                        data,
                    )
                    .await;
                __guard.start_section(994u32);
                __result
            }?;
        }
        Ok(())
    ...
    async fn test_archive_override_mechanism() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(test_archive_override_mechanism)),
            file!(),
            1001u32,
        );
        let (storage, _temp_dir) = {
            __guard.end_section(1002u32);
            let __result = create_test_storage().await;
            __guard.start_section(1002u32);
            __result
        }?;
        let check = |
            id: u32,
            force_split: bool,
            expected_id: u32,
            expected_commit: Option<u32>|
        {
            let result = storage.prepare_archive_id(id, force_split);
            assert_eq!((result.id, result.to_commit), (expected_id, expected_commit));
        };
    ...
    async fn out_of_order() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(out_of_order)),
            file!(),
            1023u32,
        );
        let (storage, _temp_dir) = {
            __guard.end_section(1024u32);
            let __result = create_test_storage().await;
            __guard.start_section(1024u32);
            __result
        }?;
        ...
        async fn save_block(
            ...
            {
                __guard.end_section(1051u32);
                let __result = store_all_block_parts(storage, &block_id).await;
                __guard.start_section(1051u32);
                __result
            }?;
            ...
            {
                __guard.end_section(1057u32);
                let __result = storage.move_into_archive(&handle, force_split).await;
                __guard.start_section(1057u32);
                __result
            }?;
            if matches!(
                destination, BlockDestination::Archive1 | BlockDestination::TriggerSplit
            ) {
        ...
        {
            __guard.end_section(1072u32);
            let __result = save_block(1, &storage, &mut ids, BlockDestination::Archive1)
                .await;
            __guard.start_section(1072u32);
            __result
        }?;
        for i in (2..10).rev() {
            __guard.checkpoint(1075u32);
            {
                __guard.end_section(1076u32);
                let __result = save_block(
                        i,
                        &storage,
                        &mut ids,
                        BlockDestination::Archive1,
                    )
                    .await;
                __guard.start_section(1076u32);
                __result
            }?;
        }
        {
            __guard.end_section(1079u32);
            let __result = save_block(
                    11,
                    &storage,
                    &mut ids,
                    BlockDestination::TriggerSplit,
                )
                .await;
            __guard.start_section(1079u32);
            __result
        }?;
        {
            __guard.end_section(1083u32);
            let __result = save_block(12, &storage, &mut ids, BlockDestination::Archive2)
                .await;
            __guard.start_section(1083u32);
            __result
        }?;
        {
            __guard.end_section(1085u32);
            let __result = storage.wait_for_archive_commit().await;
            __guard.start_section(1085u32);
            __result
        }?;
        let r = storage.db.archive_block_ids.get(1u32.to_be_bytes())?;
        assert!(r.is_none(), "Should be removed after archive commit");
        let archive = {
            __guard.end_section(1092u32);
            let __result = storage.get_archive_full(1).await;
            __guard.start_section(1092u32);
            __result
        }?
        ...
        for entry in archive {
            __guard.checkpoint(1098u32);
            let entry = entry?;
            assert!(ids.remove(& (entry.ty, entry.block_id)), "duplicate or something");
        }
