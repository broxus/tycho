...
use crate::storage::persistent_state::descriptor_cache::{CachedState, DescriptorCache};
...
impl Inner {
    ...
    async fn store_shard_state_part_impl(
        ...
        let into_store_result = |cached: Option<Arc<CachedState>>| {
            cached.map(|cs| StoreStatePartResult {
                prefix: self.shard_states_part.shard_prefix(),
                file_size: cs.file.len(),
            })
        };
        ...
        if let Some(cached) = self
            ...
            .await?
        {
            return Ok(into_store_result(Some(cached)));
        }
