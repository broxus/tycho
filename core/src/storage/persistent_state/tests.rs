...
async fn persistent_shard_state() -> Result<()> {
    ...
    assert_eq!(min_ref_mc_state.seqno(), None);
    {
        let loaded_state = {
            __guard.end_section(line!());
            let __result = shard_states.load_state(zerostate.block_id()).await;
            __guard.start_section(line!());
            __result
        }?;
    ...
    let ctx = {
        __guard.end_section(line!());
        let __result = StorageContext::new(StorageConfig::new_potato(tmp_dir.path())).await;
        __guard.start_section(line!());
        __result
    }?;
    let new_storage = {
...
async fn persistent_queue_state_read_write() -> Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(
            module_path!(),
            "::",
            stringify!(persistent_queue_state_read_write)
        ),
        file!(),
        line!(),
    );
