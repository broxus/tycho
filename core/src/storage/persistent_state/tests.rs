...
use tycho_types::models::{
    BlockId, CurrencyCollection, IntAddr, IntMsgInfo, IntermediateAddr, Message,
    MsgEnvelope, MsgInfo, OutMsg, OutMsgDescr, OutMsgNew, OutMsgQueueUpdates, ShardIdent,
    StdAddr,
};
...
async fn persistent_shard_state() -> Result<()> {
    ...
    static ZEROSTATE_BOC: &[u8] = include_bytes!(
        "../../../../core/tests/data/zerostate.boc"
    );
    ...
    assert_eq!(min_ref_mc_state.seqno(), None);
    {
        let loaded_state = {
            ...
            let __result = shard_states.load_state(0, zerostate.block_id()).await;
    ...
    let exist = persistent_states
        .state_exists(zerostate.block_id(), PersistentStateKind::Shard);
    assert!(exist);
    ...
    for _ in 0..2 {
        __guard.checkpoint(124u32);
        {
            __guard.end_section(125u32);
            let __result = read_verify_state().await;
    ...
    let new_file = persistent_states.inner.mc_states_dir(new_mc_seqno).file(file_name);
    ...
    drop(storage);
    let ctx = {
        __guard.end_section(169u32);
        let __result = StorageContext::new(StorageConfig::new_potato(tmp_dir.path()))
            .await;
        __guard.start_section(169u32);
        __result
    }?;
    let new_storage = {
...
async fn persistent_queue_state_read_write() -> Result<()> {
    let mut __guard = crate::__async_profile_guard__::Guard::new(
        concat!(module_path!(), "::", stringify!(persistent_queue_state_read_write)),
        file!(),
        196u32,
    );
    ...
    dst_router
        .insert(
            0.into(),
            BTreeSet::from([
                RouterAddr::from_int_addr(
                        &IntAddr::Std(StdAddr::from((0, HashBytes::from([1; 32])))),
                    )
                    .unwrap(),
            ]),
        );
    src_router
        .insert(
            ...
            BTreeSet::from([
                RouterAddr::from_int_addr(
                        &IntAddr::Std(StdAddr::from((-1, HashBytes::from([3; 32])))),
                    )
                    .unwrap(),
            ]),
        );
    ...
    persistent_states
        .inner
        .cache_state(target_mc_seqno, &target_block_id, PersistentStateKind::Queue)?;
    ...
    let decompressed = {
        ...
        let written = {
            ...
            let __result = tokio::fs::read(
                    states_dir
                        .file(
                            PersistentStateKind::Queue.make_file_name(&target_block_id),
                        )
                        .path(),
                )
                .await;
        ...
        tracing::info!(
            compressed_size = % ByteSize(compressed_size), decompressed_size = %
            ByteSize(decompressed_size),
        );
        written
    };
    let tail_len = blocks
