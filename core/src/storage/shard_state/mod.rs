...
impl ShardStateStorage {
    ...
    pub async fn load_state(&self, block_id: &BlockId) -> Result<ShardStateStuff> {
        ...
        tracing::warn!("load_state cell id {}", start.elapsed().as_millis());
        let start = Instant::now();
        let cell = self.cell_storage.load_cell(cell_id)?;
        tracing::warn!("load_state cell load {}", start.elapsed().as_millis());
        let start = Instant::now();
        let st = ShardStateStuff::from_root(
            block_id,
            Cell::from(cell as Arc<_>),
            &self.min_ref_mc_state,
        );
        tracing::warn!("load_state stuff {}", start.elapsed().as_millis());
        st
    }
