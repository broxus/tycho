...
impl ShardStateStorage {
    ...
    pub async fn remove_outdated_states(&self, mc_seqno: u32) -> Result<()> {
        ...
        loop {
            ...
            let cell_storage = self.cell_storage.clone();
            let accounts_split_depth = self.accounts_split_depth;
            let (total_cells, total_states, inner_alloc) = {
                __guard.end_section(line!());
                let __result = tokio::task::spawn_blocking(move || {
                        ...
                        for StateBatch { root, block_id, .. } in &current_batch {
                            if !block_id.is_masterchain() {
                                let root_cell = Cell::from(
                                    cell_storage.load_cell(root, 0)? as Arc<_>,
                                );
                                let hashes = split_shard_accounts(
                                        root_cell,
                                        accounts_split_depth,
                                    )?
                                    .into_keys()
                        ...
                        let (stats, mut batch) = cell_storage
                            .remove_cells_mt(&alloc, &roots, split_at)?;
                        ...
                        metrics::counter!("tycho_storage_state_gc_count")
                            .increment(states as u64);
                        metrics::counter!("tycho_storage_state_gc_cells_count")
                            .increment(stats as u64);
                        for StateBatch { block_id, .. } in &current_batch {
                            if block_id.is_masterchain() {
                                metrics::gauge!("tycho_gc_states_seqno")
        ...
        tracing::info!(
            removed_cells, removed_states, block_id = % top_blocks.mc_block, elapsed_sec
