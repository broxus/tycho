...
use weedb::{BoundedCfHandle, Table, rocksdb};
...
impl CellStorage {
                    ...
                    for child in &mut *iter {
                        ...
                        if let Some(entry) = self.split_at.get(child_hash) {
                            ...
                            if let Some(storage_part) = self
                                ...
                                .map(|part_ref| part_ref.value().clone())
                            {
    ...
    pub fn load_cell(
        ...
        self.load_cell_ext(hash, epoch, CellShardRouter::NoShard)
    }
    ...
    pub fn load_cell_ext(
        ...
        hash: &HashBytes,
        epoch: u32,
        shard_router: CellShardRouter,
    ) -> Result<Arc<StorageCell>, CellStorageError> {
        #[cfg(feature = "cells-metrics")]
        let _histogram = HistogramGuard::begin("tycho_storage_load_cell_time");

        // if cell shard is not specified get storage partition shard
        let shard_router = if matches!(shard_router, CellShardRouter::NoShard)
            && let Some(shard) = self.part_shard
        {
            CellShardRouter::Shard { shard }
        } else {
            shard_router
        };

        let need_replace_cell_in_cache =
            |cached_router: &CellShardRouter, load_router: &CellShardRouter| {
                match (cached_router, load_router) {
                    (cached, required) if cached == required => {
                        // do not reload cell if routers match
                        false
                    }
                    (_, CellShardRouter::NoShard) => {
                        // do not reload cell if cached with router and loading without
                        false
                    }
                    _ => {
        ...
        let mut cell = if self.part_shard.is_none()
            && let CellShardRouter::Shard { shard } = &shard_router
                ...
                .map(|part_ref| part_ref.value().clone())
        {
            let cell =
                storage_part
                    .cell_storage()
                    .load_cell_ext(hash, epoch, shard_router.clone())?;
            tracing::trace!(
                storage_shard = ?self.part_shard,
                part_shard = %shard,
                %hash,
                cell_shard_router = ?cell.shard_router,
                load_router = ?shard_router,
                "cell loaded from partition",
            );
            cell
        } else {
            // otherwise load from raw cache or db
            let cell = match self.raw_cells_cache.get_raw(&self.cells_db, hash) {
                Ok(Some(value)) => {
            ...
            tracing::trace!(
                ...
                cell_shard_router = ?cell.shard_router,
                load_router = ?shard_router,
                "cell loaded from current storage",
            );
            cell
        };
            ...
            dashmap::Entry::Occupied(mut entry) => {
                    ...
                    && !need_replace_cell_in_cache(&prev.shard_router, &cell.shard_router)
                {
                    ...
                    tracing::trace!(
                        ...
                        cached_epoch,
                        epoch,
                        "return prev cell from cache",
                    );
                } else {
                    let prev = entry.insert(CachedCell {
                        epoch,
                        weak: Arc::downgrade(&cell),
                    });
                    let prev_shard_router = prev.weak.upgrade().map(|c| c.shard_router.clone());
                    tracing::trace!(
                        storage_shard = ?self.part_shard,
                        %hash,
                        cell_shard_router = ?cell.shard_router,
                        ?prev_shard_router,
                        "replaced cell in cache",
                    );
                }
            }
        };
...
pub enum CellShardRouter {
    /// Cell does not correspond to any shard
    NoShard,
...
impl StorageCell {
    ...
    pub fn reference_raw(&self, index: u8) -> Option<&Arc<StorageCell>> {
        ...
        let mut child_shard_router = match &self.shard_router {
            CellShardRouter::Shard { shard } => CellShardRouter::Shard { shard: *shard },
            CellShardRouter::NoShard => {
                if let Some(shard) = self.cell_storage.part_shard {
                    // if current storage is partition then use its shard
                    CellShardRouter::Shard { shard }
                } else {
                    CellShardRouter::NoShard
                }
            }
            CellShardRouter::ChildIsShardAccountsRoot(idx, map) => {
                if *idx == index {
                    CellShardRouter::SplitOnPartitionsAt(map.clone())
                } else {
                    CellShardRouter::NoShard
                }
            }
            CellShardRouter::SplitOnPartitionsAt(_) => self.shard_router.clone(),
        };
        if let CellShardRouter::SplitOnPartitionsAt(map) = &child_shard_router
            && let Some(child_shard) = map.get(&child_hash)
        {
            child_shard_router = CellShardRouter::Shard {
                shard: *child_shard,
            };
        }
