...
impl CellStorage {
    ...
    pub fn apply_temp_cell(&self, root: &HashBytes) -> Result<()> {
        ...
        impl Iterator for CellHashesIter<'_> {
            ...
            fn next(&mut self) -> Option<Self::Item> {
                ...
                let item = HashBytes(
                    self.data[self.offset..self.offset + 32].try_into().unwrap(),
                );
        ...
        impl<'a> Context<'a> {
            ...
            fn load_temp(
                &self,
                key: &HashBytes,
            ) -> Result<CellHashesIter<'a>, CellStorageError> {
                ...
                Ok(
                    match self.transaction.entry(*key) {
                        hash_map::Entry::Occupied(mut entry) => {
                            entry.get_mut().additions += 1;
                            InsertedCell::Existing
                        }
                        hash_map::Entry::Vacant(entry) => {
                            if let Some(value) = self.db.cells.get(key)? {
                                let (rc, value) = refcount::decode_value_with_rc(
                                    value.as_ref(),
                                );
                                debug_assert!(
                                    rc > 0 && value.is_some() || rc == 0 && value.is_none()
                                );
                                if value.is_some() {
                                    entry
                                        .insert(TempCell {
            ...
            fn flush_new_cells(&mut self) -> Result<(), rocksdb::Error> {
                if self.new_cell_count > 0 {
                    self.db.rocksdb().write(std::mem::take(&mut self.new_cells_batch))?;
                ...
                'outer: loop {
                    ...
                    for child in &mut *iter {
                        ...
                        if self.split_at.contains_key(child_hash) {
                            let mut delayed_additions = self
                                .delayed_additions
                                .lock()
                                .unwrap();
                ...
                Ok(
                    match self.transaction.entry(*key) {
                        ...
                        Entry::Vacant(entry) => {
                            let data = if is_new {
                                Some(
                                    alloc.bump.alloc_slice_copy(alloc.buffer.as_slice())
                                        as &[u8],
                                )
                            } else {
                                None
                    ...
                    for _ in 0..num_threads {
                        ...
                        let shards = unsafe {
                            (range_start..range_end)
                                .map(|i| self.transaction._get_read_shard(i))
                        };
                        range_start = range_end;
                ...
                Ok(
                    match self.transaction.entry(key) {
                        ...
                        hash_map::Entry::Vacant(entry) => {
                            ...
                            let data = if is_new {
                                self.buffer.clear();
                                if StorageCell::serialize_to(cell, &mut self.buffer)
                                    .is_err()
                                {
                                    return Err(CellStorageError::InvalidCell);
                                }
                                Some(
                                    self.alloc.alloc_slice_copy(self.buffer.as_slice()) as &[u8],
                                )
                            } else {
                                None
                            };
    ...
    pub fn load_cell(
        ...
        let mut cell = match self.raw_cells_cache.get_raw(&self.db, hash) {
            Ok(Some(value)) => {
                match StorageCell::deserialize(self.clone(), &value.slice, epoch) {
    ...
    pub fn remove_cells_mt(
        ...
        std::thread::scope(|scope| {
            ...
            for root in roots {
                let res = scope
                    .spawn(|| -> RemoveResult { ctx.traverse_cell(root, scope) });
                results.push(res);
            }
            results
                .into_iter()
    ...
    pub fn remove_cell(
        ...
        let mut transaction: FastHashMap<&HashBytes, RemovedCell<'_>> = FastHashMap::with_capacity_and_hasher(
            128,
            Default::default(),
        );
...
impl<'a> RemovedCell<'a> {
    ...
    fn next_refs(&self) -> Option<&'a [HashBytes]> {
        if self.old_rc > self.removes as i64 { None } else { Some(self.refs) }
    }
}
...
impl StorageCell {
    ...
    pub fn deserialize(
        ...
        let data_ptr = Box::into_raw(Box::<[u8]>::from(&buffer[4..4 + allocated_len]))
            .cast::<u8>();
        let reference_states = Default::default();
        let mut reference_data = unsafe {
            MaybeUninit::<[UnsafeCell<StorageCellReferenceData>; 4]>::uninit()
                .assume_init()
        };
        const {
            assert!(
                std::mem::size_of::< UnsafeCell < StorageCellReferenceData >> () == 32
            )
        };
        unsafe {
    ...
    pub fn deserialize_references(data: &[u8], target: &mut Vec<HashBytes>) -> bool {
        ...
        let ref_count = descriptor.reference_count() as usize;
        let mut offset = 4usize + descriptor.byte_len() as usize
    ...
    pub fn serialize_to(cell: &DynCell, target: &mut Vec<u8>) -> Result<()> {
        ...
        target
            .reserve(
                4usize + descriptor.byte_len() as usize + (32 + 2) * hash_count as usize
...
impl Drop for StorageCell {
    fn drop(&mut self) {
        let allocated_len = self.data_len as usize
            + (self.hash_count as usize) * Self::HASHES_ITEM_LEN;
        _ = unsafe {
            Box::from_raw(
...
impl quick_cache::Weighter<HashBytes, RawCellsCacheItem> for CellSizeEstimator {
    fn weight(&self, key: &HashBytes, val: &RawCellsCacheItem) -> u64 {
        const STATIC_SIZE: usize = std::mem::size_of::<RawCellsCacheItem>()
            + std::mem::size_of::<i64>() + std::mem::size_of::<usize>() * 2;
...
impl RawCellsCache {
    ...
    fn new(size_in_bytes: u64) -> Self {
        ...
        tracing::info!(
            estimated_cell_cache_capacity, max_cell_cache_size = %
            bytesize::ByteSize(size_in_bytes),
        );
    ...
    fn refresh_metrics(&self) {
        metrics::gauge!("tycho_storage_raw_cells_cache_size")
            .set(self.inner.weight() as f64);
    }
}
