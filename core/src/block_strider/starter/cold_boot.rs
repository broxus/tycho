...
#[cfg(feature = "s3")]
use crate::block_strider::starter::s3_cold_boot::S3Starter;
        ...
        let last_mc_block_id = match boot_type {
            ...
            ColdBootType::PersistentFromS3 => {
                let Some(s3_client) = &self.s3_client else {
                    anyhow::bail!("S3 client not found")
                };

                let s3_starter = S3Starter::new(&self.config.start_from, s3_client, &self.storage)?;
