...
pub trait BlockProvider: Send + Sync + 'static {
    type GetNextBlockFut<'a>: Future<Output = Option<anyhow::Result<Block>>> + Send + 'a;
    type GetBlockFut<'a>: Future<Output = Option<anyhow::Result<Block>>> + Send + 'a;
...
impl<T1: BlockProvider, T2: BlockProvider> BlockProvider for ChainBlockProvider<T1, T2> {
    type GetNextBlockFut<'a> = BoxFuture<'a, Option<anyhow::Result<Block>>>;
    type GetBlockFut<'a> = BoxFuture<'a, Option<anyhow::Result<Block>>>;
...
mod test {
    ...
    fn get_empty_block() -> Block {
        ...
        everscale_types::boc::BocRepr::decode_base64(block).unwrap()
    }
