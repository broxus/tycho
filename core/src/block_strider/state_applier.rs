    ...
    S: StateSubscriber,
{
    ...
    async fn handle_block_impl(
        ...
        // We can't get gen_utime/seqno from handle, so get from block info
        let info = cx.block.load_info()?;
        let gen_utime = info.gen_utime as f64;
        let seqno = cx.block.id().seqno as f64;
        ...
        self.inner
            .block_saver
            .handle_block(cx, prepared.handle)
            .await?;
