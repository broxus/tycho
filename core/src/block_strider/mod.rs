...
pub use self::subscriber::{
    ...
    MetricsSubscriber, NoopSubscriber, PsSubscriber, StateSubscriber,
    ...
    B: BlockSubscriber,
{
    ...
    async fn process_shard_blocks(
        ...
        while let Some(prepared) = prepare_task.take() {
            ...
            {
                __guard.end_section(357u32);
                let __result = delayed_handle.join().await;
                __guard.start_section(357u32);
                __result
            }?;
            self.state
