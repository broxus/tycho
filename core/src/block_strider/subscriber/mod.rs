...
pub use self::futures::{
    DelayedTasks, DelayedTasksJoinHandle, DelayedTasksSpawner, OptionHandleFut,
    OptionPrepareFut,
};
...
impl<T: BlockSubscriber> BlockSubscriber for Option<T> {
    ...
    fn handle_block<'a>(
        ...
        OptionHandleFut::from(
            match (self, prepared) {
                (Some(subscriber), Some(prepared)) => {
                    Some(subscriber.handle_block(cx, prepared))
                }
                _ => None,
            },
        )
...
impl<T1: BlockSubscriber, T2: BlockSubscriber> BlockSubscriber
for ChainSubscriber<T1, T2> {
    ...
    fn prepare_block<'a>(
        ...
        Box::pin(async move {
            ...
            match {
                ...
                let __result = future::join(left, right).await;
...
impl<T1: StateSubscriber, T2: StateSubscriber> StateSubscriber
for ChainSubscriber<T1, T2> {
...
macro_rules! impl_subscriber_tuple {
    (
        ...
        impl <$($ty),*> BlockSubscriber for ($($ty),*) where $($ty : BlockSubscriber),* {
        type Prepared = ($($ty ::Prepared),*); type PrepareBlockFut <'a > = BoxFuture
        <'a, Result < Self::Prepared >>; type HandleBlockFut <'a > = BoxFuture <'a,
        Result < () >>; fn prepare_block <'a > (&'a self, cx : &'a
        BlockSubscriberContext) -> Self::PrepareBlockFut <'a > { $(let $var = self.$n
        .prepare_block(cx));*; Box::pin(async move { match $join_fn ($($var),*). await {
        ($(Ok($var)),*) => Ok(($($var),*)), $err_pat => Err($e), } }) } fn handle_block
        <'a > (&'a self, cx : &'a BlockSubscriberContext, ($($var),*) : Self::Prepared,)
        -> Self::HandleBlockFut <'a > { $(let $var = self.$n .handle_block(cx, $var));*;
        Box::pin(async move { match $join_fn ($($var),*). await { $err_pat => Err($e), _
        => Ok(()), } }) } } impl <$($ty),*> StateSubscriber for ($($ty),*) where $($ty :
        StateSubscriber),* { type HandleStateFut <'a > = BoxFuture <'a, Result < () >>;
        fn handle_state <'a > (&'a self, cx : &'a StateSubscriberContext) ->
        Self::HandleStateFut <'a > { $(let $var = self.$n .handle_state(cx));*;
        Box::pin(async move { match $join_fn ($($var),*). await { $err_pat => Err($e), _
        => Ok(()), } }) } }
    };
}
...
pub mod test {
    ...
    impl BlockSubscriber for PrintSubscriber {
        ...
        fn prepare_block<'a>(
            ...
            tracing::info!(
                block_id = % cx.block.id(), mc_block_id = % cx.mc_block_id,
                "preparing block"
            );
            future::ready(Ok(()))
        }
