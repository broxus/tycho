...
macro_rules! impl_select_tuple {
    ($($n:tt : $ty:ident),+) => {
        ...
        > { fn from(futures : ($($ty),*)) -> Self { SelectNonEmptyFut { futures :
        ($(Some(futures.$n),)*) } } } impl <$($ty),+, R > Future for SelectNonEmptyFut <
        ($(Option <$ty >),*) > where $($ty : Future < Output = Option < R >> + Unpin,)* {
        type Output = Option < R >; fn poll(mut self : Pin <& mut Self >, cx : & mut
        Context <'_ >) -> Poll < Self::Output > { let mut all_done = true; $(if let
        Some(f) = & mut self.futures.$n { match Pin::new(f).poll(cx) { Poll::Ready(res)
        if res.is_some() => return Poll::Ready(res), Poll::Ready(_) => self.futures.$n =
        None, Poll::Pending => all_done = false, } })* if all_done { Poll::Ready(None) }
        else { Poll::Pending } } }
    };
}
...
mod tests {
    ...
    async fn make_skip_fut(n: usize, res: Option<usize>) -> Option<usize> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(make_skip_fut)),
            file!(),
            59u32,
        );
        let n = n;
    ...
    async fn select_futures() {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(select_futures)),
            file!(),
            67u32,
        );
        {
