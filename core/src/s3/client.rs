...
use object_store::{Attribute, DynObjectStore, Error, ObjectMeta, ObjectStore};
...
impl S3Client {
    ...
    pub async fn get_key_block_full(
        &self,
        block_id: &BlockId,
    ) -> anyhow::Result<Option<BlockDataFull>, Error> {
        let location =
            PathBuf::from("states").join(PersistentStateKind::Shard.make_file_name(block_id));
        let path = Path::from(location.display().to_string());
        ...
        let archive_id = result
            .attributes
            .get(&Attribute::Metadata("archive_id".into()))
            .and_then(|v| v.parse::<u64>().ok())
            .ok_or_else(|| Error::Generic {
                store: "metadata",
                source: anyhow::anyhow!("attribute archive_id not found").into(),
            })?;
