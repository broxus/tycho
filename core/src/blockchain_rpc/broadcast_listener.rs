...
macro_rules! impl_listener_tuple {
    ($join_fn:path, { $($n:tt : $var:ident = $ty:ident,)* }) => {
        impl <$($ty,)*> BroadcastListener for ($($ty,)*) where $($ty :
        BroadcastListener,)* { type HandleMessageFut <'a > = HandleMessageFut <'a >; fn
        handle_message(& self, meta : Arc < InboundRequestMeta >, message : Bytes,) ->
        Self::HandleMessageFut <'_ > { impl_listener_tuple!(@ call self meta message $($n
        $var)*); Box::pin(async move { $join_fn ($($var),*). await; }) } }
    };
...
impl_listener_tuple! {
    futures_util::future::join, { 0 : a = T0, 1 : b = T1, }
}
impl_listener_tuple! {
    futures_util::future::join3, { 0 : a = T0, 1 : b = T1, 2 : c = T2, }
}
impl_listener_tuple! {
    futures_util::future::join4, { 0 : a = T0, 1 : b = T1, 2 : c = T2, 3 : d = T3, }
}
impl_listener_tuple! {
    futures_util::future::join5, { 0 : a = T0, 1 : b = T1, 2 : c = T2, 3 : d = T3, 4 : e
    = T4, }
}
...
impl<T: BroadcastListener> BroadcastListenerExt for T {
    fn boxed(self) -> BoxBroadcastListener {
        castaway::match_type!(
            self, { BoxBroadcastListener as listener => listener, listener =>
            BoxBroadcastListener::new(listener), }
        )
    }
}
...
type HandleMessageFn = for<'a> unsafe fn(
    &'a AtomicPtr<()>,
    Arc<InboundRequestMeta>,
    Bytes,
) -> HandleMessageFut<'a>;
...
mod tests {
    ...
    async fn boxed_listener_works() -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(boxed_listener_works)),
            file!(),
            202u32,
        );
        ...
        });
        {
            ...
            let __result = boxed.handle_message(meta.clone(), Bytes::new()).await;
