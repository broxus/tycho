...
use tycho_network::{
    try_handle_prefix, InboundRequestMeta, PeerId, Response, Service, ServiceRequest,
};
use tycho_storage::Storage;
use tycho_util::futures::BoxFutureOrNoop;
use tycho_util::metrics::HistogramGuard;

use crate::blockchain_rpc::service::util::Constructor;
...
impl Default for RateLimits {
    fn default() -> Self {
        Self {
            info_method_rps: NonZeroU32::new(10).unwrap(),
            chunk_method_rps: NonZeroU32::new(10).unwrap(),
        }
    }
}
    ...
    B: BroadcastListener,
{
    pub fn build(self) -> BlockchainRpcService<B> {
        ...
        BlockchainRpcService {
            inner: Arc::new(Inner {
                storage,
                info_rate_limiter: RateLimiter::keyed(governor::Quota::per_second(
                    self.config.rate_limits.info_method_rps,
                )),
                chunk_rate_limiter: RateLimiter::keyed(governor::Quota::per_second(
                    self.config.rate_limits.chunk_method_rps,
                )),
                config: self.config,
                broadcast_listener,
            }),
...
type RateLimiter = governor::RateLimiter<
    PeerId,
    governor::state::keyed::DefaultKeyedStateStore<PeerId>,
