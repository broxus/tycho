...
impl Neighbours {
    ...
    pub async fn wait_for_peers(&self, count: usize) {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(wait_for_peers)),
            file!(),
            33u32,
        );
        let count = count;
        loop {
            __guard.checkpoint(34u32);
            let changed = self.inner.changed.notified();
            if self.inner.entries.load().len() >= count {
                {
                    __guard.end_section(38u32);
                    __guard.start_section(38u32);
                    break;
                };
            }
            {
                __guard.end_section(41u32);
                let __result = changed.await;
    ...
    pub fn update_metrics(&self, local_id: &PeerId) {
        ...
        for neighbour in entries.iter() {
            ...
            metrics::gauge!(
                "tycho_core_overlay_client_neighbour_total_requests", & labels
            )
                .set(stats.total_requests as f64);
            metrics::gauge!(
                "tycho_core_overlay_client_neighbour_failed_requests", & labels
            )
...
impl SelectionIndex {
    ...
    fn update(&mut self, neighbours: &[Neighbour]) {
        ...
        for neighbour in neighbours.iter() {
            if let Some(score) = neighbour.compute_selection_score() {
                ...
                self.indices_with_weights.push((neighbour.clone(), total_weight));
            }
        }
    ...
    fn choose<R: Rng + ?Sized>(&self, rng: &mut R) -> Option<Neighbour> {
        ...
        let i = self.indices_with_weights.partition_point(|(_, w)| *w <= chosen_weight);
        self.indices_with_weights.get(i).map(|(neighbour, _)| neighbour).cloned()
    }
    fn choose_multiple<R: Rng + ?Sized>(
        ...
        let (_, mid, greater) = candidates
            .select_nth_unstable(self.indices_with_weights.len() - n);
        let mut result = Vec::with_capacity(n);
        let check_reliable = matches!(neighbour_type, NeighbourType::Reliable);
        let is_valid = |neighbour: &Neighbour| {
            !check_reliable || neighbour.is_reliable()
        };
