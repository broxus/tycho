...
use crate::db::*;
use crate::model::{ValueType, ShardsInternalMessagesKey};
...
impl InternalQueueStorage {
    ...
    pub fn insert_end_diff_session(
        &self,
        batch: &mut WriteBatch,
        key: ShardsInternalMessagesKey,
    ) {
        ...
        println!("insert_end_diff_session {:?}", key);
        Self::insert_end_diff(batch, cf, key)
    }
    ...
    impl InternalQueueStorage {
        ...
        fn insert_end_diff(
            ...
            unsafe {
                ...
                let value_type = ValueType::DiffEnd as u8;

                // Write the value type (u8)
                // Offset: 0
                std::ptr::write(ptr, value_type);

                // Set the length of the buffer to 1
                buffer.set_len(1);
            }

            // Use the constructed buffer in the batch operation
        ...
        fn insert_message(
            ...
            // Total buffer size:
            // 1 byte  - value type
            // 1 byte  - dest_workchain
            // 8 bytes - dest_prefix
            // N bytes - cell data (length of `cell`)
            let mut buffer = Vec::with_capacity(1 + 1 + 8 + cell.len());

            unsafe {
                let ptr = buffer.as_mut_ptr();
                let value_type = ValueType::Message as u8;

                // Write the value type (u8)
                // Offset: 0
                std::ptr::write(ptr, value_type);

                // Write dest_workchain (i8)
                // Offset: 1
                std::ptr::copy_nonoverlapping(dest_workchain.to_be_bytes().as_ptr(), ptr.add(1), 1);

                // Write dest_prefix (u64)
                // Offset: 2
                std::ptr::copy_nonoverlapping(dest_prefix.to_be_bytes().as_ptr(), ptr.add(2), 8);

                // Write cell data (length of `cell`)
                // Offset: 10
                std::ptr::copy_nonoverlapping(cell.as_ptr(), ptr.add(10), cell.len());

                // Set the final length of the buffer
                buffer.set_len(1 + 1 + 8 + cell.len());
            }
        ...
        pub fn commit(&self, ranges: FastHashMap<ShardIdent, QueueKey>) -> Result<()> {
            ...
            for range in ranges {
                ...
                while iter.valid() {
                    ...
                    batch.put_cf(&internal_messages_cf, current_position.to_vec(), value);
