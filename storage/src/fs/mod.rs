...
impl TempFileStorage {
    const MAX_FILE_TTL: Duration = Duration::from_secs(86400);
    ...
    pub async fn remove_outdated_files(&self) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(remove_outdated_files)),
            file!(),
            34u32,
        );
        ...
        F: FnMut(&Path, &Metadata) -> bool,
    {
        ...
        for e in entries {
            ...
            if let Err(e) = e {
                tracing::warn!(
                    path = % path.display(), "failed to remove downloads entry: {e:?}"
                );
            }
        }
        Ok(())
...
impl FileBuilder {
    ...
    pub fn exists(&self) -> bool {
        std::fs::metadata(&self.path).ok().map(|m| m.is_file()).unwrap_or_default()
    }
...
impl UnnamedFileBuilder {
    ...
    pub fn open_as_mapped_mut(&self) -> Result<MappedFileMut> {
        ...
        if let Some(prealloc) = self.prealloc {
            #[cfg(target_os = "linux")] alloc_file(&file, prealloc)?;
            file.set_len(prealloc as u64)?;
        } else {
