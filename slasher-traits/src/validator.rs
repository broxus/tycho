...
pub struct ValidatorEvents {
    recorder: Arc<dyn ValidatorEventsRecorder>,
}

impl ValidatorEvents {
    pub fn new(recorder: Arc<dyn ValidatorEventsRecorder>) -> Self {
        Self { recorder }
    }
...
impl ValidatorSessionScope {
    ...
    pub fn finish(&self, last_mc_seqno: u32) {
        if self.seal() {
            self.recorder
                .on_session_finished(self.session_id, Some(last_mc_seqno));
        }
    }
...
impl Drop for ValidatorSessionScope {
    fn drop(&mut self) {
        if self.seal() {
            self.recorder.on_session_finished(self.session_id, None);
        }
    }
}
...
pub trait ValidatorEventsRecorder: Send + Sync + 'static {
    ...
    fn on_session_finished(&self, session_id: ValidationSessionId, last_mc_seqno: Option<u32>);
...
impl ValidatorEventsRecorder for NoopValidatorEventsRecorder {
    fn on_session_started(
        &self,
    ...
    fn on_session_finished(&self, _session_id: ValidationSessionId, _last_mc_seqno: Option<u32>) {}
...
macro_rules! impl_recorder_for_tuples {
    ($(($($ty:ident: $n:tt),+)),*$(,)?) => {
        $(impl<$($ty),+> ValidatorEventsRecorder for ($($ty,)+)
        where
            $($ty: ValidatorEventsRecorder,)+
        {
            fn on_session_started(
                &self,
                session_id: ValidationSessionId,
            ...
            fn on_session_finished(&self, session_id: ValidationSessionId, last_mc_seqno: Option<u32>) {
                $(self.$n.on_session_finished(session_id, last_mc_seqno);)+
            }
