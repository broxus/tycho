                ...
                if let Some(avg_wu_metrics) = history.avg_metrics.get(&seqno) {
                        ...
                        && (avg > 0 || (avg < 0 && avg_wu_metrics.has_pending_messages))
                    {
                        fn calculate_target_wu_params(
                            ...
                            let in_msgs_len_log = wu_metrics.wu_on_finalize.in_msgs_len_log();
                            let target_build_in_msgs_wu = wu_metrics
                                .wu_on_finalize
                                .calc_target_build_in_msgs_wu(target_wu_price);
                            if let Some(target_wu_param) =
                                wu_metrics.wu_on_finalize.calc_target_build_in_msg_wu_param(
                            ...
                            let out_msgs_len_log = wu_metrics.wu_on_finalize.out_msgs_len_log();
                            ...
                            if let Some(target_wu_param) = wu_metrics
                                .wu_on_finalize
                                .calc_target_build_out_msg_wu_param(
                                    target_build_out_msgs_wu,
                                    out_msgs_len_log,
                                )
                            {
                            ...
                            let updated_accounts_count_log =
                                wu_metrics.wu_on_finalize.updated_accounts_count_log();
                            ...
                            if let Some(target_wu_param) = wu_metrics
                                ...
                                .calc_target_update_shard_accounts_wu_param(
                                    target_update_shard_accounts_wu as u128,
                                    threads_count as u128,
                                    shard_accounts_count_log as u128,
                            ...
                            if let Some(target_wu_param) = wu_metrics
                                ...
                                .calc_target_build_state_update_wu_param(
                                    target_build_state_update_wu as u128,
                                    threads_count as u128,
                                    shard_accounts_count_log as u128,
                                    pow_shard_accounts_count,
                                    scale,
                                    target_wu_params.finalize.state_update_min as u128,
                                    target_wu_params.finalize.state_update_accounts as u128,
                                )
                            {
                            ...
                            if let Some(target_wu_param) =
                                wu_metrics.wu_on_finalize.calc_target_build_block_wu_param(
                        ...
                        let target_params =
                            calculate_target_wu_params(target_wu_price, avg_wu_metrics);
                    ...
                    let target_wu_params =
                        target_wu_params.unwrap_or(avg_wu_metrics.wu_params.clone());
                    ...
                    // report anyway if params changed
                    else if let Some(prev_avg_wu_metrics) =
                        history.avg_metrics.get(&seqno.saturating_sub(1))
                        && avg_wu_metrics.wu_params != prev_avg_wu_metrics.wu_params
                    {
                        report_wu_params(&avg_wu_metrics.wu_params, &avg_wu_metrics.wu_params);
                    }
...
{
    let mut avg = WuMetricsAvg::new(44);
    ...
    for (_, v) in range {
        ...
        avg.accum_next(v.wu_on_prepare_msg_groups.total_wu);
        ...
        avg.accum_next(v.wu_on_finalize.updated_accounts_count);
    ...
    WuMetrics {
        ...
        wu_on_prepare_msg_groups: PrepareMsgGroupsWu {
            ...
            total_wu: avg.get_avg_next() as u64,
        ...
        wu_on_finalize: FinalizeWu {
            ...
            updated_accounts_count: avg.get_avg_next() as u64,
            in_msgs_len: avg.get_avg_next() as u64,
            out_msgs_len: avg.get_avg_next() as u64,
...
{
    let mut avg = WuMetricsAvg::new(26);
