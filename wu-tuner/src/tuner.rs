...
impl WuMetricsSpan {
    ...
    pub fn get_result(&mut self) -> Option<&WuMetrics> {
        ...
        let WuMetricsSpanValue::Result(avg) = &self.avg else { unreachable!() };
        Some(avg)
    }
}
...
impl AnchorsLagSpan {
    ...
    pub fn get_result(&mut self) -> i64 {
        ...
        let AnchorsLagSpanValue::Result(avg) = self.avg else { unreachable!() };
        avg
    }
}
#[derive(Default)]
...
impl TunerParams {
    fn calculate(config: &WuTunerConfig, seqno: BlockSeqno) -> Self {
        ...
        let wu_span_seqno = seqno / wu_span * wu_span;
        let wu_ma_seqno = seqno / wu_ma_interval * wu_ma_interval;
        let lag_span_seqno = seqno / lag_span * lag_span;
        let lag_ma_seqno = seqno / lag_ma_interval * lag_ma_interval;
        let wu_params_ma_seqno = seqno / wu_params_ma_interval * wu_params_ma_interval;
        Self {
            wu_span,
            wu_ma_interval,
            wu_ma_range,
    ...
    U: WuParamsUpdater,
{
    ...
    pub async fn handle_wu_event(&mut self, event: WuEvent) -> Result<()> {
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(handle_wu_event)),
            file!(),
            258u32,
        );
        let event = event;
        ...
        match data {
            WuEventData::Metrics(metrics) => {
                self.handle_wu_metrics(shard, seqno, metrics)?
            }
            WuEventData::AnchorLag(anchor_lag) => {
                {
                    __guard.end_section(264u32);
                    let __result = self
                        .handle_anchor_lag(shard, seqno, anchor_lag)
                        .await;
    ...
    fn handle_wu_metrics(
        ...
        if let Some((&last_key, _)) = history.metrics.last_key_value() {
            if last_key + wu_span < seqno {
                history.clear();
                tracing::info!(
                    % shard, seqno, last_seqno = last_key, wu_span,
        ...
        {
            tracing::info!(
                % shard, seqno, prev_params = ? last.last.wu_params, curr_params = ?
                metrics.wu_params, "wu params updated",
            );
            history.clear();
            report_wu_params(&metrics.wu_params, &metrics.wu_params);
        ...
        tracing::trace!(
            % shard, seqno, has_pending_messages, metrics_history_len = history.metrics
            .len(), "wu metrics received",
        );
        ...
        let avg_range = history
            .metrics
            .range_mut((Bound::Included(avg_from_boundary), Bound::Excluded(ma_seqno)));
        let Some(avg) = safe_metrics_avg(avg_range) else {
            return Ok(());
        };
        avg.report_metrics(&shard);
        tracing::trace!(
            % shard, seqno, "avg wu metrics calculated on [{0}..{1})", avg_from_boundary,
            ma_seqno,
        );
        history.avg_metrics.insert(ma_seqno, avg);
        let gc_boundary = wu_ma_seqno.saturating_sub(tune_interval);
        if let Some((&first_key, _)) = history.metrics.first_key_value()
            && first_key < gc_boundary
        {
            history.gc_wu_metrics(gc_boundary);
            tracing::trace!(% shard, seqno, "wu metrics history gc < {0}", gc_boundary,);
        }
        Ok(())
    }
    async fn handle_anchor_lag(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(module_path!(), "::", stringify!(handle_anchor_lag)),
            file!(),
            406u32,
        );
        let shard = shard;
        let seqno = seqno;
        let anchor_lag = anchor_lag;
        ...
        tracing::trace!(
            % shard, seqno, lag = anchor_lag.lag(), lag_bounds = ? self.config
            .lag_bounds_ms, lag_history_len = history.anchors_lag.len(),
            "anchor lag received",
        );
        if seqno < lag_ma_seqno || lag_ma_seqno == 0
        ...
        tracing::debug!(
            % shard, seqno, avg_lag, lag_bounds = ? self.config.lag_bounds_ms,
            "avg anchor lag calculated on [{0}..{1})", avg_from_boundary, lag_ma_seqno,
        );
        history.avg_anchors_lag.insert(lag_ma_seqno, avg_lag);
        let gc_boundary = lag_ma_seqno.saturating_sub(tune_interval);
        if let Some((&first_key, _)) = history.anchors_lag.first_key_value()
            && first_key < gc_boundary
        {
            history.gc_anchors_lag(gc_boundary);
            tracing::trace!(
                % shard, seqno, "anchors lag history gc < {0}", gc_boundary,
            );
        }
        if shard == ShardIdent::MASTERCHAIN {
            {
                __guard.end_section(493u32);
                return Ok(());
            };
        }
        {
            ...
            let __result = self
                .try_calculate_target_wu_params_and_perform_adjustment(
                    shard,
                    seqno,
                    &tuner_params,
                )
    ...
    async fn try_calculate_target_wu_params_and_perform_adjustment(
        ...
        let mut __guard = crate::__async_profile_guard__::Guard::new(
            concat!(
                module_path!(), "::",
                stringify!(try_calculate_target_wu_params_and_perform_adjustment)
            ),
            file!(),
            505u32,
        );
        let shard = shard;
        let seqno = seqno;
        let tuner_params = tuner_params;
        let &TunerParams { tune_interval, tune_seqno, wu_params_ma_seqno, .. } = tuner_params;
        let history = self.history.entry(shard).or_default();
        if seqno < wu_params_ma_seqno || wu_params_ma_seqno == 0
            ...
            && !(lag_lower_bound..lag_upper_bound).contains(&avg_lag)
        {
            ...
            adaptive_wu_price = if avg_lag >= lag_upper_bound {
                ...
                let adj = if (lag_lower_bound - avg_lag) > 500 { 0.2 } else { 0.1 };
                prev_wu_price.saturating_add_floor(adj)
            } else {
                unreachable!()
            };
            ...
            tracing::debug!(
                % shard, seqno, has_pending_messages = avg_wu_metrics
                .has_pending_messages, avg_lag, lag_bounds = ? self.config.lag_bounds_ms,
                current_wu_price = avg_wu_metrics.collation_total_wu_price(),
                prev_wu_price, adaptive_wu_price, target_wu_price,
                current_build_in_msg_wu_param = avg_wu_metrics.wu_params.finalize
                .build_in_msg, target_build_in_msg_wu_param = target_params.finalize
                .build_in_msg, "calculated target wu params",
            );
    ...
    fn calculate_target_wu_params(
        ...
        let shard_accounts_count_log = wu_metrics
            .wu_on_finalize
        ...
        let pow_shard_accounts_count = wu_metrics
            ...
            .pow_shard_accounts_count(
                wu_metrics.wu_params.finalize.state_update_msg,
                scale,
            );
        ...
        if let Some(target_wu_param) = wu_metrics
            .wu_on_finalize
            .calc_target_build_block_wu_param(
            ...
            )
        {
            ...
            target_wu_params.finalize.serialize_msg = target_wu_params
                .finalize
                .serialize_accounts;
        }
        ...
        if let Some(target_wu_param) = wu_metrics
            .wu_on_prepare_msg_groups
            .calc_target_read_existing_int_msgs_wu_param(
                target_read_existing_int_msgs_wu,
            )
        {
        ...
        let pow_updated_accounts_count = wu_metrics
            ...
            .pow_updated_accounts_count(
                wu_metrics.wu_params.finalize.serialize_diff,
                scale,
            );
        if let Some(target_wu_param) = wu_metrics
            .wu_on_do_collate
...
impl WuMetricsAvg {
    ...
    pub fn accum(&mut self, v: &WuMetrics) {
        ...
        avg_accum_list!(
            self; v.wu_on_finalize.in_msgs_len, v.wu_on_finalize.out_msgs_len, v
            .wu_on_execute.inserted_new_msgs_count,
        );
        avg_accum_list!(
            self; v.wu_on_prepare_msg_groups.fixed_part, v.wu_on_prepare_msg_groups
            .read_ext_msgs_count, v.wu_on_prepare_msg_groups.read_ext_msgs_wu, v
            .wu_on_prepare_msg_groups.read_ext_msgs_elapsed.as_nanos(), v
            .wu_on_prepare_msg_groups.read_existing_int_msgs_count, v
            .wu_on_prepare_msg_groups.read_existing_int_msgs_wu, v
            .wu_on_prepare_msg_groups.read_existing_int_msgs_elapsed.as_nanos(), v
            .wu_on_prepare_msg_groups.read_new_int_msgs_count, v.wu_on_prepare_msg_groups
            .read_new_int_msgs_wu, v.wu_on_prepare_msg_groups.read_new_int_msgs_elapsed
            .as_nanos(), v.wu_on_prepare_msg_groups.add_to_msgs_groups_ops_count, v
            .wu_on_prepare_msg_groups.add_msgs_to_groups_wu, v.wu_on_prepare_msg_groups
            .add_msgs_to_groups_elapsed.as_nanos(), v.wu_on_prepare_msg_groups
            .total_elapsed.as_nanos(),
        );
        avg_accum_list!(
            self; v.wu_on_execute.groups_count, v.wu_on_execute.sum_gas, v.wu_on_execute
            .avg_group_accounts_count.get_avg_checked().unwrap_or_default(), v
            .wu_on_execute.avg_threads_count.get_avg_checked().unwrap_or_default(), v
            .wu_on_execute.execute_groups_vm_only_wu, v.wu_on_execute
            .execute_groups_vm_only_elapsed.as_nanos(), v.wu_on_execute.process_txs_wu, v
            .wu_on_execute.process_txs_elapsed.as_nanos(),
        );
        avg_accum_list!(
            self; v.wu_on_finalize.diff_msgs_count, v.wu_on_finalize
            .create_queue_diff_wu, v.wu_on_finalize.create_queue_diff_elapsed.as_nanos(),
            v.wu_on_finalize.apply_queue_diff_wu, v.wu_on_finalize
            .apply_queue_diff_elapsed.as_nanos(), v.wu_on_finalize
            .update_shard_accounts_wu, v.wu_on_finalize.update_shard_accounts_elapsed
            .as_nanos(), v.wu_on_finalize.build_accounts_blocks_wu, v.wu_on_finalize
            .build_accounts_blocks_elapsed.as_nanos(), v.wu_on_finalize
            .build_accounts_elapsed.as_nanos(), v.wu_on_finalize.build_in_msgs_wu, v
            .wu_on_finalize.build_in_msgs_elapsed.as_nanos(), v.wu_on_finalize
            .build_out_msgs_wu, v.wu_on_finalize.build_out_msgs_elapsed.as_nanos(), v
            .wu_on_finalize.build_accounts_and_messages_in_parallel_elased.as_nanos(), v
            .wu_on_finalize.build_state_update_wu, v.wu_on_finalize
            .build_state_update_elapsed.as_nanos(), v.wu_on_finalize.build_block_wu, v
            .wu_on_finalize.build_block_elapsed.as_nanos(), v.wu_on_finalize
            .finalize_block_elapsed.as_nanos(), v.wu_on_finalize.total_elapsed
            .as_nanos(),
        );
        avg_accum_list!(
            self; v.wu_on_do_collate.resume_collation_wu, v.wu_on_do_collate
            .resume_collation_elapsed.as_nanos(), v.wu_on_do_collate
            .resume_collation_wu_per_block, v.wu_on_do_collate
            .resume_collation_elapsed_per_block_ns, v.wu_on_do_collate
            .collation_total_elapsed.as_nanos(),
        );
    }
    pub fn get_avg(&mut self) -> Option<WuMetrics> {
        ...
        Some(WuMetrics {
            ...
            wu_on_prepare_msg_groups: PrepareMsgGroupsWu {
                ...
                read_ext_msgs_elapsed: Duration::from_nanos(
            ...
            wu_on_execute: ExecuteWu {
                ...
                avg_group_accounts_count: SafeUnsignedAvg::with_initial(
                    self.avg.get_avg_next(),
                ),
                avg_threads_count: SafeUnsignedAvg::with_initial(
                    self.avg.get_avg_next(),
                ),
                execute_groups_vm_only_wu: self.avg.get_avg_next() as u64,
                execute_groups_vm_only_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
            ...
            wu_on_finalize: FinalizeWu {
                ...
                create_queue_diff_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                apply_queue_diff_wu: self.avg.get_avg_next() as u64,
                apply_queue_diff_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                ...
                update_shard_accounts_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                build_accounts_blocks_wu: self.avg.get_avg_next() as u64,
                build_accounts_blocks_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                build_accounts_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                build_in_msgs_wu: self.avg.get_avg_next() as u64,
                build_in_msgs_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                build_out_msgs_wu: self.avg.get_avg_next() as u64,
                build_out_msgs_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                ...
                build_state_update_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                build_block_wu: self.avg.get_avg_next() as u64,
                build_block_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                finalize_block_elapsed: Duration::from_nanos(
                    self.avg.get_avg_next() as u64,
                ),
                total_elapsed: Duration::from_nanos(self.avg.get_avg_next() as u64),
            },
...
fn report_wu_params(
    curr_wu_params: &WorkUnitsParams,
