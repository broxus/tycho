...
fn print_hist(label: &str, h: &Histogram<u64>) {
    ...
    println!("{label}:");
    println!("  count = {}", h.len());
    let printer = SpanPrinter::new().fractional(Some(FractionalUnit::Millisecond));

    let fmt = |v| printer.span_to_string(&Span::new().microseconds(v as i64));

    println!("  p50   = {}", fmt(p50));
    println!("  p90   = {}", fmt(p90));
    println!("  p99   = {}", fmt(p99));
    println!("  p99.9 = {}", fmt(p999));
    println!("  max   = {}", fmt(max));
}
