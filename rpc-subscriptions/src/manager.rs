...
mod tests {
    ...
    fn op_strategy() -> impl Strategy<Value = Op> {
        prop_oneof![
            ...
            (Just(0u8), 0u8..20).prop_map(|(c, a)| Op::Sub(c, a)),
            (Just(0u8), 0u8..20).prop_map(|(c, a)| Op::Unsub(c, a)),
            // Occasional random noise
            (0u8..10, 0u8..10).prop_map(|(c, a)| Op::Sub(c, a)),
            (0u8..10).prop_map(Op::Remove),
        ]
    }

    proptest::proptest! {
        #[test]
        fn prop_heavy_load(ops in proptest::collection::vec(op_strategy(), 1..200)) {
