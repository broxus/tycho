...
impl Subscriptions {
        ...
        I: IntoIterator<Item = StdAddr>,
    {
        ...
        let mut entry = match self.client_to_int_addrs.entry(client_id) {
        ...
        for addr in addrs {
            ...
            // Retries when the forward entry changes after we update the reverse index.
            loop {
                let Some((addr_id, created)) = self.addr_to_subs_ensure(addr) else {
                    if entry.get().is_empty() {
                        let subs = entry.remove();
                ...
                let (add_result, delta) = entry.get_mut().add_with_delta(addr_id);
                self.tracker.client_to_int_addrs.add_bytes(delta);

                match add_result {
                    AddResult::Inserted | AddResult::AlreadyPresent => {
                        if self.addr_to_subs_add_client(addr, addr_id, client_id) {
                            break;
                        }

                        entry = self.client_to_int_addrs_rollback(entry, addr_id, client_id);
                    }
                    AddResult::AtCapacity => {
                        if created
                            && let Entry::Occupied(se) = self.addr_to_subs.entry(addr)
    ...
    #[inline]
    fn client_to_int_addrs_rollback(
        &self,
        mut ce: OccupiedEntry<'_, ClientId, IntAddrs>,
        addr_id: InternedAddrId,
        client_id: ClientId,
    ) -> OccupiedEntry<'_, ClientId, IntAddrs> {
        let _ = ce.get_mut().remove(addr_id);
