...
Your image build locally may not run in k8s because of `Docker exit code 132 (Unsupported instruction)`.
...
### Relation to simulator chart

`simulator` applies files in the next order:
* [./default_values.yaml](./default_values.yaml)
* `./values.yaml` created manually to override defaults
* `../tycho/values.yaml` created by `simulator prepare` that may be modified manually

Note that any `simulator/**/values.yaml` file is git-ignored.
